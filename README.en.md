...
> 💡 **[PureScience v1.45.0 released](https://github.com/naiyixi/PureScience/releases/latest)** — fix & UX batch: **DeepSeek's multimodal model is now independently selectable** for image understanding even when the main model is text-only, CodeBuddy gains a real npm install source, manual-installer updates tell you to quit the app first (9 locales), the home page scrolls its project list and shows 10 recent sessions, and the credentials/memory/tags/routine/endpoint settings were re-aligned to one visual standard with every switch and CTA in brand blue. Version history with release code names: [CHANGELOG](CHANGELOG.md) · Website: [purescience.work](https://purescience.work)
