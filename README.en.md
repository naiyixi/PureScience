...
> 💡 **[PureScience v1.43.0 released](https://github.com/naiyixi/PureScience/releases/latest)** — the ecosystem-closing release: a full **English README** (README.en.md), a **weekly reference-release snapshot cron** that turns the reference product's release notes into a rolling gap list (brand-zero-trace, internal only), and **GitHub-visibility work** (platform badge, honest internal-acceptance benchmark page, Zenodo DOI archiving runbook). Plus fixes: figure_review restored (RPC gateway contract alignment), in-app check-for-updates no longer 404s (version.json is now published with every release), and README platform claims corrected to the real three-platform reality. Version history with release code names: [CHANGELOG](CHANGELOG.md) · Website: [purescience.work](https://purescience.work)
