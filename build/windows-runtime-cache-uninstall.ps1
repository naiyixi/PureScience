...
function Test-TrustedCache([string]$Path, [string]$CanonicalRoot, [string]$UserIdentity) {
  ...
  $marker = Get-Content -LiteralPath (Join-Path $Path '.purescience-cache.json') -Raw |
    ConvertFrom-Json
...
[void]$roots.Add((Join-Path $env:USERPROFILE 'PureScience\runtime'))
[void]$roots.Add((Join-Path $env:USERPROFILE '.purescience\runtime'))
$settingsPath = Join-Path $env:USERPROFILE '.purescience\settings.json'
