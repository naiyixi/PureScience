...
!macro preserveNestedDataRoot DIR BACKUP SLOT
  ...
  ${if} "${DIR}" != ""
    ...
    ${if} ${Errors}
      DetailPrint `Could not safely preserve "${DIR}\PureScience"; its parent path could not be resolved.`
      MessageBox MB_OK|MB_ICONSTOP "PureScience could not safely protect its data folder before updating.$\r$\nThe existing data was left untouched."
      StrCpy $dataProtectionFailed "1"
    ${else}
      StrCpy ${BACKUP} "$R2\.purescience-update-data-${SLOT}"
      ${if} ${FileExists} "${DIR}\PureScience\*.*"
        ${if} ${FileExists} "${BACKUP}\*.*"
          DetailPrint `Could not safely preserve "${DIR}\PureScience" because the backup path already exists: ${BACKUP}`
          MessageBox MB_OK|MB_ICONSTOP "PureScience found both the current data folder and an earlier update backup.$\r$\nNo data was changed. Please inspect:$\r$\n${BACKUP}"
          StrCpy ${BACKUP} ""
          StrCpy $dataProtectionFailed "1"
        ${else}
          ClearErrors
          Rename "${DIR}\PureScience" "${BACKUP}"
          ${if} ${Errors}
            DetailPrint `Could not safely preserve "${DIR}\PureScience"; leaving the existing installation untouched.`
            MessageBox MB_OK|MB_ICONSTOP "PureScience could not safely protect its data folder before updating.$\r$\nThe existing data was left untouched."
            StrCpy ${BACKUP} ""
            StrCpy $dataProtectionFailed "1"
          ${else}
  ...
  ${andIf} "${BACKUP}" != ""
    ${if} ${FileExists} "${BACKUP}\*.*"
      ${if} ${FileExists} "${DIR}\PureScience\*.*"
        ...
        MessageBox MB_OK|MB_ICONSTOP "PureScience could not restore its data folder because the destination already exists.$\r$\nThe preserved data remains at:$\r$\n${BACKUP}"
        StrCpy $dataRestoreFailed "1"
      ${else}
        CreateDirectory "${DIR}"
        ClearErrors
        Rename "${BACKUP}" "${DIR}\PureScience"
        ${if} ${Errors}
          DetailPrint `The preserved data remains at: ${BACKUP}`
          MessageBox MB_OK|MB_ICONSTOP "PureScience could not restore its data folder after updating.$\r$\nThe preserved data remains at:$\r$\n${BACKUP}"
          StrCpy $dataRestoreFailed "1"
        ${else}
          DetailPrint `Restored the data folder to: ${DIR}\PureScience`
          StrCpy ${BACKUP} ""
        ${endif}
      ${endif}
    ${else}
...
!macro customPageAfterChangeDir
  !ifdef MUI_PAGE_CUSTOMFUNCTION_PRE
    !define purescienceOriginalInstFilesPre ${MUI_PAGE_CUSTOMFUNCTION_PRE}
...
!macro customInit
  ...
  # More importantly, move any nested PureScience data root OUTSIDE the install directory before
...
!macro customHeader
  Function protectNestedDataRootsForInstall
    !ifdef purescienceOriginalInstFilesPre
      Call ${purescienceOriginalInstFilesPre}
    !endif
    !insertmacro protectMachineDataRootForSelectedMode
  FunctionEnd
...
!macro uninstallFailureRecoveryAt DIR REGISTERED_BACKUP
  ...
  ${else}
    ...
    # ANY directory — purescience.exe covers the app and its Electron-as-Node children (the
    ...
    # <install>\PureScience). The updated uninstaller moves EVERY child of ${DIR} into its
    ...
    ${if} $R7 != ""
      ...
      ${andIf} ${FileExists} "${REGISTERED_BACKUP}\*.*"
        ...
        MessageBox MB_OK|MB_ICONEXCLAMATION "PureScience found additional data created while closing the previous version.$\r$\nYour original data will be restored; the additional data remains at:$\r$\n$R7"
      ${else}
...
!macro customUnInstallCheck
  ...
  # a live old process can recreate PureScience, and restoring first would make the recovery path
