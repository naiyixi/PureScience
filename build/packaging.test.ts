...
describe('NSIS installer include (build/installer.nsh)', () => {
  ...
  it('runs the retry with TEMP and TMP on the old installation volume', () => {
    ...
    // cross-volume move/copy — including a nested PureScience data root — and can abort with exit
  ...
  it('preserves a nested PureScience data root around the uninstall retry', () => {
    // The reporter's configured data root is <install dir>\PureScience. An updated NSIS
    ...
    const recovery =
      include.match(/!macro uninstallFailureRecoveryAt DIR([\s\S]*?)!macroend/)?.[1] ?? ''
    const preserveAt = recovery.indexOf('Rename "${DIR}\\PureScience" "$R7"')
    const retryAt = recovery.indexOf('ExecWait')
    const restoreAt = recovery.indexOf('Rename "$R7" "${DIR}\\PureScience"')
  ...
  it('keeps registered data backups outside the install tree until recovery finishes', () => {
    // A still-running old process can recreate <install>\PureScience after customInit moved the
    ...
    expect(conflictBranch).not.toContain('Rename "$R7" "${DIR}\\PureScience"')
  })
  ...
  it('protects registered nested data roots before the first old-uninstaller attempt', () => {
    ...
    // <install>\PureScience into its disposable $PLUGINSDIR. customInit runs before the install
  ...
  it('waits for the final all-users mode before touching a distinct machine data root', () => {
    ...
    expect(header).toContain('Call ${purescienceOriginalInstFilesPre}')
