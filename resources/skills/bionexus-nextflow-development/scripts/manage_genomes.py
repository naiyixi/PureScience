...
def get_cache_dir() -> Path:
    ...
    cache_dir = os.environ.get("NF_CORE_GENOME_CACHE", os.path.expanduser("~/.nf-core/genomes"))
...
def download_genome(genome_id: str, components: Optional[List[str]] = None, force: bool = False) -> bool:
    """
    ...
    # Check for AWS CLI
    aws_available = subprocess.run(["which", "aws"], capture_output=True).returncode == 0
...
Examples:
    ...
    list_parser.add_argument("--installed", action="store_true", help="Show only installed genomes")
    ...
    dl_parser.add_argument("--components", nargs="+", help="Specific components (fasta, gtf, bwa_index, star_index)")
    ...
    params_parser.add_argument("--json", action="store_true", help="Output as JSON")
    ...
    if args.command == "list":
        ...
        if args.json:
            print(json.dumps({"genome": resolved, "installed": installed, "path": str(path) if path else None}))
        else:
            if installed:
