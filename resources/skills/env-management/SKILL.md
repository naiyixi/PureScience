---
name: env-management
description: 当 notebook 运行因缺少软件包失败（ImportError、ModuleNotFoundError、"there is no package called"）、需要检查已安装软件包版本、或需要为 notebook 运行时安装、添加或管理 Python 或 R 软件包时使用。涵盖 inspect_packages、通过 manage_packages 路由 Python 与 R、为何禁止单元格内 %pip/!pip/install.packages() 和系统安装器、安装后重启内核、以及何时停止并询问用户。
license: Apache-2.0
---
