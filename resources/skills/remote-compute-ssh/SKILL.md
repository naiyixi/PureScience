...
description: 发现并使用 SSH 计算主机。当需要运行远程命令或提交带自动采集和分析的长时作业时加载。
license: Apache-2.0
---
...
With `login_shell: true`, the remote Bash login profiles run first and then PureScience attempts to
...
//   remote_workdir: '.purescience/jobs/<job_id>',
