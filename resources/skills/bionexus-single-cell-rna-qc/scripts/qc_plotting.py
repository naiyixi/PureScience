...
def plot_qc_distributions(adata, output_path, title="Quality Control Metrics"):
    ...
    axes[2, 1].scatter(adata.obs["total_counts"], adata.obs["pct_counts_mt"], alpha=0.5, s=10, color="coral")
    axes[2, 1].set_xlabel("Total counts")
    axes[2, 1].set_ylabel("Mitochondrial %")
    axes[2, 1].set_title("Total Counts vs Mitochondrial %")

    axes[2, 2].scatter(adata.obs["n_genes_by_counts"], adata.obs["pct_counts_mt"], alpha=0.5, s=10, color="forestgreen")
...
def plot_filtering_thresholds(adata, outlier_masks, thresholds, output_path):
    ...
    def plot_with_threshold(ax, metric, outlier_mask, n_mads, hard_threshold=None):
        ...
        if hard_threshold is not None:
            ax.axvline(
                hard_threshold, color="darkred", linestyle=":", linewidth=2, label=f"Hard threshold ({hard_threshold})"
            )
    ...
    for idx, (metric, label) in enumerate(metrics):
        if metric in outlier_masks and metric in thresholds:
            ...
            plot_with_threshold(axes[idx], metric, outlier_masks[metric], thresholds[metric]["n_mads"], hard)
...
def plot_qc_after_filtering(adata, output_path):
    ...
    axes[1, 1].scatter(adata.obs["total_counts"], adata.obs["pct_counts_mt"], alpha=0.5, s=10, color="coral")
    axes[1, 1].set_xlabel("Total counts")
    axes[1, 1].set_ylabel("Mitochondrial %")
    axes[1, 1].set_title("Total Counts vs Mitochondrial %")

    axes[1, 2].scatter(adata.obs["n_genes_by_counts"], adata.obs["pct_counts_mt"], alpha=0.5, s=10, color="forestgreen")
