...
runtime_text_references_managed <- function(text) {
  ...
  grepl("PURESCIENCE_RUNTIME_DIR", text, fixed = TRUE) ||
...
runtime_value <- Sys.getenv("PURESCIENCE_RUNTIME_DIR", "")
source_root <- NULL
...
run <- base::local({
  ...
  blank_capture_hashes <- function() {
    blank_dir <- tempfile("purescience-blank-r-", tmpdir = kernel_figures_dir)
  ...
  create_capture_page_dir <- function() {
    for (parent_dir in c(kernel_figures_dir, tempdir())) {
      page_dir <- tempfile("purescience-r-pages-", tmpdir = parent_dir)
  ...
  install_capture_binding_wrapper <- function(package, name, make_wrapper) {
    ...
    for (env in envs) {
      ...
      if (isTRUE(attr(original, "purescience_capture_wrapper", exact = TRUE))) {
        next
      }
      wrapper <- make_wrapper(original)
      attr(wrapper, "purescience_capture_wrapper") <- TRUE
