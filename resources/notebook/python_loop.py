...
_protected_dirs = [
    _guard_path(entry)
    for entry in os.environ.get("PURESCIENCE_PROTECTED_DIRS", "").split(os.pathsep)
    if entry
]
_runtime_dir_value = os.environ.get("PURESCIENCE_RUNTIME_DIR", "")
...
def _text_references_managed_runtime(text):
    ...
    return bool(root and root in comparable) or "PURESCIENCE_RUNTIME_DIR" in str(text)
...
import venv as _purescience_venv
_purescience_venv.create = _blocked_environment_mutation
_purescience_venv.EnvBuilder.create = _blocked_environment_mutation
try:
    import ensurepip as _purescience_ensurepip
    _purescience_ensurepip.bootstrap = _blocked_environment_mutation
except ImportError:
    pass
try:
    import pip._internal as _purescience_pip_internal
    import pip._internal.cli.main as _purescience_pip_cli
    import pip._internal.commands as _purescience_pip_commands
    _purescience_pip_internal.main = _blocked_environment_mutation
    _purescience_pip_cli.main = _blocked_environment_mutation
    ...
    _purescience_pip_create_command = _purescience_pip_commands.create_command
    def _guarded_pip_create_command(name, *args, **kwargs):
        if str(name).strip().casefold() in ("install", "uninstall"):
            _blocked_environment_mutation()
        return _purescience_pip_create_command(name, *args, **kwargs)
    _purescience_pip_commands.create_command = _guarded_pip_create_command
    if hasattr(_purescience_pip_cli, "create_command"):
        _purescience_pip_cli.create_command = _guarded_pip_create_command
