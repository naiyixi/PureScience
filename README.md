...
> 💡 **[PureScience v1.71.0 released](https://github.com/naiyixi/PureScience/releases/latest)** — Capabilities that only went one way can now be walked back. Five of them had their work implemented and reachable from the command layer while the interface stopped halfway: a collection could be added to but not left; a runtime could be discovered but not deregistered, because the removal compared the path as typed against the resolved one and so matched nothing; a session package could be exported, but from no entry; a version could be replayed with its result never shown; and a turn interrupted by a restart could be sent again but not continued — sending it again asks the same question twice, while continuing hands the recorded turn back so the agent carries on from where it stopped. Continuing was broken in three ways that reading the code did not reveal: the guard that accepts a continuation required a record nothing ever wrote, so the action could not succeed for anyone; the request never attached a session first, and the agent layer refuses a turn whose session it cannot find; and clearing the interrupted state writes the session back without that record, so clearing it before asking erased the very record the request was about to be checked against. Each was found by driving the running app and reading what the session and the agent actually did, and the end-to-end spec that proves the continued answer was stored now passes. Provenance verification names the file whose digest disagrees, its on-disk digest and the runs that reference it instead of reporting a checksum alone, and the fixture no longer borrows the developer’s own data root — it isolated the storage root only for packaged runs while the data root is derived from it, which is why one test passed in CI and failed on a workstation. A screen-reader defect came out of the same work: the composer’s group label was a template that never evaluated, so what reached the accessibility tree was `{t('workspace.sendMessage')} options`.
