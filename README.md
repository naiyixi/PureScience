...
> 💡 **[PureScience v1.52.0 released](https://github.com/naiyixi/PureScience/releases/latest)** — **GB/T 7714-2015 citation export** (Chinese-journal sequential numbering with 等/et al. author handling, [J]/[EB/OL] record typing, per-citation copy and whole-view .txt export) plus a **PDF annotation loop** — attach a project PDF to any library record and annotate passages with page-numbered provenance in the existing Files-panel preview — and the library UI fully localized across 9 locales. Agent tool catalogs stay deny-first: zero reference-library tools are exposed to agents. Quality baseline: dual-config typecheck clean, Nightly + Windows CI green. Version history with release code names: [CHANGELOG](CHANGELOG.md) · Website: [purescience.work](https://purescience.work)
