...
- ✅ **Shipped and machine-verified**: multi-agent orchestration and delegated subagents · Python and R runtimes with audited notebook runs · artifact versions and lineage · **reproducibility verification** (isolated re-run + byte-exact per-file comparison + environment fingerprint) · the review-to-evidence loop · a nine-language interface · a signed specialist marketplace (24 Chinese-medical packages, 18 skill packs) · 24 scientific connectors (209 tools: gnomAD, Ensembl, UCSC, STRING, PubMed, OpenAlex, …) plus ENA run metadata that reports the bytes and MD5 each archive declares instead of downloading it · **PDF text, tables, figures and captions** with page-level geometry (`pdf_tables`, `pdf_figures`) · **gene-set enrichment recomputed locally** from the integers a service reports · **NCBI reference sequences with SHA-256, byte count and base count** · global search · `.science` session packages · **a citation-style layer** (nine built-in styles plus CSL import) · **private session bookmarks** pinned to an exact artifact version · **session forking** that carries a count first and a provenance link back · a session information card · and a command line whose `doctor` reports measured facts while the readiness verdict stays where it belongs.
- 🚧 **Partly done**: **CSL** import runs a **fidelity probe** against each document — of eight real published styles, five pass and three are labelled drafts with the missing piece named (their author macros branch on record types we do not model yet). ENA is metadata and reachability only: no FASTQ is downloaded. Distribution has just started — both READMEs lead with a result the product produced and nine languages now exist — but the download site is not deployed yet and the repository is still young.
- 🗺️ **Planned**: one more enrichment service beside the one already available · filter columns and pinned filters in global search · one shared diff viewer used by both skill updates and session-package record verification · a provider save gate with runtime availability kept in sync · explicit guidance when a plan is resumed · `.science` packages that carry the literature PDFs behind their citations · a wider serverless GPU surface.
...
> 💡 **[PureScience v1.68.0 released](https://github.com/naiyixi/PureScience/releases/latest)** — A `.science` session package now carries the papers the session cited, and an imported package lands a session you can actually read. The library's PDFs travel under `references/…` on the same terms as the session's own files: injected storage, a byte bound, a stable suffix when two papers share a file name, and a name (never a silent drop) for every PDF that did not make it — carried in their own manifest field, so a reader can still tell a session artifact from a paper the session cited. Three defects came out of verifying that on a real machine, none of them reachable from a fixture: the port looked for the *current* attachment in the history table that only records *replaced* files, so every package shipped zero papers; an imported session was written without container timestamps, so the graph the app materializes carried frames its own validator drops, and the document was quarantined as `.invalid-` — an import that "succeeded" into a session nobody could open; and the read-only posture was written beside the imported session with nothing reading it, so an imported session could be resumed and run like any other. All three are fixed, with a source guard that fails if anything asks the version history again, a round trip through the app's own session-file validation (plus the negative control showing the timestamps are what carry it), and a refusal at Resume — the one door that makes a restored session runnable — with a session this machine ran itself as the control. A fourth fix came from the scan that ran after the import: the sidecar that keeps an imported session read-only was itself read as a session document, failed to normalize, and was quarantined, which silently destroyed the posture. Records are metadata now, like summary caches, and a test writes one beside a real session and asserts the scan leaves it where it is. Real-machine evidence: an isolated instance, a real project, a real arXiv PDF, a real agent turn that read the paper, export and import through the desktop channels, and every packaged member independently re-hashed with Python's zipfile against the source file and the checksum the application itself recorded (29/29). Also in this release: a newly added provider must be reachable before it counts as saved (with the gate extracted into a testable function), a settings-store snapshot that crashed the whole settings page when a framework list was missing, the provider panel saying it is applying changes to the runtime, and a provider that cannot be reached now giving an actionable next step in one shared sentence across nine languages. Quality baseline: dual typecheck, whole-tree lint, the full suite, and the PR gate + Nightly green.
...
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| macOS / Windows / Linux | Signed installers for every platform are published on each [release](https://github.com/naiyixi/PureScience/releases/latest) — pick yours from the Assets list |
...
Between 2026-09-04 and 2026-09-07, PureScience was handed **30 adversarial research challenges** by external reviewers — no demo data, no curated inputs. The reviewer spread them across five daily sessions, and the runs themselves finished in **~6 hours of total runtime on a single Apple M2 laptop with 8 GB of RAM**. Each challenge described a state-of-the-art but deeply flawed research claim (a kinase "selectivity" chemistry cannot support, an antibody whose affinity defeats its own brain delivery, a condensate drug that "dissolves" a transcription factory by gelling it, a CAR whose affinity can never compensate sparse antigen…) and demanded a mechanism-level verdict. Every run ended with a versioned deliverable set: a Chinese adjudication report (with its own Data-Gap list), the solver code, and publication-grade figures — visible in the project's Files panel and replayable from the conversation. Every wave also carries an external reviewer scorecard — see the [scoring archive](docs/extreme-tests-2026-09/SCORECARD.md).
...
_The projects page after the marathon — every row lists the actual file types its session produced (MD · PY · PNG · CSV · HTML)._
...
> **中文速览（2026-09 极限压测）**：30 道研发委员会级对抗性压测（激酶同源选择性 → 血脑屏障转胞吞 → 相分离凝胶化 → 免疫突触 → 演化博弈…）于 09-04 → 09-07 在一台 Apple M2 8GB 笔记本上完成 —— **合计计算 ≈6 小时，评审分 5 天逐批发出**；每题交付「中文裁决研报 + 数值求解器 + 出版级图件」，全部保留在会话 Files 面板、可版本回放；报告延续「Data Gap 清单」与「已验证 / 无法核实」分离口径。压测中系统还多次驳回出题方的诱导性前提（如「强表达必致混沌」被数值结论直接否定）。**全量证据已入库**：30 个案例文件夹（研报/代码/图件）见 [`docs/extreme-tests-2026-09/cases/`](docs/extreme-tests-2026-09/cases/)，带会话截图与逐题一句话裁决的完整图库见 [`docs/extreme-tests-2026-09/README.md`](docs/extreme-tests-2026-09/README.md)，**历轮评审打分**（外部评审 1–10 分/等级制原表）见 [`SCORECARD.md`](docs/extreme-tests-2026-09/SCORECARD.md)；实证介绍 PPT（含会话截图与产物图）随 Release 资产与演示素材提供。
...
| **Distribution and support**   | Signed, notarized installers for macOS (Apple Silicon and Intel), Windows, and Linux published with every release, plus update guidance, local diagnostics, and community links.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
