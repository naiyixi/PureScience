...
> 💡 **[PureScience v1.65.0 released](https://github.com/naiyixi/PureScience/releases/latest)** — The citation layer learned real CSL. Nine built-in styles (GB/T 7714-2015 first, byte-compatible with the previous renderer) plus a deliberately limited CSL compiler that refuses to install a style without a licence and **names every construct it cannot render** rather than pretending fidelity. Against the eight real `.csl` documents a researcher actually imports, the outcome moved from **5 verified and 3 partial** — APA, MLA and Chicago silently lost their authors — to **8 of 8 verified**, authors and titles both present: those styles ask an author macro for a variable we hold no data for and fall back through `substitute`, which the compiler now honours. Building that surfaced a worse self-inflicted defect: a substitute compiled inside another macro shared its expansion cache and stored a depth-truncated expansion the layout still needed, so APA went from "title but no authors" to "authors but no title" — caught by an A/B against the stashed file, fixed by giving fallbacks their own expansion state, and pinned by a regression test verified red on the old behaviour. Sessions fork by **measuring first**: the card shows what the copy will hold (messages, agent replies, artifact references, uploads) and names the three things it will not carry (activity links, run state, duplicated versions), and only a separate confirmation writes anything — every message gets a new id, reply links are remapped, artifact references are inherited, the source stays **byte-identical**, and the copy carries its lineage. A fork of an imported read-only session comes out writable, with the posture dropped and the provenance kept. Probing that on the packaged app found a real defect worth more than the feature: the card is re-mounted while it is open, so a measured plan kept in component state was discarded by a background update — the numbers now live per session, pinned by a test that mounts a second instance and requires them back. Bookmarks can be taken from a preview passage and jumped back to the **exact Artifact Version** it was read from; a version that no longer exists opens nothing, because substituting whatever carries the file name now would put different bytes behind a saved passage. The `pdf-region` anchor is filed as work, not claimed: the model and validation exist, the creation path does not. Quality baseline: dual typecheck, whole-tree lint, the full suite — **1049 files / 13996 tests** — and Nightly + Windows CI green, with two new real-window certification specs in the p0 lane.
