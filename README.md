...
> 💡 **[PureScience v1.56.0 released](https://github.com/naiyixi/PureScience/releases/latest)** — The science-delivery chain, with honesty wired into every step: **figure→data** digitisation (two-anchor linear/log calibration that refuses degenerate anchors, a strict pick session, and CSV that carries its estimation provenance plus an audit verdict — estimated output routes to review, never straight into a manuscript); **probe-before-compute for large omics files** (h5ad structure read read-only, VCF streamed, with the "基于 N/M 降采样" scope label derived from the manifest so a subset can never pass as a full answer, and a full-run proposal that always requires approval); and an **engine policy** — a catalog of seven engines that declares measured vs predicted output, GPU needs, weight sizes and licences, a single availability gate (no silent downloads, no GPU claims without a probed accelerator), routes that send folding and physics-based ΔΔG to an approval-gated remote host while labelling CPU predictions as predictions, and "not computed: <what> — requires <engine or host>" when nothing can serve the request. SciBench-Local grew to seven cases / thirteen rules. Quality baseline: dual-config typecheck clean, compute cluster 526 green, Nightly + Windows CI green. Version history with release code names: [CHANGELOG](CHANGELOG.md) · Website: [purescience.work](https://purescience.work)
