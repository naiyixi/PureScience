...
> 💡 **[PureScience v1.47.0 released](https://github.com/naiyixi/PureScience/releases/latest)** — quality & UX batch: **DeepSeek's multimodal model is now selectable for image understanding in every setup** (the vision picker derives candidates from the vendor catalog instead of the active-model list, so a text main model no longer hides deepseek-v4-flash-vision-exp), the full tree is **lint-clean (0 problems)** after fixing all 25 hook-dependency gaps (root cause: the no-provider i18n fallback `t` is now a stable singleton), formatting was normalized repo-wide with zero behavior change, and a **pre-push gate keeps README.md and README.en.md byte-synced** (badges / intro / banner / DOI). Version history with release code names: [CHANGELOG](CHANGELOG.md) · Website: [purescience.work](https://purescience.work)
