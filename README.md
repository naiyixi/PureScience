...
> 💡 **[PureScience v1.60.0 released](https://github.com/naiyixi/PureScience/releases/latest)** — Background results now come home on their own, and everything about them is inspectable. **Delivery moved into the main process**: a finished job's result is written back into its session even if the window was closed — or if the job finished while the app was shut and lands on the next launch — with one turn per batch instead of one per result, and the renderer's old delivery path deleted behind a guard test that only one path may ever be live. **A delivery ledger** on the job shows state, content fingerprint, trigger, files and time, copyable as a line anyone can recheck, and the interface language is now reported to the main process so the continuation turn and the ledger speak the language the window was last showing — even written with no window open — from one shared nine-language label set. **Three privacy gates**: a plaintext endpoint needs that provider's explicit opt-in (refused when saving, when probing, and at spawn — the last line of defence sits in the environment assembly, so a hand-edited settings file is still caught), image metadata is stripped before sending from **both** fresh attachments and history images (the latter was a real gap; verified on a live app byte for byte: what goes out is the original minus its metadata segments, pixels untouched), and remote images in replies are not fetched until the reader clicks. **PDF tables** extract with the two methods honestly separated and exports that carry page, method, shape and confidence plus a mandatory "candidate — verify against the source" note. **Session packages** expand one member at a time (peak memory: archive plus one member). The command line gained `purescience ready`, `runtime list` and `connectors list`, which project the app's own judgements rather than deciding anything themselves. **Small print, done properly**: the Codex ACP adapter and its native CLI now show their versions side by side, long conversations are bounded at the write path with the drop recorded and stated above the transcript, the marketplace installs everything you tick in one pass (only what validated, failures named, one failure never abandons the rest), and multiple prompts for the same credential settle in a single decision. **And a regression that would have stopped the app from starting at all** — a new application command shipped without its inventory certification, so composition refused to boot — was caught by live verification and fixed. Quality baseline: dual-config typecheck, lint, the full suite, and Nightly + Windows CI green. PDF DOI extraction moves to v1.61.0 rather than shipping half of it. Version history: [CHANGELOG](CHANGELOG.md) · Website: [purescience.work](https://purescience.work)
