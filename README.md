...
> 💡 **[PureScience v1.69.0 released](https://github.com/naiyixi/PureScience/releases/latest)** — The last thirty centimetres: an entry-layer audit counted 431 renderer contract surfaces, 33 capabilities whose backend exists with no UI caller at all, 61 collection surfaces and 11 overlays with no keyboard way out — and this release closes the keyboard first. The `#` session picker was a black hole for keys: arrows did nothing and Enter neither selected nor sent, because the composer hands those keys to the picker and the picker never claimed them; it now answers arrows, Enter and Escape, and lets Enter commit the message when nothing matches. Four hand-rolled overlays (folder grants, references library, session bookmarks, annotation) move onto the app's Radix dialog shell, gaining Escape, a focus trap and initial focus. Closing one hands focus back: Radix restores focus to its own `Dialog.Trigger`, which almost no dialog here has — the open flag lives in page state — so focus landed on `<body>` and dropped keyboard users at the top of the document; a hook now records the opener and returns to it, re-asserting once an exit animation has released its focus trap. The file browser's go-to menu takes focus and returns it on Escape, its directory listing is a single tab stop the arrow keys and Home/End move, the preview's file-actions menu opens on `Shift+F10` instead of right-click only, and the command palette and message bell hand focus back to the control that opened them. The accessibility suite asserts that contract against a real Electron build.
