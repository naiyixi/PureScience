...
> 💡 **[PureScience v1.67.0 released](https://github.com/naiyixi/PureScience/releases/latest)** — Sequencing data you can check before you analyse it: the `omics_archives` connector reads ENA run metadata and, for every file the run names, reports the **byte count and MD5 the archive itself declares** plus whether the location is readable — it never downloads a FASTQ to imply it analysed one, and the run the response shape was verified against lists two files of 4.98 GB and 5.21 GB, which is exactly the point. Bulk connector changes arrive as a batch **that never hides an item**: the settings panel enables or disables every connector it is showing and reports each one as changed, already in that state, or not applied with its reason, with a summary line above them; an unknown or empty id is refused by name and writes nothing, an idempotent item is not rewritten, the whole batch triggers a single connector refresh, and the real-window check reads the outcome back out of the app's own settings store rather than trusting the list it just drew. The command line gains `init` and `doctor` under a rule the repository had already written down: **readiness has exactly one implementation, and it is the application.** `init` creates or confirms the config root at mode 0700 and writes nothing else; `doctor` reports only what it measured on this machine, each line carrying its evidence — whether the config root can really be written (a probe, not an assumption), whether the pid in the state file is still alive, whether a web token exists (reported by length, never by value), whether an installed binary can be located, and whether the running Node satisfies the requirement the package's own `engines` field declares; it exits non-zero only for its own failures and points the verdict at `ready`. Capability management stays outside the CLI, where the repository put it. Distribution now leads with a result instead of a diagram: both READMEs open on a figure the product produced (an EGFR T790M IC50 curve) pointing at the evidence directory, and the repository also speaks German, Spanish, French, Japanese, Korean, Russian and Traditional Chinese in their own landing pages — real screenshots, a three-minute start, the real installer sizes — each linking back to the full Chinese and English text. Quality baseline: dual typecheck, whole-tree lint, the full suite, and Nightly + Windows CI green, with **ten** real-window certification specs in the p0 lane.
