...
> 💡 **[PureScience v1.61.0 released](https://github.com/naiyixi/PureScience/releases/latest)** — Evidence you can re-check, not conclusions you have to trust. **Import the literature a PDF cites**: open a paper, choose “import references from this PDF”, and the DOIs it carries are resolved and written through the same paths a hand-typed reference uses — capped at five per document with the cap reported, every unresolvable identifier named with its reason, and never an entry invented to make the count look complete (live verification caught two extraction bugs that fixtures had missed: a PDF text layer drops line breaks, so an identifier swallowed the next sentence’s first word). **Session packages are checked against their own record** before they are believed: each member is compared with the digest the exporter wrote — a comparison nothing had ever made — reported per file with the first differing byte when the bytes are at hand, with “could not check” kept apart from “checked and matched”, and with reproduction and record integrity kept as two claims in two different words. **A Chinese search that reads the way it was written**: cursor paging per scope (an unreadable cursor restarts and says so), a term with no literal occurrence accepted when every two-character part of it is present and marked as such, filters (sender, file format, reference type) applied before counting and paging, and a ranking fixed by evidence — a paraphrase had been outranking the phrase itself. **Not yet verified, and named as such**: re-running a recorded artifact is built end to end (sealed recipe, isolated directory, the app’s own notebook execution, byte comparison, and `purescience replay` verified from the command line) but has no end-to-end evidence yet, because no artifact carrying recorded code exists in any environment reachable from here. WSL2 sandbox support is archived with its reopen conditions rather than half-built. Quality baseline: dual typecheck, whole-tree lint, the full suite, and Nightly + Windows CI.
