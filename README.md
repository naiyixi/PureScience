...
> 💡 **[PureScience v1.57.0 released](https://github.com/naiyixi/PureScience/releases/latest)** — The engine policy becomes an executable chain: **structure lookups are declared as database predictions** (entry, provider, tool, retrieval time and the explicit "not an experimental structure" line, with mean pLDDT labelled as a confidence metric, and a no-entry answer that points at the folding route instead of guessing a structure); **remote job templates generated from the catalogue** (slurm or direct ssh, carrying the engine's provenance, the labels the result must report, and a must-be-approved header — with derived warnings for commercial-restricted licences, predicted output, GPU engines against hosts that never reported an accelerator, on-demand weights and MSA needs); **zero-shot ESM ΔΔG with mandated calibration** — no kcal/mol without a declared factor and its source, a fixed sign convention, dispersion across models in both units, and "not computed" for unknown or non-finite scores; **on-demand weights that refuse to download without consent or a published SHA256**, re-hash their own cache and discard anything that fails verification; the **large-file omics preview** reachable from the content menu (a launcher that loads a schema-checked manifest rather than inventing counts); and a **SciBench v1 golden replay baseline** pinning the negative guarantees — no case passes without evidence, wording alone never satisfies tool-backed rules, evaluation is deterministic, and the scorecard names the failing rule ids. Quality baseline: dual-config typecheck clean, lint clean, Nightly + Windows CI green. Version history with release code names: [CHANGELOG](CHANGELOG.md) · Website: [purescience.work](https://purescience.work)
