...
> 💡 **[PureScience v1.74.0 released](https://github.com/naiyixi/PureScience/releases/latest)** — No new capability surface this time: both halves of the work were measurable only in the running application. Streaming output was narrowed layer by layer — a streamed fragment no longer re-renders the panel or the list container, save echoes and state snapshots were coalesced into windows, the broadcast log was trimmed to what has not been sent plus the overlap (about 209KB per message down to a constant ~30KB), tool-activity updates stopped re-rendering the panel and eight unrelated icons, and a plan preview now re-renders zero times per chunk — with the per-turn payload across the bridge falling from 0.47–0.85MB to 0.10–0.13MB on the same measures. Interaction cost was turned into numbers before anything was touched: typing about 7.9ms of script and 9.4ms of task per keystroke, scrolling about 7.6ms per step, one streamed turn about 100–160ms of task. The attribution is stated as measured — only ~1.0–1.2ms per keystroke is the application's own work — so no state-ownership refactor was done for 12–15%, and a standing assertion keeps typing from re-rendering the transcript instead. Focus now returns to the control that opened every dialog the interface opens from page state (both Radix families: 22 `Dialog` files and 20 `AlertDialog` files / 22 instances), including three cases only the packaged application exposed: a wholesale unmount, a dialog that focuses its own field during the commit, and an opener that itself lives inside an outer dialog. Nested modals also keep Escape to their own layer, so closing a confirm no longer tears the settings surface down with it. A static coverage guard and a real-entry migration test keep both classes of defect from returning. Every claim above was verified by driving the packaged application, not only by tests.
