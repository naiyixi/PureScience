...
> 💡 **[PureScience v1.46.0 released](https://github.com/naiyixi/PureScience/releases/latest)** — scenario-strength release: each scenario row (session details / sub-agents / review) now carries its **own reasoning-strength control** — "same as the main model" by default, or pin any level the scenario's model supports — backed by a shared scenario-backend resolver (`resolveScenarioBackend`: override ?? active model, pinned-scenario effort > global > model default) that every app-originated scenario request will route through. Review/sub-agent calls executed by the agent's own framework keep their own configuration by design. Version history with release code names: [CHANGELOG](CHANGELOG.md) · Website: [purescience.work](https://purescience.work)
