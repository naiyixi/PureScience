...
> 💡 **[PureScience v1.50.0 released](https://github.com/naiyixi/PureScience/releases/latest)** — HPC scheduler execution: registered SSH hosts can now switch between **direct-SSH and Slurm dispatch** (sbatch submission with resource-request directives, `--job-name`-keyed recovery, squeue polling, scancel timeout fallback, and a 30-day scheduler wall-clock ceiling), gated on probe-detected schedulers so no host is ever put into a mode its cluster lacks — plus a per-host execution-mode selector in Compute settings. Direct-SSH semantics and the 7-day ceiling are untouched for plain hosts. Quality baseline: dual-config typecheck clean, Nightly + Windows CI green. Version history with release code names: [CHANGELOG](CHANGELOG.md) · Website: [purescience.work](https://purescience.work)
