...
> 💡 **[PureScience v1.54.0 released](https://github.com/naiyixi/PureScience/releases/latest)** — Probe-before-compute for large omics files and a fail-closed licence stance: an `OmicsPreviewManifest` contract plus an `omics-data-preview` skill that reads h5ad structure read-only (anndata → h5py → honest failure, never a guessed cell count) and streams VCF counts, with the "基于 N/M 降采样" scope label derived from the contract so a subset can never be presented as a full answer; a full-run **proposal** that always requires approval, refuses to propose when the preview already covered the file, and lists the labels the finished result must carry; skill licences classified `allowed`/`restricted`/**`needs-review`** (unknown ⇒ confirm before commercial use) with conflict detection against the licence text; an execution-trace report (steps with evidence, artifacts, reused checkpoint facts, scope labels, mandatory honesty section) wired into the existing export pipeline; attachment version history for library PDFs; and SciBench-Local grown to six cases / eleven rules. Quality baseline: dual-config typecheck clean, Nightly + Windows CI green. Version history with release code names: [CHANGELOG](CHANGELOG.md) · Website: [purescience.work](https://purescience.work)
