...
> 💡 **[PureScience v1.58.0 released](https://github.com/naiyixi/PureScience/releases/latest)** — The self-evolution loop becomes something you can check. **Memory invites instead of dropping**: with memory off, a save request now returns a structured result naming the way to enable it, and the system prompt says not to write anywhere else — a refused save is surfaced in the answer, never silently lost. **Learnt skills carry provenance and a verification state**, and an unverified one no longer materialises until you allow it explicitly; failure-mode skills are refused without the reproduction that proves the failure, the trust state is declared at creation time and checked by a rule, and the gate now reads trust for personal skills too — where learnt entries actually live. **The supervisor wakes on evidence**: four signals, each at most once, a degrade that says it degraded, and handles carried into the review the reviewer actually runs (and into the log, so a past run stays auditable) — with the heartbeat false positive removed, so only an explicit failure counts as one. **Missing inputs are named before planning**, in the content the model reads rather than in your message. **A fetched domain is authorised once per session** (persistent capability per host, `www.` normalised, derived only from structured input — never from a title the model could write), while open-web search stays once-only. **Context cost is finally readable per server**: `contextUsage.breakdown.sections` attributes each app-owned static section individually — measured on a real session, the `mcp` aggregate of 3865 equals the six sections exactly, which shows the app-owned MCP surface is *not* where the context budget goes (same session: `other` 30,945, `system` 4,583). Fixes: the selected runtime now actually applies, notebook honours the persisted runtime on the execution path, app-local RPC is no longer routed through undici, the auto-audit is opt-out so it really runs, figure rules 6/7 carry their evidence on the agent path, a full-access Specialist no longer pins every skill against deletion, and a failed plan refresh no longer escapes as an unhandled rejection. Quality baseline: dual-config typecheck clean, lint clean, Nightly + Windows CI green. Version history with release code names: [CHANGELOG](CHANGELOG.md) · Website: [purescience.work](https://purescience.work)
