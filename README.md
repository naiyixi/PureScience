...
> 💡 **[PureScience v1.66.0 released](https://github.com/naiyixi/PureScience/releases/latest)** — The PDF reader used to see only tables; it now sees figures too, with the caption the document itself wrote. A figure is an image placement large enough to be a picture, and its caption is a label found near it (**Figure 3**, **Fig. 2**, **图 3**) — attached by proximity and never invented: a figure the document left unlabelled comes back with `captionSource` **none** and is counted in `withoutCaption`, decoration is counted in `skippedSmall` instead of being reported, and every figure states that its box is the axis-aligned box from the page transform, because rotation and skew are not modelled. The rules live in one shared module and reach the agent as `pdf_figures`, the host command `pdf:figures`, the local Web surface and the preload path, exactly as the table extraction does. Gene-set enrichment arrives with the rule that a service's word is not the result: a pure local engine (log-space hypergeometric tail, Lanczos lgamma, Benjamini–Hochberg with monotone enforcement, fold enrichment) **writes the background and the correction into the answer** — a default background says so, and an uncorrected result says its p-values must not be read against 0.05. The g:Profiler connector works offline when you pass the annotation, and otherwise **recomputes every returned term locally** from the four integers the service reports: rows that disagree land in `mismatches` with both numbers, and rows missing the sizes are marked `recomputed:false` rather than counted as agreeing. The tail probability is checked against exact rational arithmetic in BigInt. NCBI reference sequences now enter through three tools that refuse to be labels: the payload comes back with its **SHA-256 over the exact bytes received**, its byte size and its base count, a payload past 200k bases is **summarised as truncated** while length and digest still describe the whole thing, and an optional `assembly` assertion **fails closed** when the definition line does not name it — so GRCh37 can never be handed back as GRCh38 — with `ncbi_list_assemblies` there so the choice happens explicitly. Both shapes were verified against the live services (efetch returns the bare `GRCh38.p14` form; the Datasets report returns `total_count 2583`, `assembly_name GRCh38.p14`, `refseq_category 'reference genome'`). Quality baseline: dual typecheck, whole-tree lint, the full suite, and Nightly + Windows CI green, with **nine** real-window certification specs in the p0 lane.
