...
> 💡 **[PureScience v1.59.0 released](https://github.com/naiyixi/PureScience/releases/latest)** — The supervisor finally reaches the reviewer, and skill reuse becomes something you can look up. **Skill reuse**: the provider's own Skill activity carries no name (measured across 81 of them — no title, no input, no locations), so an app-owned PostToolUse hook captures the tool input and attributes it by app session id. The capture can never break a turn — a non-zero exit from a PostToolUse hook blocks the call, so the script swallows every error, always exits 0, and the command carries a platform-specific success guard; a test runs the generated script for real, with unreadable input, and expects exit 0. Reading is bounded (4 MB, tail kept, torn lines skipped) and only names that are actually present count, because a wrong name would show up as real reuse. The panel shows uses and failures per skill, says "no reuse recorded yet" instead of an empty table, notes that earlier sessions recorded no names, and now lists connector-provided skills that have no row of their own — their reuse used to be recorded and then invisible. **Supervision, three breaks fixed**: the ledger was dropped at the main-process boundary (the request was destructured field by field and `supervisor` was not in the list), only the auto entry point carried it (the manual "Request review" path assembled the same request without it, and a skipped attempt discarded the plan), and the audit line lived on the fix-loop re-review path, so the FIRST review — the common case — attached the ledger to the reviewer's instructions but left no trace at all. Measured live on the running app: a review request carrying a plan now logs `reviewer: supervisor ledger attached` with `repeated-tool-failure@turn:3` and an `activity:` handle. Fixes: the capture hook is registered as a Windows-sensitive source; a new application command has to land in all three registrations or the runtime inventory certificate refuses to start the app; and a renderer change needs the web bundle rebuilt. Quality baseline: dual-config typecheck clean, lint clean, full suite green (974 files / 13,463 tests), Nightly + Windows CI green. Version history with release code names: [CHANGELOG](CHANGELOG.md) · Website: [purescience.work](https://purescience.work)
