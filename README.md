...
> 💡 **[PureScience v1.13.0 released](https://github.com/naiyixi/PureScience/releases/latest)** — 独立开源的科研 AI 工作台：注解图片版补全（agent 生成图片框选区域注解，放大精调/重选/确认）；跨资源目录标签与收藏（技能/连接器/专才统一打标、星标收藏、标签过滤）；技能面板重设计（主 agent / 专才作用域过滤、批量启禁删）；会话式技能创建（agent 自然语言起草技能）；@path 本地文件夹授权（跨盘浏览、授权管理、撤销即失效）；run-marks 轮次导航轨与按轮次导出会话；消息中心接入事件源（回合完成/失败/等待授权，桌面与 Web 共享收件箱）；安全加固批次（渲染器权限默认拒绝、作业输出凭据脱敏、连接器超时快速失败、事件流有界接纳）；结构化澄清工作流（agent 多问题选择卡）；独立版本号体系（v1.0.0 起）；Serverless GPU 与模型端点（Modal 作业 + NVIDIA NIM 推理）、统一凭据管理（8 类科学服务加密存储 + 连接测试）、完整记忆体系、官方专才市场（签名验证 + jsDelivr 国内加速）、可追溯产物、MCP 连接瞬断自愈、多智能体编排、生物医学技能包、全界面中文适配。**更新日记（含版本代号）见 [CHANGELOG](CHANGELOG.md)**；官网：[purescience.work](https://purescience.work)
