...
> 💡 **[PureScience v1.70.0 released](https://github.com/naiyixi/PureScience/releases/latest)** — The interface now says what it means in all three states. Empty, loading and failed used to look alike across much of the app, so "not read yet" was reported as "nothing here": the home screen rendered its empty state during the one IPC round-trip before the project list arrived, telling users who have projects that they had none, and the running-jobs badge rendered nothing both when a session had no jobs and when nobody had read its feed — nothing in the renderer read that feed except one message scroller, so a session outside it never showed its running jobs at all. Eleven empty states now say why they are empty and offer the next step, twenty-seven loading and failure lines come from the dictionary, and the store layer no longer writes prose: a write failure crosses into React as a dictionary key while transport messages pass through untouched. The nine languages gain 128 keys covering the settings and workspace surfaces, the shared components, the figure digitisation panel and the omics preview — where the whole panel was Chinese-only: the calibration instructions that make a digitised curve auditable, and the warning that stops a downsampled preview from being delivered as a result, were unreadable in eight of nine languages. Sentences built in shared code that the main process also imports now receive the translator and fill their own placeholders, the picking session carries keys the renderer translates instead of its own sentences, and instruction text the agent reads stays English so the same manifest is acted on the same way in every language.
