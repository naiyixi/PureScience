...
> 💡 **[PureScience v1.73.0 released](https://github.com/naiyixi/PureScience/releases/latest)** — Two capabilities the agent could already reach and a reader could not are now on screen: a paper's contents tree and its per-page figure inventory are read through the same channels the agent's tools use, so what the panel shows is what the parser returned — including saying that a document declares no bookmarks rather than leaving an empty box — and a figure's declared structure can be reviewed against the findings the rules engine actually produced, with the panel stating plainly what its own checks do not cover. A recorded notebook cell can be re-run from the pane that lists it: the row carries the same command the agent uses, names the reason when it cannot run, and reports the interpreter's own status back. That work exposed a larger defect, found by driving the packaged application rather than reading the code: the window was never told a notebook existed — the announcement had a declaration, a subscription and a publisher, but no caller on any run path — so the whole notebook pane, and everything added to it, was unreachable. The caller is now there on every path a run can take, with a source-level assertion so the next path cannot drop it. Two guards close the class of defect behind four separate findings: a contract surface the desktop app installs must be reachable from the window or carry a recorded archival decision, and a modal surface must use the shared dialog shell. Each entry was verified in the running application, not only in tests.
