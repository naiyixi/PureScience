...
> 💡 **[PureScience v1.49.0 released](https://github.com/naiyixi/PureScience/releases/latest)** — dual-product convergence batch: **right-click any preview content** for copy-path / download / save-as-artifact (previously tab-only), and freshly refreshed **flagship model catalogs** (OpenAI GPT-6 Astra at 1.05M context, Anthropic Claude Fable 5.1 at 1M) alongside four audited-and-archived comparison items — no hollow parity, every surface either shipped or explicitly closed. Quality baseline: dual-config typecheck clean, Nightly + Windows CI green. Version history with release code names: [CHANGELOG](CHANGELOG.md) · Website: [purescience.work](https://purescience.work)
