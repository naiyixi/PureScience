...
> 💡 **[PureScience v1.62.0 released](https://github.com/naiyixi/PureScience/releases/latest)** — A conclusion you cannot check is not a conclusion. **A stalled artifact version now has a name.** Versions whose publication never resolved are labelled for what they are — `no-publication-intent` (the intent was never written) or `ambiguous-turn-message` (the intent is there, but that turn has more than one candidate message, and the proof refuses to guess) — and they keep their rows and their bytes: never deleted silently, never marked as published. All seven silent `continue`s in the reconciliation loop now carry a reason and a count, because “refused” and “had nothing to do” used to look identical in the log. Verified live, not with fixtures: on a real data root 26 stranded versions were named (`finalized 445 / unpublished 26`, pending zero, none deleted), and three of them, spanning two projects and three runs, were measured byte-for-byte on disk. The fix caught its own bug on the way — labelling hit an old CHECK constraint and failed the whole startup pass, because a migration rebuilt a table only when a constraint was *missing*, never when its value set had gone stale. **Re-running a recorded artifact now returns a real verdict.** It never ran in the isolated directory it was graded in — the verdict said “finished without producing the file”, mistaking a path mismatch for an absent file. A re-run now claims its own session in the recorded project, is graded in the directory it actually runs in, and is torn down afterwards: the same version that read `unverifiable` reads `reproduced`, byte for byte, and the recorded session’s files are untouched. **Still not applied, and named as such**: the environment lock — this reproduction holds under the default runtime, which is not the same claim as “the environment reproduced”. Quality baseline: dual typecheck, whole-tree lint, the full suite, and Nightly + Windows CI.
