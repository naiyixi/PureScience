...
> 💡 **[PureScience v1.64.0 released](https://github.com/naiyixi/PureScience/releases/latest)** — The lag has a number, and now it has a fix. **The session list stopped lifting 55 MB in the last release; it no longer re-parses the corpus to answer.** Measured on the real machine over the real corpus: the old read returned **55,371,532 bytes in 1541 ms** for 59 sessions, while the list tier now returns **569,858 bytes (0.54 MB)** — **68 ms on a cold first call**, and **p95 23 ms** across twenty reads — with the server's own counters reporting that **21 of 21 reads were served from the index and parsed zero documents**. It is answered from an on-disk index keyed by each document's own size and mtime: a match means the file is never opened, a mismatch re-parses exactly that one file and rewrites its entry, a session created or deleted outside the app is still seen immediately because the directory owns the file list, and deleting the whole index costs one 503 ms rebuild before the next read is index-only again. The index carries the list projection itself — a metadata slice holds no messages, so it could not report a count or a preview at all. Reconciliation needs whole documents, so it moved off the user's read to a single pass at startup. **And driving the shipped UI on the real machine found a defect that ate data**: pinning a session that had never been opened truncated its transcript on disk, 9 messages to 0 — the summary-only guard was keyed by object identity, and every store update replaces the object, so the first edit dropped the mark and the saver wrote the summary over the document. It is keyed by session id now, and the same UI path proves it: the un-opened session's file comes back byte-for-byte unchanged when pinned, and once its document has been read, the same action writes it with all 9 messages intact. Quality baseline: dual typecheck, whole-tree lint, the full suite, and Nightly + Windows CI.
