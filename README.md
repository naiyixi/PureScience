...
> 💡 **[PureScience v1.63.0 released](https://github.com/naiyixi/PureScience/releases/latest)** — Every claim, answerable. **The environment lock could never succeed, and now it does.** A re-run compared `sha256` of the whole environment manifest — a document that carries its own capture timestamps — so two runs in the *same* environment digested differently (measured: `b6d0cf04…` and `cc324682…`, minutes apart, one interpreter). A check that can never succeed is not a check. Storage and comparison now have two separate values: the document hash keeps addressing the archived manifest exactly as before, and an **environment fingerprint** (runtime, kernel kind, platform, architecture, sorted package list) is what two runs may legitimately share — the live verdict reads `environmentLock: applied`, and when either side has no fingerprint it still says `not-applied`, which means “could not compare”, never “the environments matched”. **A produced file that never became an artifact now says why, per file**, in nine languages: `no-publication-intent`, or `ambiguous-turn-message` — the turn cannot be attributed to a single reply, and the proof refuses to guess. Rows named before there was anywhere to record a reason are back-filled from the same evidence the naming uses: 26 of them, all `ambiguous-turn-message`, matching the measurement that all 26 runs left an intent marker behind. **Table extraction from PDFs is wired to the agent** — it had been fully built and had no production caller at all, which by this project's rule is not done. It now runs through `pdf_tables` end to end, and getting there exposed two real defects: the parser collected positioned items and then dropped them one line later (every PDF returned zero candidates), and centred cells split one column into ten. The merge guard is now the splitter's own threshold used in the other direction, and the live run returns `7 rows × 4 columns` with the header and its values in the same columns. **And the lag is a number now**: `sessions:load-all` takes a median 1.37 s and returns 55 MB for 59 sessions, while reading the same 63 MB from disk costs 10 ms — so the cost is parsing and serialising, not I/O. The optimisation itself is next, with its acceptance written down. Quality baseline: dual typecheck, whole-tree lint, the full suite, and Nightly + Windows CI.
