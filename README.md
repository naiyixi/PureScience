...
> 💡 **[PureScience v1.53.0 released](https://github.com/naiyixi/PureScience/releases/latest)** — Five scientific-capability upgrades with a machine-checkable acceptance benchmark: an **agent compute ladder** (list probed accelerators, propose a job for approval, label database predictions, or state "not computed" with the exact missing engine — never a qualitative substitute, never a fabricated number), **figure discipline grown to seven rules** (log-axis tick sanity plus .png/.py dual artifacts and a font floor, all advisory), a **live zoom readout** with per-file reset in image/TIFF previews, a **project task checkpoint** (`checkpoint_save`/`checkpoint_load` with a freshness verdict so a restart resumes verified work instead of redoing it), **batch PDF import** with progress and stop, and **PDF content fingerprints** recorded on attach. SciBench-Local v0 encodes the five real failure cases as nine deterministic rules over a session trace. Quality baseline: dual-config typecheck clean, Nightly + Windows CI green. Version history with release code names: [CHANGELOG](CHANGELOG.md) · Website: [purescience.work](https://purescience.work)
