...
> 💡 **[PureScience v1.72.0 released](https://github.com/naiyixi/PureScience/releases/latest)** — Five places where the software could already do something but the interface gave no way to ask for it are closed, and three defects that only appear when the app is actually driven were fixed. A PDF's table could be read by the agent and never shown to the reader; a figure could be picked with the mouse but the pick could not be undone a step at a time from the keyboard; and the actions a file has — digitize it, extract its tables, import its references, download it, stage it as an artifact — existed only in a right-click menu, so nothing on screen said a preview could do anything at all, which is why they are now buttons in the preview header, drawn from the same single list the menu renders so the two surfaces cannot drift apart. ⌘K opened a search that searched files and sessions and offered no commands, while the dialog called itself a command palette: it now answers the same queries the settings search does, so typing `mirror` reaches the network panel from either entry, and a new keyboard sheet lists only the chords the app really handles. Opening a session's verification checklist required a review to exist before the entry point to it did, so a session with none had no way in; the session card now opens the checklist and says what an empty one means. Two of the three defects surfaced by driving the app rather than reading it: a window-wide ⌘K listener inside the settings page was registered unconditionally, so it consumed the key before the palette could see it and the palette would not open from any other screen, and the review store returned a freshly built empty object on every read, so the checklist panel re-rendered itself the moment it mounted until React refused further updates. Each unit was verified in the running application, not only in tests.
