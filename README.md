...
> 💡 **[PureScience v1.67.1 released](https://github.com/naiyixi/PureScience/releases/latest)** — The notebook server is now named `purescience_notebook`, with no hyphen, because a framework embeds the server name in the tool it hands the model (`mcp__<server>__<tool>`) and each one escapes a hyphen its own way — one underscore here, two there. A hyphen therefore made the callable name depend on who rendered it, and a call the runtime did not recognise came back as `No such tool available` with the session unable to attribute it. The old spelling stays in an alias table, so a grant, a config or a reported tool name written before the rename still resolves to the same identity: nothing has to be re-granted, and the notebook grant keeps its language scope and its display label. Following the rename through every place that keys on the name also closed three gaps the tests caught: the Codex leaf-name alias no longer recognised `execute` as `notebook_execute`, the broker's notebook constant stopped matching its own underscore-normalised comparison, and the prose renderer rewrote a framework's escaped spelling but not the canonical one — which left guidance telling a model to call a server name that framework never registered. Recognisers now accept one or two separators, and prose naming a server is rewritten to the name the framework actually sees. Also in this patch: the storage timing assertion that failed twice on commits that never touched storage now gives its deliberate slow read a signal an order of magnitude above the filesystem noise it competes with, so a busy machine stops reading as misattribution while a genuinely misattributed read still fails. Quality baseline: dual typecheck, whole-tree lint, the full suite, and Nightly + Windows CI green.
