...
> 💡 **[PureScience v1.51.0 released](https://github.com/naiyixi/PureScience/releases/latest)** — Project **reference library**: import records by DOI/PMID/PMCID/arXiv ID with authoritative metadata plus a provenance snapshot (source + resolution timestamp) stored on every record, manual add as fallback, collection folders, duplicate protection on ingest, and one-click dedupe/merge across DOI/PMID/title-normalized groups. Every capability is exposed through the UI, as IPC, and as application commands. Quality baseline: dual-config typecheck clean, Nightly + Windows CI green. Version history with release code names: [CHANGELOG](CHANGELOG.md) · Website: [purescience.work](https://purescience.work)
