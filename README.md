...
> 💡 **[PureScience v1.68.1 released](https://github.com/naiyixi/PureScience/releases/latest)** — Update checks no longer wait on a mirror that has not been deployed: the publish target in the build config is untouched, and a failed check now falls back to GitHub Releases for one retry. The in-app update notes come from the release’s own CHANGELOG entry instead of the hard-coded `See the release notes on GitHub.` — the already-published v1.68.0 assets were refreshed in place (32 → 1,533 characters) with every platform’s url/size/sha256 left byte-for-byte identical. And one boundary fix that came out of a real machine: hunting for a project file, the agent ran `find / -iname "…"` across the whole machine — including **another installation’s** data tree — and reported what it read there as project data. Every absolute path a file tool or shell call spells out now passes a PreToolUse path guard: inside this install’s data root or config root it passes, outside it is refused with a reason that names where the project’s files actually are and asks the user to attach anything else, so the agent stops and asks rather than routing around. Credential reads stay refused, system prefixes (/usr, /bin, TLS material) still pass, and every decision is written to `hooks/path-guard-decisions.jsonl`. The first real run also caught a spelling defect: `/tmp` and `/private/tmp` named the same directory and got opposite answers, so paths are now resolved through symlinks before they are judged.
