...
> 💡 **[PureScience v1.64.1 released](https://github.com/naiyixi/PureScience/releases/latest)** — The first screen asks once instead of fifty-two times, and the artifact cards ask once per render tick instead of once per card. **Home used to send one Files read per active project** — on a real corpus that measured **52 reads in the startup window**, each queueing behind the same engine; one batched call now answers for every project's file-type chips, and the same instrument on the same drive reads **52 → 1** (list-files 52 → 0, list-kinds 0 → 1), with the chips rendered identically at **177 elements and 16 distinct kinds**. The engine canary — a trivial `SELECT 1` started alongside the waiting query — went from a **42 ms median to 3 ms**. Each transcript card then asked "is this file still there?" on its own, and because a card's path is a version locator every ask resolved and checksummed the whole file in main: **40 calls, 15 in flight at once, 22 ms median**. Cards now ask once per render tick through a batched channel that resolves the batch in a single query and stats each file without reading it: **40 calls became 0**, and **2 batched calls (15 + 5 paths) at 1 in flight and 2 ms median** — with the card previews themselves falling from 11 ms to 4 ms median once the queue stopped filling with probes. Storage no longer waits **13.7 s** for its first disk walk either: a segment instrument put **13,669 of 13,863 ms** in the usage read, so a cold read answers in **2 ms** carrying a pending flag and the panel says it is measuring until the walk lands (20 s later: **7.36 GB across 5 categories**). Two real defects surfaced on the way — the shared request limiter never released a slot when a task threw synchronously, so four of them starved every later read, and `StorageUsage` existed twice, letting a new field compile on one side only. One criterion is recorded as not reproducible here rather than claimed: the engine-queue canary's >100 ms high never appeared on a faithful copy of the real root (deepest reading 4 ms), so it stands as direction and magnitude. Quality baseline: dual typecheck, whole-tree lint, the full suite, and Nightly + Windows CI.
