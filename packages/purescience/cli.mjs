...
const usage = `Usage: purescience <command> [options]
...
Options:
  ...
  --app-path <path>      Installed PureScience executable
...
export const buildAppLaunchArgs = (appArgs, options, port) => [
  ...
  ...appArgs,
  // `--purescience-headless` instead of `--headless`: Chromium consumes `--headless` and renders
...
export const formatStartupFailure = (outcome, logTail, options) => {
  if (!options.noSandbox && sandboxFailurePattern.test(logTail)) {
    return [
      'PureScience could not start because Chromium sandboxing is unavailable on this host.',
      logTail,
      ...
      'For an explicit rootless fallback, run "purescience start --no-sandbox".',
  ...
  if (outcome.kind === 'error') return `Could not start PureScience: ${outcome.error.message}`

  const exitStatus = outcome.signal
    ? ` after receiving ${outcome.signal}`
    : ` with exit code ${outcome.code ?? 'unknown'}`
  return `PureScience exited before becoming healthy${exitStatus}.${logTail ? `\n\n${logTail}` : ''}`
}
...
const startCommand = async (options, deps = DEFAULT_DEPS) => {
  ...
  if (await healthCheck(existing, deps)) {
    ...
    deps.log(`PureScience is already running (PID ${existing.pid}).`)
    if (options.open) openBrowser(url)
    else deps.log('Run "purescience url" to print a browser login URL.')
    return
  }
  ...
  const childEnv = {
    ...process.env,
    ...(app.packaged ? {} : { PURESCIENCE_STORAGE_ROOT: configRoot }),
    PURESCIENCE_WEB_PORT: String(port)
  }
  ...
  if (startup.kind !== 'ready') {
    ...
    throw new Error(
      `PureScience did not become healthy within ${START_TIMEOUT_MS / 1000}s.${logTail ? `\n\n${logTail}` : ''}`
    )
  }
  const state = startup.state
  const url = await authenticatedUrl(state, deps)
  deps.log(`PureScience started (PID ${state.pid}).`)
  if (options.open) openBrowser(url)
  else deps.log('Run "purescience url" to print a browser login URL.')
}
...
export const stopCommand = async (options, deps = DEFAULT_DEPS) => {
  ...
  if (!state) {
    deps.log('PureScience is not running.')
    return
  }
  const token = await deps.readWebToken(state.configRoot)
  try {
  ...
  if (state.attached) {
    ...
    if (!stopped) {
      throw new Error(
        `Could not stop the PureScience web service (PID ${state.pid}); the app is still serving.`
      )
    }
    await deps.removeState(state.configRoot)
    deps.log('PureScience web service stopped; the app is still running.')
    return
  }
  ...
  if (!stopped) {
    throw new Error(`Could not stop PureScience (PID ${state.pid}); it is still running.`)
  }
  await deps.removeState(state.configRoot)
  deps.log('PureScience stopped.')
}
...
export const statusCommand = async (options, deps = DEFAULT_DEPS) => {
  ...
  if (options.json) {
    ...
    deps.log(`PureScience is running (PID ${state.pid}, port ${state.port}).`)
  } else {
    deps.log('PureScience is not running.')
  }
...
export const urlCommand = async (options, deps = DEFAULT_DEPS) => {
  const state = await findCurrentState(options, deps)
  if (!(await healthCheck(state, deps))) throw new Error('PureScience is not running.')
...
const TASK_DEPS = {
  connect: (options) => connectToPureScience(options),
...
export const rollbackCommand = async (options, dependencies = {}) => {
  ...
  deps.log(`Prepared an isolated PureScience ${manifest.targetVersion} rollback.`)
  ...
  deps.log(`You can now install and start PureScience ${manifest.targetVersion}.`)
}
...
const emitRunEvent = (event, options, deps) => {
  if (options.jsonl) {
    ...
    deps.warn(
      'Run is waiting for approval. Approve the request in PureScience Desktop or the Web UI.'
    )
  } else if (
