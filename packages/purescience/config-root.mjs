...
export const DEV_CONFIG_DIR = '.purescience-project'
export const PROD_CONFIG_DIR = '.purescience'
...
export const resolveConfigRoot = ({ packaged = false, override, env = process.env } = {}) => {
  const requested = override ?? env.PURESCIENCE_CONFIG_ROOT ?? env.PURESCIENCE_STORAGE_ROOT
  if (requested) {
...
export const candidateConfigRoots = ({ override, env = process.env } = {}) => {
  if (override ?? env.PURESCIENCE_CONFIG_ROOT ?? env.PURESCIENCE_STORAGE_ROOT) {
