...
const defaultInstalledCandidates = (env = process.env) => {
  if (process.platform === 'win32') {
    return [
      env.LOCALAPPDATA && join(env.LOCALAPPDATA, 'Programs', 'PureScience', 'PureScience.exe'),
      env.PROGRAMFILES && join(env.PROGRAMFILES, 'PureScience', 'PureScience.exe')
    ].filter(Boolean)
  }
  if (process.platform === 'darwin') {
    const app = 'PureScience.app/Contents/MacOS/PureScience'
    return [join('/Applications', app), env.HOME && join(env.HOME, 'Applications', app)].filter(
      Boolean
    )
  }
  return ['/usr/bin/purescience', '/usr/local/bin/purescience']
}
...
export const locateApp = async ({ appPath, env = process.env } = {}) => {
  const explicit = appPath ?? env.PURESCIENCE_APP_PATH
  if (explicit) {
    const command = resolve(explicit)
    if (!(await exists(command))) throw new Error(`PureScience executable not found: ${command}`)
  ...
  if (process.platform === 'linux') {
    const command = await executableOnPath('purescience', env)
