...
const temporaryRoot = async (): Promise<string> => {
  const root = await import('node:fs/promises').then(({ mkdtemp }) =>
    mkdtemp(join(tmpdir(), 'purescience-rollback-'))
  )
...
describe('rollback-to-0.7.3', () => {
  ...
  it('creates and activates an isolated 0.7.3 data root while preserving newer data', async () => {
    ...
    const configRoot = join(root, '.purescience')
    const dataRoot = join(root, 'PureScience')
    const rollbackDataRoot = join(root, 'PureScience-Rollback-0.7.3')
    ...
    const database = new DatabaseSync(join(configRoot, 'purescience.db'))
    database.exec(`
  ...
  it('removes incomplete copies and leaves the active Config Root unchanged on conversion failure', async () => {
    ...
    new DatabaseSync(join(configRoot, 'purescience.db')).close()
  ...
  it('revalidates prepared Version bytes before recovering a cutover', async () => {
    ...
    const manifest = {
      ...
      originalConfigRoot: configRoot,
      preservedConfigRoot,
      originalDataRoot: join(root, 'PureScience'),
      preservedDataRoot: join(root, 'PureScience'),
      rollbackDataRoot,
      sessionsConverted: 0
    }
    await mkdir(configRoot, { recursive: true })
    ...
    const database = new DatabaseSync(join(stagingConfigRoot, 'purescience.db'))
    database.exec(`
      CREATE TABLE UploadVersion (
        id TEXT PRIMARY KEY,
        state TEXT NOT NULL,
  ...
  it('rechecks that PureScience is closed immediately before cutover', async () => {
    const root = await temporaryRoot()
    const configRoot = join(root, '.purescience')
    const dataRoot = join(root, 'PureScience')
    const rollbackDataRoot = join(root, 'PureScience-Rollback-0.7.3')
    ...
    ).rejects.toThrow('PureScience is still running')
  ...
  it('rechecks offline state after validating an interrupted prepared copy', async () => {
    ...
    await writeFile(join(stagingConfigRoot, 'rollback-to-0.7.3.json'), JSON.stringify(manifest))
    await writeFile(join(rollbackDataRoot, 'rollback-to-0.7.3.json'), JSON.stringify(manifest))
    new DatabaseSync(join(stagingConfigRoot, 'purescience.db')).close()
    await writeFile(markerPath, JSON.stringify({ ...marker, phase: 'preparing' }))
    await writeFile(`${markerPath}.prepared`, JSON.stringify({ ...marker, phase: 'prepared' }))
  ...
  it('does not resume an interrupted cutover while PureScience is running', async () => {
    const root = await temporaryRoot()
    const configRoot = join(root, '.purescience')
    const rollbackDataRoot = join(root, 'PureScience-Rollback-0.7.3')
    const preservedConfigRoot = `${configRoot}.before-rollback-20260729T010203Z`
