...
const resolveVersion = (kind, candidate, versions) => {
  ...
  if (!version) {
    throw new Error(`Cannot materialize ${kind} Version ${versionId} for PureScience 0.7.3.`)
  }
  return version
}
...
const resolveRollbackMessages = (source) => {
  ...
  if (
    ...
    throw new Error('Session Conversation Graph cannot be projected for PureScience 0.7.3.')
  }
...
const assertDatabaseIntegrity = (database, label) => {
  ...
  if (rows.length !== 1 || String(rows[0]?.quick_check).toLowerCase() !== 'ok') {
    throw new Error(`${label} PureScience database failed SQLite quick_check.`)
  }
}
...
const resolveConfiguredDataRoot = async (configRoot) => {
  ...
  return hasLegacyData ? configRoot : join(homedir(), 'PureScience')
}
...
const assertOffline = async (configRoot) => {
  for (const suffix of ['-wal', '-shm']) {
    if (await exists(join(configRoot, `purescience.db${suffix}`))) {
      throw new Error(
        `PureScience may still be running (${`purescience.db${suffix}`} exists). Quit the app completely before rollback.`
      )
    }
  }
  try {
    ...
    if (Number.isInteger(state.pid) && state.pid > 0) {
      try {
        ...
        throw new Error('PureScience is still running. Quit it completely before rollback.')
      } catch (error) {
        if (error?.code === 'EPERM') {
          throw new Error('PureScience is still running. Quit it completely before rollback.')
        }
    ...
    try {
      ...
      const error = new Error(
        [
          ...
          `Restore one of them to ${configRoot} before starting PureScience.`
        ].join(' '),
...
const validatePreparedRollback = async (marker) => {
  ...
  validateCopiedDatabase(join(marker.stagingConfigRoot, 'purescience.db'))
  const indexes = readVersionIndexes(join(marker.stagingConfigRoot, 'purescience.db'))
...
export const runRollbackToV073 = async (options = {}) => {
  if (options.confirm !== true) {
    throw new Error(
      'Rollback requires explicit confirmation. Re-run with --yes after closing PureScience.'
    )
  }
  const now = options.now ?? Date.now
  const stamp = formatStamp(now())
  const configRoot = resolve(options.configRoot ?? join(homedir(), '.purescience'))
  ...
  if (!(await exists(join(configRoot, 'purescience.db')))) {
    throw new Error(`PureScience database does not exist: ${join(configRoot, 'purescience.db')}`)
  }
  ...
  await assertOffline(configRoot)
  const indexes = readVersionIndexes(join(configRoot, 'purescience.db'))
  ...
  try {
    ...
    validateCopiedDatabase(join(stagingConfigRoot, 'purescience.db'))
