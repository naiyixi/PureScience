...
import { connectToPureScience, PureScienceClient } from './index.mjs'
...
describe('PureScienceClient', () => {
  it('pins the SDK method inventory without exposing management capabilities', () => {
    expect(Object.getOwnPropertyNames(PureScienceClient.prototype).sort()).toEqual(
      [
        'constructor',
        'health',
        'listProjects',
  ...
  it('reads readiness, runtimes, a re-run verdict and connectors from the versioned API', async () => {
    ...
    await expect(
      client.replayVersion({
        projectId: 'project-1',
        appSessionId: 'session-1',
        artifactId: 'artifact-1',
        versionId: 'version-1'
      })
    ).resolves.toEqual({ url: 'http://127.0.0.1:44100/api/v1/artifacts/replay' })
    ...
    expect(fetch.mock.calls.map(([url]) => url)).toEqual([
      ...
      'http://127.0.0.1:44100/api/v1/artifacts/replay',
  ...
  it('discovers a daemon from its state and token files before returning a client', async () => {
    const configRoot = await mkdtemp(join(tmpdir(), 'purescience-sdk-'))
    ...
    const fetch = vi.fn().mockImplementation(async () => response(200, { appName: 'PureScience' }))

    const client = await connectToPureScience({ configRoot, fetch })

    await expect(client.health()).resolves.toEqual({ appName: 'PureScience' })
