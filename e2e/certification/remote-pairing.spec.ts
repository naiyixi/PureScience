...
test('re-enters with a trusted browser after the Electron app restarts', async ({ app }) => {
  ...
  const pairingCookie = cookieValue(pairing.setCookie, 'purescience_remote_pairing')
  ...
  const status = await remoteRequest('/__purescience_remote/pair/status', pairingCookie as string)
  expect(status.status).toBe(200)
  expect(JSON.parse(status.body)).toEqual({ status: 'approved' })
  const sessionCookie = cookieValue(status.setCookie, 'purescience_remote_session')
