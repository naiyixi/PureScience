...
// so there is ONE source of truth for how a language's interpreter is chosen and whether PureScience
...
//   `python -m venv --system-site-packages`). PureScience owns it, so writes are safe by design.
...
// How (and whether) PureScience may install packages into a runtime. `via` names the writer so a
...
export type RuntimeReadiness = {
  ...
  runnable: boolean
  // PureScience may install packages here (managed: yes; external: only when authorized).
...
// `installAuthorized` is the SEPARATE high-risk opt-in that lets PureScience write packages into an
