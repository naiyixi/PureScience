// Status of the `purescience` command-line launcher that the app can install into the user's PATH.
...
export type CliLauncherStatus = {
  ...
  target: string
  // Whether `target`'s directory is on the user's PATH, i.e. `purescience` is callable by name.
  onPath: boolean
