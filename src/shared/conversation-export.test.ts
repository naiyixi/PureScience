...
describe('conversation export projection', () => {
  ...
  it('replaces legacy truncated automatic titles with a complete prompt-derived title', () => {
    const session = createSession()
    const prompt =
      'Create a detailed reproducible research note comparing three purescience data management approaches. Include:\n- tables\n- code'
    ...
    expect(createConversationExportDocument(session, 1_710_000_003_000).title).toBe(
      'Create a detailed reproducible research note comparing three purescience data management approaches. Include: - tables - code'
    )
  })
  ...
  it('creates portable filenames and only truncates near filesystem byte limits', () => {
    ...
    expect(
      sanitizeExportFilename(
        'Create a detailed reproducible research note comparing three purescience data management approaches.'
      )
    ).toBe(
      'Create a detailed reproducible research note comparing three purescience data management approaches'
    )
