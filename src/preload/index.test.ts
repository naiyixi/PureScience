...
describe('preload bridge — public surface inventory', () => {
  it('pins every callable path exposed through window.api', () => {
    expect(collectFunctionPaths(api)).toEqual([
      ...
      'acp.getState',
      'acp.onElicitationRequest',
      'acp.onEvent',
      'acp.onPermissionRequest',
      'acp.onState',
      'acp.resetSessionContext',
      'acp.respondElicitation',
      'acp.respondPlan',
      ...
      'reviewer.evidence',
      'reviewer.getChecklist',
      'reviewer.getChunks',
      'reviewer.getForSession',
      'reviewer.mutateChecklist',
      ...
      'settings.deleteExternalComputeEndpoint',
      ...
      'settings.getEgress',
      'settings.getExternalComputeEndpoints',
      'settings.getMemory',
      ...
      'settings.getThirdPartyLicenses',
      ...
      'settings.setEgress',
      'settings.setExternalComputeEndpoint',
      'settings.setMemory',
      ...
      'settings.xaiOauthComplete',
      'settings.xaiOauthLogout',
      'settings.xaiOauthRefresh',
      'settings.xaiOauthStart',
      'settings.xaiOauthStatus',
...
describe('preload bridge — Connector configuration files', () => {
  it('forwards dropped file contents for main-process validation', async () => {
    const request = {
      ...
      contents: '{"schemaVersion":1,"kind":"purescience.connector"}'
    }
...
describe('preload bridge — core renderer contract catalog', () => {
  it('pins the exact 34-group, 204-contract T1d complement', () => {
    expect(coreContractGroups.map(({ capability }) => capability)).toEqual([
      ...
      'routine',
      'endpoint',
      // Saved search filter sets (v1.67 unit): beside the bookmarks, which it sits next to in the host
      // group order as well.
      'searchPins',
      'bookmark',
      'annotation',
    ...
    expect(coreContracts).toHaveLength(204)
    expect({
    ...
    }).toEqual({ requests: 166, events: 27, sends: 10, surfaceNative: 1 })
  })
  ...
  it('routes every core request method through its cataloged Electron channel', async () => {
    ...
    expect(requestContracts).toHaveLength(166)
  ...
  it('preserves Session, Storage, and native-upload request argument shapes', async () => {
    ...
    await api.storage.validateDataRoot('/data/purescience')
    await api.storage.setDataRootAndRelaunch('/data/purescience', true)
...
const cases: ForwardingCase[] = [
  ...
  {
    name: 'settings.setVisionModel → settings:set-vision-model',
    invoke: (a) => a.settings.setVisionModel({ configuration: undefined }),
    channel: 'settings:set-vision-model',
