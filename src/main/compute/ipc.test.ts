...
// directory (PURESCIENCE_STORAGE_ROOT is preferred when set).
// ---------------------------------------------------------------------------
...
vi.mock('electron', () => ({
  ...
  app: {
    ...
    getPath: (key: string) => {
      ...
      return process.env.PURESCIENCE_STORAGE_ROOT ?? '/tmp'
    }
  }
}))
...
describe('installComputeIpcHandlers', () => {
  ...
  beforeEach(async () => {
    ...
    process.env.PURESCIENCE_STORAGE_ROOT = storageRoot
  })

  afterEach(async () => {
    delete process.env.PURESCIENCE_STORAGE_ROOT
    await rm(storageRoot, { recursive: true, force: true })
  })
