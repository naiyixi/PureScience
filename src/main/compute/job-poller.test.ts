  ...
  it('polls >8 jobs for one provider in size-bounded sub-batches, updating every job', async () => {
    ...
    const jobs = Array.from({ length: 10 }, (_, i) =>
      makeJob({
        ...
        remote_handle: JSON.stringify({
          pid: 1000 + i,
          exit_code_path: `~/.purescience/jobs/job-${i}/exit_code`,
          stdout_path: `~/.purescience/jobs/job-${i}/stdout`,
          stderr_path: `~/.purescience/jobs/job-${i}/stderr`,
          workdir: `~/.purescience/jobs/job-${i}`
        })
      })
    )
