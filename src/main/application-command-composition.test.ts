...
describe('application command composition', () => {
  it('certifies the complete group inventory behind the local Web view', () => {
    ...
    expect(composition.localWeb.commandNames()).toHaveLength(316)
  })
  ...
  it('partitions remote Web dispatch from fail-closed pre-dispatch rejections', async () => {
    ...
    expect(composition.remoteWeb.rejectedCommandNames()).toHaveLength(105)
    await expect(
  ...
  it('exposes only the eleven Task commands and no transport-wide capability', async () => {
    const composition = createApplicationCommandComposition(dependencies())
  ...
  it('installs every registrar family in a fixed order and stops at a partial failure', () => {
    ...
    composition.dispose()
    expect(normalEvents.slice(12)).toEqual([
      'uninstall:11',
      'uninstall:10',
      'uninstall:9',
      'uninstall:8',
    ...
    expect(disposeFailureEvents.slice(12)).toEqual([
      'uninstall:11',
      'uninstall:10',
      'uninstall:9',
      'uninstall:8',
    ...
    ).toEqual(['complete failed:11', 'router dispose failed'])
    expect(slotFailureEvents.slice(12)).toEqual([
      'uninstall:10',
      'uninstall:9',
      'uninstall:8',
      'uninstall:7',
