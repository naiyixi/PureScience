...
const displayName = 'PureScience'
const appId = 'com.zerolink.purescience'

describe('app display branding', () => {
  it('uses the PureScience display name in shell and workspace surfaces', () => {
  ...
  it('uses the Zerolink app identifier for packaged and window integration metadata', () => {
    const mainSource = readFileSync(resolve(projectRoot, 'src/main/index.ts'), 'utf8')
    const builderSource = readFileSync(resolve(projectRoot, 'electron-builder.yml'), 'utf8')
