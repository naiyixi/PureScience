...
describe('routeSecondInstance', () => {
  it('starts the web service (attached) with the requested port for a --serve=PORT launch', () => {
    const deps = makeDeps()
    routeSecondInstance(['/app', '.', '--purescience-headless', '--serve=52020'], deps)

    expect(deps.ensureWebService).toHaveBeenCalledWith(52020, { attached: true })
    expect(deps.showMainWindow).not.toHaveBeenCalled()
  })

  it('starts the web service on the default port for bare --serve / --purescience-headless', () => {
    ...
    routeSecondInstance(['/app', '.', '--purescience-headless'], headless)
  ...
  it('decides purely on argv, ignoring any PURESCIENCE_WEB_PORT in the primary env', () => {
    const previous = process.env.PURESCIENCE_WEB_PORT
    process.env.PURESCIENCE_WEB_PORT = '55555'
    try {
      const deps = makeDeps()
      routeSecondInstance(['/app', '.'], deps)
      ...
      if (previous === undefined) delete process.env.PURESCIENCE_WEB_PORT
      else process.env.PURESCIENCE_WEB_PORT = previous
    }
  })
