...
describe('Responses-compatible bridge conversion', () => {
  const legacyReviewerMarker = '<purescience_reviewer_session>'
  ...
  it('restores a namespaced MCP call when its Chat id, name, and arguments are fragmented', async () => {
    ...
    try {
      ...
      expect(output).toContain('"namespace":"mcp__purescience_notebook"')
      expect(output).toContain('"name":"notebook_execute"')
      expect(output).toContain('"call_id":"call-notebook-1"')
      expect(output).not.toContain('"name":"mcp__purescience_notebook__notebook_execute"')
    } finally {
      await bridge.close()
    }
  })
  ...
  it('carries DeepSeek reviewer tool calls only for a trusted registered session key', async () => {
    ...
    const bridge = new ResponsesBridge(
      {
        ...
        reviewerScope: {
          namespacedTools: [
            {
              namespace: 'mcp__purescience_reviewer',
              name: 'read_turn',
              parameters: { type: 'object' }
            },
            {
    ...
    try {
      ...
      expect(toolNames[0]).toEqual(['exec_command', 'mcp__purescience_notebook__notebook_execute'])
      ...
      expect(reviewerOutput).toContain('"namespace":"mcp__purescience_reviewer"')
