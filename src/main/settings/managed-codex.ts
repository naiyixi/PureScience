...
const CODEX_ACP_SKILL_INPUT_REPLACEMENT = [
  ...
  '    switch (block.type) {',
  '      case "text": {',
  '        const requestedSkills = Array.isArray(block._meta?.["purescience/skill-inputs"])',
  '          ? block._meta["purescience/skill-inputs"]',
  '          : [];',
...
export const verifyManagedCodexPair: PairVerifier = async (adapterPath, codexPath, codexHome) => {
  ...
  const initialize = JSON.stringify({
    ...
    params: {
      ...
      clientInfo: { name: 'purescience-installer', version: '0.0.0' }
    }
  })
