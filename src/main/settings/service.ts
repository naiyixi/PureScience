...
class SettingsService {
  ...
  // the refreshed enablement. This is the separate opt-in that lets PureScience write packages into an
  ...
  // Resolves the persisted Vision model into an explicit backend target for the image relay, or
  // undefined when the relay is disabled. Throws when the persisted target is no longer usable.
  async getVisionModelTarget(): Promise<ExplicitAgentBackendTarget | undefined> {
  ...
  // a runtime installed outside PureScience between launches is picked up and onboarding can be
