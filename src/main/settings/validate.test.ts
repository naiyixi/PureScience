  ...
  it('builds the streaming function-tool probe required by the Codex bridge', () => {
    ...
    expect(JSON.parse(request.body)).toMatchObject({
      ...
      tools: [
        {
          type: 'function',
          function: { name: 'purescience_bridge_probe' }
        }
      ],
      tool_choice: 'auto'
    })
...
describe('validate: bridge contract', () => {
  it('accepts a complete streaming function tool call with fragmented fields', () => {
    expect(
      hasBridgeProbeToolCall(
        [
          'data: {"choices":[{"delta":{"tool_calls":[{"index":0,"id":"call_","function":{"name":"purescience_","arguments":"{"}}]}}]}',
    ...
    expect(
      hasBridgeProbeToolCall(
        '{"choices":[{"message":{"tool_calls":[{"id":"call_1","function":{"name":"purescience_bridge_probe","arguments":"{}"}}]}}]}'
      )
    ).toBe(false)
