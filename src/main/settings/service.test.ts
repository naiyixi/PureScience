...
const validBridgeToolCallResponse = (): Response =>
  new Response(
    'data: {"choices":[{"delta":{"tool_calls":[{"index":0,"id":"call_1","function":{"name":"purescience_bridge_probe","arguments":"{}"}}]},"finish_reason":"tool_calls"}]}\n\ndata: [DONE]\n\n',
    { status: 200, headers: { 'content-type': 'text/event-stream' } }
  )

const validNativeCompatibilityToolCallResponse = (): Response =>
  new Response(
    'data: {"type":"response.output_item.added","item":{"type":"function_call","id":"fc_1","call_id":"call_1","name":"purescience__bridge_probe","arguments":"{}"}}\n\ndata: {"type":"response.completed","response":{"output":[{"type":"function_call","id":"fc_1","call_id":"call_1","name":"purescience__bridge_probe","arguments":"{}"}]}}\n\n',
    { status: 200, headers: { 'content-type': 'text/event-stream' } }
  )
...
beforeEach(async () => {
  storageRoot = await mkdtemp(join(tmpdir(), 'purescience-settings-service-'))
...
describe('SettingsService: providers', () => {
  ...
  it('deleting a Codex subscription removes only its app-owned authentication and route', async () => {
    ...
    await expect(
      readFile(join(storageRoot, 'codex-subscription', 'config.toml'), 'utf8')
    ).resolves.not.toContain('PureScience:')
  ...
  it('reports imported login guidance when imported subscription auth is unavailable', async () => {
    ...
    await expect(
      ...
      category: 'auth',
      message:
        'No existing Codex login was found. Run `codex login` or use the isolated PureScience login.'
    })
  ...
  it('rejects isolated logout for imported authentication without deleting its copy', async () => {
    ...
    expect(result).toEqual({
      ok: false,
      category: 'unknown',
      message: 'No isolated PureScience Codex login is configured.'
    })
  ...
  it('persists a custom context window and carries it into the OpenCode model metadata', async () => {
    vi.stubEnv('PURESCIENCE_AGENT_FRAMEWORK', 'opencode')
  ...
  it('keeps OpenCode connector details in on-demand skills instead of baseline context', async () => {
    ...
    const backend = await resolveActiveBackend(service, {
      systemPromptAppends: ['Stable PureScience app guidance.']
    })

    expect(backend.persistentSystemPrompt).toContain('Stable PureScience app guidance.')
    const appInstructions = await readFile(
      join(storageRoot, 'opencode', 'config', 'opencode', 'instructions', 'purescience.md'),
      'utf8'
    )
    expect(appInstructions).toBe('Stable PureScience app guidance.')
...
describe('SettingsService: preflight & spawn config', () => {
  ...
  it('resolves a forced Codex backend only for a Responses provider', async () => {
    ...
    const backend = await resolveActiveBackend(service, {
      systemPromptAppends: ['Stable PureScience developer guidance.']
    })
    ...
    expect(developerInstructions).toContain('Stable PureScience developer guidance.')
  ...
  it('fails closed instead of spawning a detected global Codex ACP adapter', async () => {
    ...
    await expect(resolveActiveBackend(service)).rejects.toThrow(
      'PureScience Codex ACP adapter not found. Install Codex in settings.'
    )
  })
  ...
  it('resolves a Chat Completions provider through the Codex Responses bridge', async () => {
      ...
      tools: expect.arrayContaining([
        expect.objectContaining({
          ...
          function: expect.objectContaining({
            name: 'mcp__purescience_notebook__notebook_execute',
        ...
        expect.objectContaining({
          type: 'function',
          function: expect.objectContaining({
            name: 'mcp__purescience_artifacts__write_artifact_file'
          })
        }),
        expect.objectContaining({
          type: 'function',
          function: expect.objectContaining({
            name: 'mcp__purescience_skills__request_skill_import'
          })
        })
      ])
    })
    ...
    expect(disabledToolNames).toContain('mcp__purescience_notebook__notebook_execute')
    expect(disabledToolNames).not.toContain('mcp__purescience_skills__request_skill_import')
  ...
  it('routes a native-Responses vendor through the protocol-preserving compatibility proxy', async () => {
    ...
    expect(codexConfig.model_providers['purescience']).toMatchObject({
      base_url: backend.providerConfiguration?.baseUrl,
      wire_api: 'responses'
    })
    expect(codexConfig.model_providers['purescience']).not.toHaveProperty('requires_openai_auth')
...
describe('SettingsService: official vendors', () => {
  ...
  it('probes DeepSeek flash through the native Responses route under Codex', async () => {
    ...
    expect(JSON.parse(String(fetchMock.mock.calls[0][1]?.body))).toMatchObject({
      model: 'deepseek-v4-flash',
      stream: true,
      tools: [{ type: 'function', name: 'purescience__bridge_probe' }]
    })
  })
...
describe('SettingsService: reasoning effort', () => {
  ...
  it('does not guess an effort profile for an unpinned Claude subscription model', async () => {
    vi.stubEnv('PURESCIENCE_AGENT_FRAMEWORK', 'claude-code')
    const service = createService()
    await repository.setClaudeInfo({ resolvedPath: execPath, version: '2.1.0' })
...
describe('SettingsService: claude-shared login orchestration', () => {
  ...
  it('logoutClaudeShared disconnects locally without logging out the global CLI profile', async () => {
    ...
    await expect(
      ...
      ok: false,
      category: 'auth',
      message: expect.stringContaining('disconnected from PureScience')
    })
    await expect(resolveActiveBackend(service)).rejects.toThrow(/disconnected from PureScience/)
