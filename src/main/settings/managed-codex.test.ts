...
describe('patchCodexAcpTurnUsageSource', () => {
  ...
  it('accumulates every model request in one Codex prompt without double-counting updates', () => {
    ...
    expect(response._meta?.['purescience/turn-usage']).toEqual(usage(45, 31, 8, 6, 0))
    expect(response._meta?.['purescience/model-turn-count']).toBe(2)
  ...
  it('uses the first request snapshot when a resumed session has no cumulative baseline', () => {
    ...
    expect(response._meta?.['purescience/turn-usage']).toEqual(usage(18, 12, 3, 3, 0))
  })
  ...
  it('normalizes omitted cached-input counters without double-counting repeated updates', () => {
    ...
    expect(adapter.finishPrompt()._meta?.['purescience/turn-usage']).toEqual(usage(45, 31, 0, 6, 0))
  })
          ...
          buildPromptItems([
            {
              ...
              text: 'Keep this text',
              _meta: {
                'purescience/skill-inputs': [{ name: 'escape', path: skillPath }]
              }
            }
          ])
