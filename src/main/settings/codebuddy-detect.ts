...
export type CodeBuddyDetectResult = {
  resolvedPath: string
  version?: string
}

// Detects a runnable CodeBuddy executable across the locations a GUI app might miss because its PATH
...
// platform-specific rules unit-testable, mirroring opencode-detect.ts.
export type CodeBuddyDetectDeps = {
...
// Candidate binary filenames for CodeBuddy, in probe order. Windows resolves a bare command through
// PATHEXT, so an npm-installed `codebuddy.cmd`, a native `codebuddy.exe`, or a `.bat` shim must each
// be tried explicitly; the extensionless name is kept last as a catch-all. Unix has only `codebuddy`.
const codeBuddyBinaryNames = (platform: NodeJS.Platform): string[] =>
  platform === 'win32'
    ? ['codebuddy.cmd', 'codebuddy.exe', 'codebuddy.bat', 'codebuddy']
    : ['codebuddy']

// Common non-PATH install locations by platform. Windows: npm's global bin (%APPDATA%\npm). Unix:
// Homebrew + /usr/local. `~/.local/bin` and `~/.codebuddy/bin` (install-script targets) are added
...
// Builds the ordered list of directories to search for the codebuddy binary.
const collectCandidateDirs = async (deps: CodeBuddyDetectDeps): Promise<string[]> => {
  ...
  return Array.from(
    new Set([
      ...
      p.join(deps.homePath, '.codebuddy', 'bin'),
      ...(deps.extraDirs ?? []),
...
// succeeds. Returns undefined when codebuddy is not installed anywhere we look.
const detectCodeBuddy = async (
  ...
  const binaryNames = codeBuddyBinaryNames(deps.platform)
  ...
  for (const dir of candidateDirs) {
    for (const name of binaryNames) {
      ...
      // A path that exists but cannot report a version is not a usable codebuddy; keep searching.
...
// Parses the first version-looking token out of `codebuddy --version` output (it prints just "2.138.0").
...
const createDefaultDetectDeps = (): CodeBuddyDetectDeps => {
  ...
  return {
    ...
    getVersion: runCodeBuddyVersion(platform),
