...
const createStorageRoot = async (): Promise<string> => {
  storageRoot = await mkdtemp(join(tmpdir(), 'purescience-settings-'))
  return storageRoot
}
...
describe('settings repository', () => {
  ...
  it('migrates two legacy Codex subscription cards into one active-mode provider', () => {
    const settings = sanitizeSettings({
      ...
      providers: [
        ...
        { id: 'builtin-codex-isolated', type: 'codex-isolated', name: 'PureScience Codex login' }
      ]
    })
  ...
  it('sanitizeSettings drops a relative dataRoot and keeps only an absolute, normalized one', () => {
    ...
    expect(sanitizeSettings({ dataRoot: './PureScience' }).dataRoot).toBeUndefined()
