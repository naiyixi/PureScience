...
export class AgentRuntimeManager {
  ...
  async resolveCodexExecutable(
    ...
    if (!(await this.pathExists(adapterPath))) {
      throw new Error('PureScience Codex ACP adapter not found. Install Codex in settings.')
    }
  ...
  private async resolveCodexRuntime(settings: StoredSettings): Promise<ClaudeDetectResult> {
    ...
    if (components.nativeCliFound && !components.adapterFound) {
      diagnostic = `Native Codex ${components.nativeCliVersion} is installed at ${components.nativeCliPath}, but the Codex ACP adapter required by PureScience is missing.`
