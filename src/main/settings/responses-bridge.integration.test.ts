...
it.runIf(runLiveContract)(
  ...
  async () => {
    const tempRoot = await mkdtemp(join(tmpdir(), 'purescience-codex-native-responses-'))
    ...
    try {
      ...
      const result = await acp
        .client({ name: 'purescience-native-responses-contract' })
        ...
        .connectWith(stream, async (ctx) => {
          await ctx.request(acp.methods.agent.initialize, {
            ...
            clientInfo: { name: 'purescience-native-responses-contract', version: '1.0.0' },
...
it.runIf(runLiveContract)(
  ...
  async () => {
    const tempRoot = await mkdtemp(join(tmpdir(), 'purescience-codex-mcp-bridge-'))
    ...
    try {
      ...
      const result = await acp
        .client({ name: 'purescience-mcp-bridge-contract' })
        .onRequest(acp.methods.client.session.requestPermission, (ctx) => ({
          outcome: {
            outcome: 'selected',
            optionId:
        ...
        .connectWith(stream, async (ctx) => {
          await ctx.request(acp.methods.agent.initialize, {
            protocolVersion: acp.PROTOCOL_VERSION,
            clientInfo: { name: 'purescience-mcp-bridge-contract', version: '1.0.0' },
            clientCapabilities: { fs: { readTextFile: true, writeTextFile: true } }
          })
...
it.runIf(runLiveContract)(
  ...
  async () => {
    const tempRoot = await mkdtemp(join(tmpdir(), 'purescience-codex-reviewer-bridge-'))
    const codexHome = join(tempRoot, 'codex-home')
    const workspace = join(tempRoot, 'workspace')
    await Promise.all([mkdir(codexHome), mkdir(workspace)])
                  ...
                  tool_calls: [
                    {
                      ...
                      type: 'function',
                      function: {
                        name: 'mcp__purescience_reviewer__read_turn',
                        arguments: '{}'
                      }
                    }
                  ]
                  ...
                  tool_calls: [
                    {
                      ...
                      type: 'function',
                      function: {
                        name: 'mcp__purescience_reviewer__submit_findings',
                        arguments: '{"checks":[]}'
                      }
                    }
                  ]
    ...
    try {
      ...
      const result = await acp
        .client({ name: 'purescience-reviewer-bridge-contract' })
        .onRequest(acp.methods.client.session.requestPermission, (ctx) => {
        ...
        .connectWith(stream, async (ctx) => {
          await ctx.request(acp.methods.agent.initialize, {
            protocolVersion: acp.PROTOCOL_VERSION,
            clientInfo: { name: 'purescience-reviewer-bridge-contract', version: '1.0.0' },
            clientCapabilities: { fs: { readTextFile: true, writeTextFile: true } }
          })
          await ctx.request(acp.methods.agent.providers.set, {
            providerId: 'custom-gateway',
