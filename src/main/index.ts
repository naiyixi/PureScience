...
// 提高主进程 V8 堆上限, 防止密集科学计算任务 (REPL/对接分析) OOM 导致会话中断。
// 必须在 app ready 之前设置; Electron 主进程不认 NODE_OPTIONS 的 max-old-space-size,
// 只能通过 js-flags 传递。注意: MCP server 子进程以纯 node 模式运行 (无 electron 模块),
// 必须 try/catch 保护, 否则 artifacts/notebook MCP 启动即崩溃。
try {
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  const { app: electronApp } = require('electron')
  // 8GB 机器: 8192 经 Electron 对齐后实际堆 ~4GB (此前 16384 会撑爆 8GB 物理内存导致 SIGTRAP 崩溃)
  electronApp?.commandLine?.appendSwitch?.('js-flags', '--max-old-space-size=8192')
} catch {
  // MCP server 子进程 (纯 node) 无 electron 模块, 跳过
}
...
const APP_NAME = 'PureScience'
const APP_USER_MODEL_ID = 'com.zerolink.purescience'
...
const shouldRunHostQueryMcpServer = process.argv.includes(HOST_QUERY_MCP_SERVER_ARG)
...
} else if (shouldRunContextSummaryMcpServer) {
  void import('./settings/context-summary-mcp-server')
    .then(({ runContextSummaryMcpServer }) => runContextSummaryMcpServer())
    .catch((error: unknown) => {
      console.error(error)
      process.exitCode = 1
    })
} else if (shouldRunRoutineMcpServer) {
  void import('./settings/routine-mcp-server')
    .then(({ runRoutineMcpServer }) => runRoutineMcpServer())
    .catch((error: unknown) => {
      console.error(error)
      process.exitCode = 1
    })
...
} else if (shouldRunHostQueryMcpServer) {
  void import('./settings/host-query-mcp-server')
    .then(({ runHostQueryMcpServer }) => runHostQueryMcpServer())
    .catch((error: unknown) => {
      console.error(error)
      process.exitCode = 1
    })
...
async function startElectronApp(mainEntryPath: string): Promise<void> {
  ...
  await orchestrateAppStartup({
    ...
    installAppLifecycle: (ctx) => {
      ...
      // `purescience start` forwards --serve/--purescience-headless → start the web service on demand
