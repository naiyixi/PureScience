...
const resource = {
  id: 'resource-1',
  url: 'purescience-preview://resource-1/report.docx',
  size: 1024,
...
const createDependencies = (
  ...
  createRuntimeUrl: (sessionId) =>
    `purescience-office-preview://runtime/office-preview.html?sessionId=${sessionId}`,
...
describe('OfficePreviewSupervisor OOPIF sessions', () => {
  it('opens a resource-backed runtime URL without allocating a native child view', async () => {
    ...
    await expect(supervisor.open(7, request)).resolves.toEqual({
      kind: 'started',
      sessionId: 'session-1',
      runtimeUrl: 'purescience-office-preview://runtime/office-preview.html?sessionId=session-1',
      size: 1024,
