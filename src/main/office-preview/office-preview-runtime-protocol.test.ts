...
describe('Office preview runtime protocol', () => {
  it('serves the isolated runtime from its own site and proxies development assets', async () => {
    ...
    const response = await handler(
      new Request('purescience-office-preview://runtime/src/office-preview/main.ts')
    )

    const runtimeUrl = new URL(runtimeProtocol.createOfficePreviewRuntimeUrl('session / 1'))
    expect(runtimeUrl.protocol).toBe('purescience-office-preview:')
  ...
  it('preserves Vite query parameters when proxying development modules', async () => {
    ...
    const request = new Request(
      'purescience-office-preview://runtime/@fs/sheet.worker.js?worker&url'
    )
  ...
  it('rejects requests outside the dedicated runtime host', async () => {
    ...
    const response = await handler(
      new Request('purescience-office-preview://untrusted/office-preview.html')
    )
  ...
  it('maps packaged assets inside the renderer bundle and applies the runtime CSP', async () => {
    ...
    const request = new Request(
      'purescience-office-preview://runtime/office-preview.html?sessionId=session-1'
    )
    ...
    expect(response.headers.get('content-security-policy')).toContain(
      'connect-src purescience-preview:'
    )
  ...
  it('allows only the local Vite bootstrap requirements in development', async () => {
    ...
    const response = await handler(
      new Request('purescience-office-preview://runtime/office-preview.html?sessionId=session-1')
    )
  ...
  it('rejects encoded paths that escape the packaged renderer bundle', async () => {
    ...
    const response = await handler(
      new Request('purescience-office-preview://runtime/%2e%2e%2fsecrets.txt')
    )
  ...
  it('preserves HEAD semantics without buffering an source body', async () => {
    ...
    const response = await handler(
      new Request('purescience-office-preview://runtime/assets/runtime.js', { method: 'HEAD' })
    )
  ...
  it('buffers trusted runtime assets and replaces stale transfer headers', async () => {
    ...
    const response = await handler(
      new Request('purescience-office-preview://runtime/assets/runtime.js')
    )
