...
describe('codexFramework', () => {
      ...
      models: [
        {
          ...
          base_instructions: expect.stringContaining(
            'inside PureScience through the Agent Client Protocol'
          ),
  ...
  it('isolates the native Windows home used for user-installed Skills', () => {
    ...
    const config = framework.prepareModelConfig(
      { type: 'codex-isolated', apiEndpoints: ['responses'] },
      { storageRoot: 'C:\\PureScience', executablePath: 'C:\\runtime\\codex-acp.exe' }
    )

    expect(config.env).toMatchObject({
      HOME: join('C:\\PureScience', 'codex-subscription'),
      USERPROFILE: join('C:\\PureScience', 'codex-subscription'),
      CODEX_HOME: join('C:\\PureScience', 'codex-subscription')
    })
  })
  ...
  it('does not add a custom model_provider for an isolated Codex subscription', () => {
    // The ChatGPT subscription is codex-acp's default provider; layering an purescience custom
  ...
  it('delivers PureScience session guidance as persistent developer instructions', () => {
    const framework = createCodexFramework()
  ...
  it('runs an app-managed JavaScript adapter with Electron as Node', () => {
    ...
    const framework = createCodexFramework({
      execPath: '/Applications/PureScience/Electron',
      platform: 'darwin',
      spawnProcess
    })
    ...
    expect(spawnProcess).toHaveBeenCalledWith(
      '/Applications/PureScience/Electron',
