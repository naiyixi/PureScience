...
// The bridge path layers the purescience custom provider on top of this; the codex-isolated path
...
export const createCodexFramework = ({
  ...
  prepareModelConfig(provider, ctx: ModelConfigContext): AgentModelConfig {
    ...
    if (isCodexSubscriptionProvider(provider.type)) {
      // Every PureScience subscription session uses the same app-owned home. `codex-shared` is
