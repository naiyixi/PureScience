...
describe('opencodeFramework.prepareModelConfig', () => {
  ...
  it('writes stable app guidance to native instructions instead of a user prompt', () => {
    const config = opencodeFramework.prepareModelConfig(
      { type: 'custom', baseUrl: 'https://gw/v1', model: 'm', key: 'k' },
      {
        ...
        executablePath: '/bin/opencode',
        systemPromptAppends: [
          'Use `notebook_execute` from `purescience-notebook`.',
    ...
    const instructionsFile = config.configFiles?.find((file) =>
      file.path.endsWith('purescience.md')
    )
    expect(instructionsFile?.content).toBe(
      'Use `purescience_notebook_notebook_execute` from `purescience_notebook`.\n\nThen call `purescience_artifacts_write_artifact_file`.'
    )
    const opencodeJson = config.configFiles?.find((file) => file.path.endsWith('opencode.json'))
  ...
  it('registers immutable provider/model routes with separate local credentials', () => {
    ...
    expect(config.sessionModel).toBe('purescience-a/model-a')
    expect(fileConfig.model).toBe('purescience-a/model-a')
    expect(Object.keys(fileConfig.provider)).toEqual(['purescience-a', 'purescience-b'])
    expect(fileConfig.provider['purescience-a'].options).toEqual({
      baseURL: 'http://127.0.0.1:41001/v1',
      apiKey: '{env:OPENCODE_APP_API_KEY_PURESCIENCE_A}'
    })
    expect(fileConfig.provider['purescience-b'].options).toEqual({
      baseURL: 'http://127.0.0.1:41002/v1',
      apiKey: '{env:OPENCODE_APP_API_KEY_PURESCIENCE_B}'
    })
    expect(fileConfig.provider['purescience-a'].models).toEqual({ 'model-a': {} })
    expect(fileConfig.provider['purescience-b'].models).toEqual({ 'model-b': {} })
    expect(pinnedConfig.provider).toEqual(fileConfig.provider)
    expect(config.env?.OPENCODE_APP_API_KEY_PURESCIENCE_A).toBe('local-token-a')
    expect(config.env?.OPENCODE_APP_API_KEY_PURESCIENCE_B).toBe('local-token-b')
  })
})
