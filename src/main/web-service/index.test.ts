...
describe('parseWebModeOptions', () => {
  ...
  it('supports serve, explicit ports, environment ports, and headless mode', () => {
    ...
    expect(parseWebModeOptions(['electron'], { PURESCIENCE_WEB_PORT: '44200' }).port).toBe(44200)
    expect(parseWebModeOptions(['electron', '--purescience-headless'], {})).toMatchObject({
      enabled: true,
      headless: true
    })
  })
