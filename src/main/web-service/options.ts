...
const parseWebModeOptions = (
  ...
  const headless = argv.includes('--purescience-headless')
  const serveArg = argv.find((arg) => arg === '--serve' || arg.startsWith('--serve='))
  const envPort = env.PURESCIENCE_WEB_PORT?.trim()
  ...
  if (!Number.isInteger(parsedPort) || parsedPort < 0 || parsedPort > 65535) {
    throw new Error(`Invalid PureScience web port: ${requestedPort}`)
  }
