...
const createStorageRoot = async (): Promise<string> => {
  storageRoot = await mkdtemp(join(tmpdir(), 'purescience-artifact-mcp-'))
  return storageRoot
}
...
describe('artifact MCP server', () => {
  ...
  it('builds an ACP stdio MCP server config for the artifact tool process', () => {
    const config = createArtifactMcpServerConfig({
      ...
      storageRoot: '/Users/example/.purescience',
      projectName: 'default-project',
      sessionId: 'session-1',
      currentRunFile:
        '/Users/example/.purescience/artifacts/default-project/session-1/.pending/current-run.json',
      allowedImportRoots: ['/Users/example/workspace', '/Users/example/.purescience/notebooks']
    })
    ...
    expect(config).toEqual({
      ...
      env: [
        ...
        { name: 'PURESCIENCE_ARTIFACT_STORAGE_ROOT', value: '/Users/example/.purescience' },
        { name: 'PURESCIENCE_ARTIFACT_PROJECT_NAME', value: 'default-project' },
        { name: 'PURESCIENCE_ARTIFACT_SESSION_ID', value: 'session-1' },
        {
          name: 'PURESCIENCE_ARTIFACT_CURRENT_RUN_FILE',
          value:
            '/Users/example/.purescience/artifacts/default-project/session-1/.pending/current-run.json'
        },
        {
          name: 'PURESCIENCE_ARTIFACT_ALLOWED_IMPORT_ROOTS',
          value: JSON.stringify([
            '/Users/example/workspace',
            '/Users/example/.purescience/notebooks'
          ])
        }
      ]
    })
  ...
  it('passes the Windows named-pipe path to the artifact MCP process', () => {
    const config = createArtifactMcpServerConfig({
      command: 'C:\\PureScience.exe',
      entryPath: 'C:\\app\\main.js',
      storageRoot: 'C:\\PureScience',
      projectName: 'default-project',
      sessionId: 'session-1',
      currentRunFile: 'C:\\PureScience\\current-run.json',
      allowedImportRoots: ['C:\\workspace'],
      rpcEndpoint: 'http://localhost',
      rpcSocketPath: '\\\\.\\pipe\\purescience-notebook'
    })

    expect(config.env).toContainEqual({
      name: 'PURESCIENCE_ARTIFACT_RPC_SOCKET_PATH',
      value: '\\\\.\\pipe\\purescience-notebook'
    })
  })
  ...
  it('parses allowed import roots from the MCP process environment', () => {
    expect(
      createArtifactMcpEnvironmentFromProcess({
        PURESCIENCE_ARTIFACT_STORAGE_ROOT: '/Users/example/.purescience',
        PURESCIENCE_ARTIFACT_PROJECT_NAME: 'default-project',
        PURESCIENCE_ARTIFACT_SESSION_ID: 'session-1',
        PURESCIENCE_ARTIFACT_CURRENT_RUN_FILE: '/tmp/current-run.json',
        PURESCIENCE_ARTIFACT_ALLOWED_IMPORT_ROOTS: JSON.stringify([
          '/Users/example/workspace',
          '/Users/example/.purescience/notebooks'
        ])
      })
