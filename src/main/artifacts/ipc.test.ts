...
const ARTIFACT_DATA_ROOT = '/tmp/purescience-artifact-data-root'
vi.mock('../storage-root', () => ({
  resolveDataRoot: () => ARTIFACT_DATA_ROOT,
  resolveStorageRoot: () => '/tmp/purescience-artifact-config-root'
}))

let storageRoot: string | undefined

const createStorageRoot = async (): Promise<string> => {
  storageRoot = await mkdtemp(join(tmpdir(), 'purescience-artifact-ipc-'))
  return storageRoot
}
