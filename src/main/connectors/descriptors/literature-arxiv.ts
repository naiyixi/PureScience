...
// 可复用的 arXiv 搜索核心: 供 arxiv_search 工具与多源聚合工具 (literature_review_search) 并行调用。
...
export const searchArxiv = async (
  ...
  sort_by: string
  sort_order: string
  records: ReturnType<typeof parseEntry>[]
}> => {
  const query = params.query != null ? params.query.trim() : ''
  const category = params.category != null ? params.category.trim() : ''
  const dateFrom = params.dateFrom != null ? params.dateFrom.trim() : ''
  const dateTo = params.dateTo != null ? params.dateTo.trim() : ''
  ...
  const start = params.start != null ? Math.trunc(Number(params.start)) || 0 : 0
  const maxResults = clamp(Number(params.maxResults ?? 25), 1, 100)
  const sortBy = String(params.sortBy ?? 'relevance')
  const sortOrder = String(params.sortOrder ?? 'descending')
  const url =
...
export const ARXIV_LITERATURE_TOOLS: ToolDescriptor[] = [
  {
    ...
    run: async (ctx, a) =>
      searchArxiv(ctx, {
        query: a.query != null ? String(a.query) : undefined,
        category: a.category != null ? String(a.category) : undefined,
        dateFrom: a.date_from != null ? String(a.date_from) : undefined,
        dateTo: a.date_to != null ? String(a.date_to) : undefined,
        start: a.start != null ? Number(a.start) : undefined,
        maxResults: a.max_results != null ? Number(a.max_results) : undefined,
        sortBy: a.sort_by != null ? String(a.sort_by) : undefined,
        sortOrder: a.sort_order != null ? String(a.sort_order) : undefined
      })
  },
  {
    id: 'arxiv_get_papers',
  ...
  {
    id: 'arxiv_get_fulltext',
    connector: 'literature',
    description:
      'Fetch the full text of one arXiv paper by ID — grabs the arXiv HTML version (https://arxiv.org/html/<id>) and returns its main body text with nav/scripts stripped, truncated to ~60K chars. Args: arxiv_id (any common form — 2103.14030, versioned 2103.14030v2, old-style q-bio/0601001, arXiv:-prefixed, or abs/pdf URLs). Returns {arxiv_id, title, fulltext, source_url, note}. Papers without an HTML version (older submissions) return fulltext: null with a note — use the pdf_url from arxiv_search/arxiv_get_papers for those.',
    input: {
      type: 'object',
      properties: {
    ...
    required: ['arxiv_id'],
    returns:
      '`{ arxiv_id: str, title: str|null, fulltext: str|null, source_url: str, note: str|null }` — `fulltext` is the paper body text (≤60K chars) when the HTML version exists, else null.',
    example:
      'const r = await host.mcp("literature", "arxiv_get_fulltext", {"arxiv_id": "1706.03762"})',
    run: async (ctx, a) => {
      const raw = String((a as { arxiv_id?: unknown }).arxiv_id ?? '')
      if (!raw) throw new Error('arxiv_get_fulltext requires arxiv_id')
      const base = splitVersion(normalizeId(raw)).base
      if (!isValidArxivId(base)) throw new Error(`Malformed arXiv id: ${raw}`)
      const url = `https://arxiv.org/html/${encodeURIComponent(base)}`
      let html: string
      try {
        ...
        return {
          ...
          fulltext: null,
          source_url: url,
          note: `No HTML version available (${(e as Error).message || 'fetch failed'}). Older papers may only have PDF — use the pdf_url from arxiv_search/arxiv_get_papers.`
        }
      }
      if (html.length < 200) {
        return {
          ...
          fulltext: null,
          source_url: url,
          note: 'HTML page appears empty (no HTML version).'
        }
      }
      // Strip script/style/nav/header/footer blocks, then all tags, then collapse whitespace.
      const body = html
      ...
      const fulltext = body.length > 60000 ? `${body.slice(0, 60000)}…[truncated]` : body
      return {
        arxiv_id: base,
        title,
        fulltext: fulltext.length > 0 ? fulltext : null,
        source_url: url,
        note: fulltext.length > 0 ? null : 'Extracted empty text from the HTML page.'
      }
    }
  }
]
