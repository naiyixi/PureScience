...
const resolveE2eStorageRoot = (): string | undefined => {
  const root = process.env.PURESCIENCE_E2E_STORAGE_ROOT?.trim()
  if (!root) return undefined
  if (!isAbsolute(root)) {
    throw new Error('PURESCIENCE_E2E_STORAGE_ROOT must be an absolute path.')
  }
...
const resolveStorageRoot = (): string => {
  ...
  const previewRoot = process.env.PURESCIENCE_STORAGE_ROOT?.trim()

  if (!app.isPackaged && previewRoot) {
    if (!isAbsolute(previewRoot)) {
      throw new Error('PURESCIENCE_STORAGE_ROOT must be an absolute path.')
    }
...
const dataFolderName = (): string => (app.isPackaged ? 'PureScience' : 'PureScience-DEV')
...
// (`<picked>/PureScience`), but when the user navigated INTO and selected the PureScience folder
// itself (its basename already equals the data folder name), it is used as-is. Without this,
// picking the existing/default data folder would derive `<picked>/PureScience/PureScience` — a
// doubled, non-existent path that reports "data folder not found" on the next launch. The name
// match is case-insensitive on Windows (its filesystem is), so `...\purescience` is still
...
// Default data root for a fresh install is `~/PureScience` (dev `~/PureScience-DEV`). A legacy
// install - config root already holds data and never got an PureScience subdir - keeps its data
...
// settings explicitly points at `<home>/PureScience`; directory existence alone is not evidence,
...
// the parent-based relocation flow (inspect/migrate) reproduces the default `<home>/PureScience`
