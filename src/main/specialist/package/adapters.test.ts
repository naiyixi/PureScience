...
describe('Specialist package source adapters', () => {
  ...
  it('accepts Finder-created wrapper archives with macOS metadata', async () => {
    const packageFiles = Object.fromEntries(
      await Promise.all(
        ['manifest.json', 'specialist.json', 'README.txt'].map(async (fileName) => [
          `purescience-specialist-template/${fileName}`,
    ...
    const zip = zipSync({
      ...packageFiles,
      '__MACOSX/purescience-specialist-template/._manifest.json': new Uint8Array([1, 2, 3]),
      '__MACOSX/purescience-specialist-template/._specialist.json': new Uint8Array([4, 5, 6])
    })
  ...
  it('accepts Finder directory entries in wrapper archives', async () => {
    ...
    const zip = zipSync({
      'purescience-specialist-template/': new Uint8Array(),
      ...packageFiles,
      '__MACOSX/': new Uint8Array(),
      '__MACOSX/purescience-specialist-template/': new Uint8Array(),
      '__MACOSX/purescience-specialist-template/._manifest.json': new Uint8Array([1, 2, 3])
    })
