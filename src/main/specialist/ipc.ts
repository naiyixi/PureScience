...
export const registerSpecialistIpcHandlers = (
  ...
  marketplace?: {
    ...
    broadcastDownloadProgress: (progress: MarketplaceDownloadProgress) => void
  }
): void => {
  ...
  if (marketplace) {
    ...
    ipcMainHandle(
      ...
      async (_event, request: InspectGitHubMarketplaceSourceRequest) =>
        marketplaceService.inspectGitHubSource(request)
    )
    ipcMainHandle(
      SPECIALIST_MARKETPLACE_IPC.ADD_SOURCE,
      async (_event, request: AddMarketplaceSourceRequest) => marketplaceService.addSource(request)
    )
    ipcMainHandle(
    ...
    ipcMainHandle(
      ...
      async (_event, request: PrepareMarketplaceInstallRequest) =>
        marketplaceService.prepareInstall(request, undefined, broadcastDownloadProgress)
    )
    ipcMainHandle(
      SPECIALIST_MARKETPLACE_IPC.CANCEL_CANDIDATE,
      async (_event, request: CancelMarketplaceCandidateRequest) => {
        marketplaceService.cancel(request.candidateToken)
      }
    )
    ipcMainHandle(
      SPECIALIST_MARKETPLACE_IPC.INSTALL,
      async (_event, request: MarketplaceInstallRequest): Promise<MarketplaceInstallResult> =>
        marketplaceService.install(request)
    )
  }
}
