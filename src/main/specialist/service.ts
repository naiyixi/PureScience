...
export class ProfileService {
  ...
  async create(input: CreateSpecialistInput): Promise<SpecialistProfileView> {
    ...
    // Resolve the immutable public ID: an explicit valid id wins; otherwise infer from the name and
    // fall back to a UUID when the inferred value is unsafe or already taken.
    let resolvedId = input.id ?? inferSpecialistId(name)
    if (resolvedId === undefined || usedIds.has(resolvedId)) {
