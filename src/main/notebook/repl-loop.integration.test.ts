...
describe('repl_loop local RPC transport', () => {
  it('routes host.mcp through the issued local socket', async () => {
    ...
    const { child, send } = startLoop({
      ...
      PURESCIENCE_NOTEBOOK_SESSION_ID: 'session-1'
    })
...
gate('repl_loop.js', () => {
  ...
  it('blocks child process payloads that write into the managed runtime', async () => {
    ...
    const { child, send } = startLoop({ PURESCIENCE_RUNTIME_DIR: runtimeRoot })
    try {
      const result = await send(
        `require('node:child_process').execFileSync('/bin/sh', ` +
          `['-c', 'touch "$PURESCIENCE_RUNTIME_DIR/child-blocked.txt"'])`
      )
      expect(result.error).toMatch(/manage_packages/)
  ...
  it.each([
    [
      'shell',
      '/bin/sh',
      ['-c', 'cd "$PURESCIENCE_RUNTIME_DIR"; touch relative-shell.txt'],
      'relative-shell.txt'
    ],
    [
      'PowerShell',
      ...
      [
        '-NoProfile',
        '-c',
        'Set-Location $env:PURESCIENCE_RUNTIME_DIR; Set-Content relative-powershell.txt x'
      ],
    ...
    [
      ...
      '/bin/sh',
      ['-c', 'cd "$PURESCIENCE_RUNTIME_DIR/subdir"; cd ..; touch relative-shell-subdir.txt'],
    ...
    [
      ...
      [
        '-NoProfile',
        '-c',
        'Set-Location $env:PURESCIENCE_RUNTIME_DIR\\subdir; Set-Location ..; Set-Content relative-powershell-subdir.txt x'
      ],
  ...
  it('allows child process payloads that only read the runtime and write elsewhere', async () => {
    ...
    try {
      const payload = [
        ...
        `console.log(process.env.PURESCIENCE_RUNTIME_DIR)`,
      ...
      if (process.platform !== 'win32') {
        ...
        const shellPayload =
          `cp "$PURESCIENCE_RUNTIME_DIR/source.txt" ${JSON.stringify(shellCopy)}; ` +
          `printf '%s' "$PURESCIENCE_RUNTIME_DIR" > /dev/null; ` +
          `cd "$PURESCIENCE_RUNTIME_DIR"; ` +
  ...
  it('uses PowerShell write targets instead of blocking every runtime-path mention', async () => {
    ...
    try {
      const allowedPayload =
        `Copy-Item "$env:PURESCIENCE_RUNTIME_DIR\\source.txt" ${JSON.stringify(join(workspace, 'report.txt'))}; ` +
        `Write-Output $env:PURESCIENCE_RUNTIME_DIR; ` +
        `Set-Location $env:PURESCIENCE_RUNTIME_DIR; ` +
      ...
      const blocked = await send(
        `require('node:child_process').execFileSync('powershell.exe', ` +
          `['-NoProfile', '-Command', ` +
          `${JSON.stringify(`Set-Content "$env:PURESCIENCE_RUNTIME_DIR\\blocked.txt" x`)}])`
      )
  ...
  it('blocks managed-runtime writes routed through a temporary variable', async () => {
    ...
    try {
      const result = await send(
        `const target = process.env.PURESCIENCE_RUNTIME_DIR + '/blocked.txt'; ` +
  ...
  it.each([
    `require('node:fs').mkdtempSync(process.env.PURESCIENCE_RUNTIME_DIR + '/sync-')`,
    `await require('node:fs').promises.mkdtemp(process.env.PURESCIENCE_RUNTIME_DIR + '/promise-')`,
    `await new Promise((resolve, reject) => require('node:fs').mkdtemp(` +
      `process.env.PURESCIENCE_RUNTIME_DIR + '/callback-', ` +
...
gate('repl_loop.js host.compute', () => {
  ...
  it('threads session/project identity from the spawn env into the call_command payload', async () => {
    ...
    const { child, send } = startLoop({
      ...
      PURESCIENCE_NOTEBOOK_PROJECT_NAME: 'my-project'
    })
    try {
      const r = await send(
  ...
  it('removes the session/project identity from process.env so sandbox code cannot read it', async () => {
    ...
    try {
      const r = await send(
        'return JSON.stringify([process.env.PURESCIENCE_NOTEBOOK_SESSION_ID, process.env.PURESCIENCE_NOTEBOOK_PROJECT_NAME])'
      )
      expect(r.error).toBeNull()
