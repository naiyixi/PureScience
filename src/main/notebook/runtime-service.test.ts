...
const createStorageRoot = async (): Promise<string> => {
  storageRoot = await mkdtemp(join(tmpdir(), 'purescience-notebook-runtime-'))
  return storageRoot
}
...
describe('notebook runtime service', () => {
  ...
  it('does not thread the mcp RPC connection into the data-cell execute request', async () => {
    ...
    service.setMcpRpcConnectionResolver(async () => ({
      ...
      socketPath: '\\\\.\\pipe\\purescience-notebook',
      token: 'tok'
    }))
  ...
  describe('executeShell', () => {
    ...
    it.each([
      'Set-Location $env:PURESCIENCE_RUNTIME_DIR; New-Item conda-meta\\pwn.json',
      'Remove-Item "$env:PURESCIENCE_RUNTIME_DIR\\conda-meta\\history"'
    ...
    it.skipIf(process.platform === 'win32')(
      ...
      async () => {
        const root = await createStorageRoot()
        process.env.PURESCIENCE_TEST_SECRET = 'super-secret-token'
        try {
          const service = new NotebookRuntimeService({
            configRoot: root,
            dataRoot: root,
          ...
          const result = await service.executeShell({
            ...
            workspaceCwd: root,
            command:
              'echo "secret=[${PURESCIENCE_TEST_SECRET}]"; echo "handoff=[${PURESCIENCE_HANDOFF_DIR:+set}]"'
          })
          ...
          delete process.env.PURESCIENCE_TEST_SECRET
        }
      }
    )
  ...
  describe('lifecycle & concurrency (G2/G3/G4/G5)', () => {
    ...
    it('keeps the kernel idle when the runtime mutation policy rejects before execution', async () => {
      ...
      const run = await service.execute({
        sessionId: 'session-1',
        workspaceCwd: root,
        code: `open(os.path.join(os.environ["PURESCIENCE_RUNTIME_DIR"], "blocked"), "w")`
      })
  ...
  it('surfaces (without throwing) when the session cwd has disappeared before a run', async () => {
    ...
    const changedCwd = await mkdtemp(join(tmpdir(), 'purescience-notebook-chdir-'))
  ...
  describe('default executor factory (D-B4)', () => {
    afterEach(() => {
      delete process.env.PURESCIENCE_PYTHON_LOOP
      delete process.env.PURESCIENCE_R_LOOP
      delete process.env.PURESCIENCE_REPL_LOOP
    })
    ...
    it('honors PURESCIENCE_PYTHON_LOOP / PURESCIENCE_R_LOOP / PURESCIENCE_REPL_LOOP overrides', () => {
      process.env.PURESCIENCE_PYTHON_LOOP = '/tmp/custom-python-loop.py'
      process.env.PURESCIENCE_R_LOOP = '/tmp/custom-r-loop.R'
      process.env.PURESCIENCE_REPL_LOOP = '/tmp/custom-repl-loop.js'
...
describe('v4 runtime bindings & agent tools', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const manualDir = await mkdtemp(join(tmpdir(), 'purescience-manual-interp-'))
