...
//   RUN_KERNEL=1 PURESCIENCE_TEST_PY_ENV=/opt/homebrew/bin/python3 \
//   PURESCIENCE_TEST_R_ENV=/usr/local \
//   npx vitest run src/main/notebook/e2e.certification.test.ts
const pyBin = process.env.PURESCIENCE_TEST_PY_ENV
...
const makeHarness = async (opts: { idleTimeoutMs?: number } = {}): Promise<Harness> => {
  const storageRoot = await mkdtemp(join(tmpdir(), 'purescience-cert-'))
...
gate('notebook capability certification (E2E)', () => {
  ...
  it('4. cross-kernel handoff: repl fetches via host.mcp, writes ./handoff, python reads + computes', async () => {
    ...
    const write = await viaMcp<NotebookControlResult>(h.env, 'executeControl', {
      code: [
        ...
        'const dir = process.env.PURESCIENCE_HANDOFF_DIR',
    ...
    const read = await viaMcp<NotebookRunSummary>(h.env, 'execute', {
      code: [
        'import os, json',
        'p = os.path.join(os.environ["PURESCIENCE_HANDOFF_DIR"], "data.json")',
  ...
  it('6a. TRUST BOUNDARY: python has no host and no connector RPC env; repl in the same session does', async () => {
    ...
    const probe = await viaMcp<NotebookRunSummary>(h.env, 'execute', {
      code: [
        'import os',
        'print("ENDPOINT_IN_ENV=", "PURESCIENCE_MCP_RPC_ENDPOINT" in os.environ)',
        'print("TOKEN_IN_ENV=", "PURESCIENCE_MCP_RPC_TOKEN" in os.environ)',
        'try:',
        '    host',
        '    print("HOST=present")',
        'except NameError:',
  ...
  rIt(
    ...
    async () => {
      ...
      const probe = await viaMcp<NotebookRunSummary>(h.env, 'execute', {
        code: [
          'cat("ENDPOINT=[", Sys.getenv("PURESCIENCE_MCP_RPC_ENDPOINT"), "]\\n", sep="")',
          'cat("TOKEN=[", Sys.getenv("PURESCIENCE_MCP_RPC_TOKEN"), "]\\n", sep="")',
