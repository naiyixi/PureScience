...
// Resolves the exec-loop scripts the default executor spawns. Env overrides (PURESCIENCE_PYTHON_LOOP
// / PURESCIENCE_R_LOOP / PURESCIENCE_REPL_LOOP) win for tests and dev, then the packaged/dev
// candidates above.
const resolveLoopScriptPaths = (): {
  ...
  pythonLoopPath: resolveLoopScript(process.env.PURESCIENCE_PYTHON_LOOP, 'python_loop.py'),
  rLoopPath: resolveLoopScript(process.env.PURESCIENCE_R_LOOP, 'r_loop.R'),
  replLoopPath: resolveLoopScript(process.env.PURESCIENCE_REPL_LOOP, 'repl_loop.js')
})
