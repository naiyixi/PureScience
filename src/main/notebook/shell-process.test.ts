...
describe('notebook shell process behavior', () => {
  describe('invocation', () => {
    ...
    it('uses an absolute non-interactive PowerShell command on Windows without relying on PATH', () => {
      ...
      expect(script).toContain('[Console]::OutputEncoding = $purescienceUtf8')
      expect(script).toContain('$OutputEncoding = $purescienceUtf8')
      ...
      const encodedCommand = script.match(/\$purescienceCommandBase64 = '([A-Za-z0-9+/=]+)'/)?.[1]
      expect(Buffer.from(encodedCommand ?? '', 'base64').toString('utf8')).toBe(
        'cp "source.png" "destination.png"'
      )
      expect(script).toContain('[ScriptBlock]::Create($purescienceCommandText)')
      expect(script).toContain('& $purescienceCommand')
      expect(script).toContain('$purescienceSucceeded = $?')
      expect(script).toContain('exit $purescienceNativeExitCode')
      expect(script).toMatch(/if \(\$purescienceSucceeded\) \{ exit 0 \}/)
      expect(script.indexOf('exit $purescienceNativeExitCode')).toBeLessThan(
        script.indexOf('if ($purescienceSucceeded) { exit 0 }')
      )
