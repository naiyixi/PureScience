...
// Run with: RUN_KERNEL=1 PURESCIENCE_TEST_PY_ENV=/path/to/env/bin/python \
    ...
    async () => {
      ...
      try {
        ...
        const workspaceWrite = await send(
          `subprocess.run(["sh", "-c", ` +
            `${JSON.stringify(`printf '%s' "$PURESCIENCE_RUNTIME_DIR" >/dev/null; mkdir ${JSON.stringify(outputDir)}`)}], check=True)`
        )
...
gate('python_loop.py data-kernel isolation', () => {
  it('exposes no host symbol even when the connector RPC env is present', async () => {
    ...
    const { child, send } = startLoop(pyBin as string, {
      PURESCIENCE_MCP_RPC_ENDPOINT: 'http://127.0.0.1:9/x',
