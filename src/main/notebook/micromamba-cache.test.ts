...
describe('selectMicromambaCache', () => {
  ...
  it('uses a compact profile cache when the volume root is untrusted and the legacy leaf is too long', () => {
    ...
    const selected = selectMicromambaCache(
      'E:\\open science\\PureScience\\runtime',
      DEFAULT_MAX_CACHE_RELATIVE_PATH,
      windowsDeps({
        env: {
  ...
  it('uses the public directory when the runtime volume is untrusted and the profile path is too long', () => {
    ...
    const selected = selectMicromambaCache(
      'E:\\deeply\\nested\\storage\\PureScience\\runtime',
      DEFAULT_MAX_CACHE_RELATIVE_PATH,
      windowsDeps({
        env: {
          USERNAME: 'peipeidamowang',
  ...
  it('keeps non-Windows cache behavior unchanged', () => {
    expect(
      selectMicromambaCache('/Users/alice/PureScience/runtime', 999, {
        platform: 'darwin',
        env: {},
      ...
      path: '/Users/alice/PureScience/runtime/pkgs',
      lockKey: '/Users/alice/PureScience/runtime/pkgs'
    })
  })
...
describe('removeMicromambaCacheForRoot', () => {
  const root = 'D:\\PureScience\\runtime'
  const env = {
    USERNAME: 'alice',
    PUBLIC: 'C:\\Users\\Public',
