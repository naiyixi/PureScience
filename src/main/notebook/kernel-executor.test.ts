...
const resolvePython3 = (): string | undefined =>
  [
    process.env.PURESCIENCE_TEST_PY_ENV,
    ...
    try {
      ...
      const posixSpawnResult = await executor.execute({
        ...
        code:
          ...
          `'touch "$PURESCIENCE_RUNTIME_DIR/blocked-from-posix-spawn.txt"'], os.environ)`,
        language: 'python'
      })
  ...
  it('blocks managed-runtime writes routed through a temporary R variable', async () => {
    ...
    try {
      const result = await executor.execute({
        ...request,
        code:
          'target <- file.path(Sys.getenv("PURESCIENCE_RUNTIME_DIR"), "blocked-r.txt"); ' +
      ...
      const childResult = await executor.execute({
        ...request,
        code:
          'target <- file.path(Sys.getenv("PURESCIENCE_RUNTIME_DIR"), "blocked-r-child.txt"); ' +
      ...
      const cases = [
        {
          ...
          code:
            'target <- file.path(Sys.getenv("PURESCIENCE_RUNTIME_DIR"), "blocked-file-append.txt"); ' +
        ...
        },
        {
          ...
          code:
            'target <- file.path(Sys.getenv("PURESCIENCE_RUNTIME_DIR"), "blocked-file-copy.txt"); ' +
        ...
        },
        {
          ...
          code:
            'target <- file.path(Sys.getenv("PURESCIENCE_RUNTIME_DIR"), "blocked-download.txt"); ' +
        ...
        },
        {
          ...
          code:
            'target <- file.path(Sys.getenv("PURESCIENCE_RUNTIME_DIR"), "blocked-fifo"); ' +
        ...
        },
        {
          ...
          code:
            'target <- file.path(Sys.getenv("PURESCIENCE_RUNTIME_DIR"), "blocked-pipe.txt"); ' +
...
gate('NotebookKernelExecutor idle-timeout shutdown', () => {
  ...
  it('arms NO idle timer by default, so the kernel namespace persists across a pause', async () => {
    ...
    const h = makeTimerHarness()
    // No idleTimeoutMs and no PURESCIENCE_KERNEL_IDLE_MS → idle reclaim is disabled (the default).
    const executor = new NotebookKernelExecutor({
      pythonBin: python3,
      pythonLoopPath: FIXTURE,
      platform: 'linux',
...
describe('NotebookKernelExecutor spawn env', () => {
  it('injects PURESCIENCE_HANDOFF_DIR under the notebook session root for every kernel language', () => {
    ...
    expect(buildEnv('python', request, '/tmp/figs').PURESCIENCE_HANDOFF_DIR).toBe(expected)
    expect(buildEnv('r', request, '/tmp/figs').PURESCIENCE_HANDOFF_DIR).toBe(expected)
    expect(buildEnv('repl', request, '/tmp/figs').PURESCIENCE_HANDOFF_DIR).toBe(expected)
  })
  ...
  it('gives the repl kernel ELECTRON_RUN_AS_NODE plus the connector RPC endpoint/token', () => {
    ...
    const request = {
      ...
      mcpRpcSocketPath: '\\\\.\\pipe\\purescience-notebook',
      mcpRpcToken: 'tok'
    }
    const buildEnv = (executor as unknown as { buildEnv: BuildEnvFn }).buildEnv.bind(executor)
    ...
    expect(replEnv.PURESCIENCE_MCP_RPC_ENDPOINT).toBe('http://127.0.0.1:9/x')
    expect(replEnv.PURESCIENCE_MCP_RPC_SOCKET_PATH).toBe('\\\\.\\pipe\\purescience-notebook')
    expect(replEnv.PURESCIENCE_MCP_RPC_TOKEN).toBe('tok')
  })
  ...
  it('withholds the connector RPC env from python/r data kernels (host.mcp is repl-only)', () => {
    ...
    for (const kind of ['python', 'r'] as const) {
      ...
      expect(env.PURESCIENCE_MCP_RPC_ENDPOINT).toBeUndefined()
      expect(env.PURESCIENCE_MCP_RPC_SOCKET_PATH).toBeUndefined()
      expect(env.PURESCIENCE_MCP_RPC_TOKEN).toBeUndefined()
  ...
  it('does not contaminate an external Windows R interpreter with managed conda DLL paths', () => {
    ...
    expect(env.PURESCIENCE_R_ENV_PREFIX).toBeUndefined()
  ...
  it('activates an external Windows conda R interpreter with its own DLL paths', () => {
    ...
    expect(env.PURESCIENCE_R_ENV_PREFIX).toBe(prefix)
