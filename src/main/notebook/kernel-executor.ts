...
// kernel is opt-in via PURESCIENCE_KERNEL_IDLE_MS (a positive ms value); 0 / unset keeps kernels
...
// Resolves the idle window: an explicit option wins, then PURESCIENCE_KERNEL_IDLE_MS (a positive ms
...
const resolveIdleTimeoutMs = (configured?: number): number => {
  ...
  const raw = Number(process.env.PURESCIENCE_KERNEL_IDLE_MS)
...
export type NotebookKernelExecutorOptions = {
  ...
  rEnvPrefix?: string
  // Path to resources/notebook/python_loop.py (env override PURESCIENCE_PYTHON_LOOP).
  pythonLoopPath?: string
  // Path to resources/notebook/r_loop.R (env override PURESCIENCE_R_LOOP).
  rLoopPath?: string
  // Path to resources/notebook/repl_loop.js (env override PURESCIENCE_REPL_LOOP). Spawned under
  ...
  // respawns a fresh one (namespace cleared). Defaults to PURESCIENCE_KERNEL_IDLE_MS if that is a
...
const defaultPythonLoopPath = (): string => {
  if (process.env.PURESCIENCE_PYTHON_LOOP) return process.env.PURESCIENCE_PYTHON_LOOP
...
const defaultRLoopPath = (): string => {
  if (process.env.PURESCIENCE_R_LOOP) return process.env.PURESCIENCE_R_LOOP
...
const defaultReplLoopPath = (): string => {
  if (process.env.PURESCIENCE_REPL_LOOP) return process.env.PURESCIENCE_REPL_LOOP
...
class NotebookKernelExecutor implements NotebookExecutor {
  ...
  private buildEnv(
    ...
    const env: NodeJS.ProcessEnv = {
      ...
      PURESCIENCE_NOTEBOOK_DIR: request.notebookSessionRoot,
      PURESCIENCE_NOTEBOOK_DATA_DIR: request.dataRoot,
      PURESCIENCE_RUNTIME_DIR: request.runtimeRoot,
      // Cross-kernel workspace channel (see repository.ts): same path every kernel kind sees.
      PURESCIENCE_HANDOFF_DIR: join(request.notebookSessionRoot, 'handoff'),
      // App-owned directories the kernel must not read (e.g. materialized skill files).
      PURESCIENCE_PROTECTED_DIRS: (request.protectedDirs ?? []).join(delimiter),
      ...
      ...(kind === 'repl' && request.mcpRpcEndpoint
        ? { PURESCIENCE_MCP_RPC_ENDPOINT: request.mcpRpcEndpoint }
        : {}),
      ...(kind === 'repl' && request.mcpRpcSocketPath
        ? { PURESCIENCE_MCP_RPC_SOCKET_PATH: request.mcpRpcSocketPath }
        : {}),
      ...(kind === 'repl' && request.mcpRpcToken
        ? { PURESCIENCE_MCP_RPC_TOKEN: request.mcpRpcToken }
        : {}),
      ...
      ...(kind === 'repl' && request.sessionId
        ? { PURESCIENCE_NOTEBOOK_SESSION_ID: request.sessionId }
        : {}),
      ...(kind === 'repl' && request.projectName
        ? { PURESCIENCE_NOTEBOOK_PROJECT_NAME: request.projectName }
        : {}),
      ...(kind === 'repl' ? { ELECTRON_RUN_AS_NODE: '1' } : {}),
      ...(rEnvPrefix ? { PURESCIENCE_R_ENV_PREFIX: rEnvPrefix } : {})
    }
  ...
  private ensureFiguresDir(): string {
    if (!this.figuresDir) {
      this.figuresDir = mkdtempSync(join(tmpdir(), 'purescience-kernel-figs-'))
    }
