...
// Run with: RUN_KERNEL=1 PURESCIENCE_TEST_R_ENV=/path/to/r/env/prefix \
//   npx vitest run src/main/notebook/r-loop.integration.test.ts
// PURESCIENCE_TEST_R_ENV is the R environment's prefix directory; the Rscript binary is expected
// at <prefix>/bin/Rscript.
const rEnvPrefix = process.env.PURESCIENCE_TEST_R_ENV
...
const installBlankPngMaterializationTrace = (
  ...
  [
    '.purescience_test_png <- new.env(parent = emptyenv())',
    `.purescience_test_png$blank_bytes <- ${blankPngVector}`,
    `.purescience_test_png$capture_bytes <- ${options.capturePngVector ?? blankPngVector}`,
    `.purescience_test_png$materialize_capture <- ${options.materializeCapture ? 'TRUE' : 'FALSE'}`,
    'trace(grDevices::png, quote({',
    '  .purescience_test_png$filename <- filename',
    '}), print = FALSE)',
    'trace(grDevices::dev.off, exit = quote({',
    '  pattern <- .purescience_test_png$filename',
    '  figures_dir <- Sys.getenv("PURESCIENCE_KERNEL_FIGURES_DIR")',
    ...
    '    should_materialize_blank <- grepl("purescience-blank-r-", pattern, fixed = TRUE)',
    '    should_materialize_capture <- isTRUE(.purescience_test_png$materialize_capture) && is_capture_path',
    '    if (should_materialize_blank && !file.exists(path)) writeBin(.purescience_test_png$blank_bytes, path)',
    '    if (should_materialize_capture) writeBin(.purescience_test_png$capture_bytes, path)',
    '  }',
    '}), print = FALSE)'
  ].join('\n')
...
gate('r_loop.R', () => {
  ...
  it.each(['file.link', 'file.symlink'] as const)(
    ...
    async (operation) => {
      ...
      const { child, send } = startLoop(rscriptBin(), {
        PURESCIENCE_RUNTIME_DIR: runtimeRoot
      })
      try {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      try {
        ...
        const shellPayload =
          `printf '%s' "$PURESCIENCE_RUNTIME_DIR" >/dev/null; ` +
  ...
  it('captures a base graphics figure as a content-addressed PNG', async () => {
    ...
    const { child, send } = startLoop(rscriptBin(), {
      PURESCIENCE_KERNEL_FIGURES_DIR: figuresDir
    })
    try {
  ...
  it('skips unreadable raw PNG page paths without killing the loop', async () => {
    ...
    try {
      const r = await send(
        [
          'dir.create(file.path(Sys.getenv("PURESCIENCE_KERNEL_FIGURES_DIR"), "page-001.png"))',
          'print("text only")'
        ].join('; ')
      )
      expect(r.error).toBeNull()
    ...
    try {
      ...
      const r = await send(
        [
          ...
          'plot.new()',
          'grDevices::dev.off()',
          'writeBin(blank_png, file.path(Sys.getenv("PURESCIENCE_KERNEL_FIGURES_DIR"), "page-001.png"))'
        ].join('; ')
      )
      expect(r.error).toBeNull()
  ...
  it('does not confuse a user png device after graphics.off closes notebook capture', async () => {
    ...
    try {
      ...
      const r = await send(
        [
          '{',
          '  .purescience_test_png$materialize_capture <- TRUE',
  ...
  it('does not run user graphics traces for kernel-owned capture preflight', async () => {
    ...
    try {
      const installTrace = await send(
        [
          '.purescience_trace_counts <- c(png = 0L, dev.off = 0L)',
          'trace(grDevices::png, quote({',
          '  .purescience_trace_counts["png"] <<- .purescience_trace_counts["png"] + 1L',
          '}), print = FALSE)',
          'trace(grDevices::dev.off, quote({',
          '  .purescience_trace_counts["dev.off"] <<- .purescience_trace_counts["dev.off"] + 1L',
          '}), print = FALSE)'
        ].join('\n')
      )
      expect(installTrace.error).toBeNull()

      const r = await send('print("text only"); print(.purescience_trace_counts)')
      expect(r.error).toBeNull()
      expect(r.stdout).toContain('[1] "text only"')
