    ...
    if (!bundle) {
      throw new Error(
        `No verified runtime pack is available for ${spec.name} (${spec.language} ${spec.version}). ` +
          'Connect to the runtime CDN or provide an offline bundle in PURESCIENCE_ENV_BUNDLE_DIR.'
      )
    }
    onProgress({
      phase: `create-${spec.language}`,
...
export const createProductionProvisioner = (
  ...
  if (!mm) {
    throw new Error(
      'micromamba binary not found (set PURESCIENCE_MICROMAMBA_BIN or ship it as a resource)'
    )
  }
  // A packaged/dropped-in local bundle takes precedence. Shipped apps then fetch exactly one verified
  // language pack from the official CDN (or PURESCIENCE_ENV_CDN_BASE) and always create from its lock.
  ...
  const bundleSource: RuntimeBundleSource = {
    kind:
      opts.cdnBase !== undefined || process.env.PURESCIENCE_ENV_CDN_BASE !== undefined
        ? 'override'
        : 'official',
    baseUrl: cdnBase
  }
