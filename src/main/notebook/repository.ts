...
class NotebookRunRepository {
  ...
  private async mutate(
    ...
    const operation = this.saveQueue.then(async () => {
      ...
      // 持久化窗口限流: 保留最近 N 次运行, 丢弃更早记录 (防止 run.json 无限膨胀)。
      // 仅在追加/更新运行后截断 — 其他 transform (如删除) 不受影响。
      if (nextDocument.runs.length > NOTEBOOK_PERSISTED_RUN_LIMIT) {
        nextDocument.runs = nextDocument.runs.slice(-NOTEBOOK_PERSISTED_RUN_LIMIT)
      }
