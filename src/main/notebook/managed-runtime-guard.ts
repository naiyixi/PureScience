...
const referencesManagedRuntimePath = (
  ...
  if (/(?:^|[^A-Za-z0-9_])PURESCIENCE_RUNTIME_DIR(?:$|[^A-Za-z0-9_])/u.test(source)) {
    return true
  }
...
const resolveSequenceCwd = (
  ...
  const expanded = unquoted
    .replace(/\$\{?env:PURESCIENCE_RUNTIME_DIR\}?/giu, () => runtimeRoot)
    .replace(/\$\{?PURESCIENCE_RUNTIME_DIR\}?/gu, () => runtimeRoot)
    .replace(/%PURESCIENCE_RUNTIME_DIR%/giu, () => runtimeRoot)
  ...
  if (rule) {
    return {
      ...
      message:
        ...
        'Use manage_packages so PureScience can preserve the bound interpreter and audit the change.'
    }
  }
