...
export type CliLauncherEnv = {
  ...
  // it is also the app the CLI should spawn, so the shim pins PURESCIENCE_APP_PATH to it.
  appExecPath: string
...
const posixShim = (env: CliLauncherEnv): string => {
  ...
  const appPathLine = env.packaged ? `PURESCIENCE_APP_PATH=${quote(env.appExecPath)} ` : ''
  return [
    '#!/bin/sh',
    '# PureScience command-line launcher. Managed by the app (Settings -> General -> Command line',
...
// Windows: an purescience.cmd in a per-user bin dir. %* forwards all arguments.
const windowsShim = (env: CliLauncherEnv): string => {
  const appPathLine = env.packaged ? `set "PURESCIENCE_APP_PATH=${env.appExecPath}"\r\n` : ''
  return [
    '@echo off',
    'rem PureScience command-line launcher. Managed by the app; edits are overwritten on reinstall.',
...
export const planCliLauncher = (env: CliLauncherEnv): CliLauncherPlan => {
  if (env.platform === 'win32') {
    ...
    return {
      binDir,
      target: join(binDir, 'purescience.cmd'),
  ...
  return {
    binDir,
    target: join(binDir, 'purescience'),
    shim: posixShim(env),
    mode: 0o755,
    onPath: isOnPath(binDir, env.pathVar, env.platform)
  }
...
// status (installed + whether `purescience` is callable, with a hint when a manual step remains).
export const installCliLauncher = async (
  ...
  if (!onPath) {
    if (env.platform === 'win32') {
      ...
      pathHint = onPath
        ? 'Added to your PATH — open a new terminal to use "purescience".'
        : `Add ${plan.binDir} to your PATH to use "purescience".`
    } else {
      pathHint = `Add ${plan.binDir} to your PATH (e.g. in your shell profile) to use "purescience".`
    }
  }
