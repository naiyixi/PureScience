...
describe('managed preview protocol', () => {
  it('can register the capability handler on an isolated Electron session', () => {
    ...
    expect(targetProtocol.handle).toHaveBeenCalledWith('purescience-preview', expect.any(Function))
    unregister()
    expect(targetProtocol.unhandle).toHaveBeenCalledWith('purescience-preview')
  })
  ...
  it('rejects a capability that is not assigned to the isolated child session', async () => {
    ...
    const response = await handle(
      new Request('purescience-preview://different-resource/report.xlsx')
    )
  ...
  it('streams the capability URL with constrained HTML response headers', async () => {
    ...
    const request = new Request('purescience-preview://resource-1/plot.html', {
  ...
  it('caps strict Office streams to the admitted size even when the open inode grows', async () => {
    ...
    try {
      const response = await createManagedPreviewProtocolHandler(resources)(
        new Request('purescience-preview://resource-1/report.xlsx')
      )
  ...
  it('rejects URLs that are not an acquired resource capability', async () => {
    ...
    const response = await handle(new Request('purescience-preview://missing-resource/report.html'))

    expect(response.status).toBe(404)
    expect(response.headers.get('content-type')).toContain('text/html')
    const body = await response.text()
    expect(body).toContain('purescience-preview-load-error')
  ...
  it('errors the stream when verifyUnchanged rejects the final identity check mid-pull', async () => {
    ...
    const response = await handle(new Request('purescience-preview://resource-1/report.xlsx'))
    const reader = response.body!.getReader()
    const drain = async (): Promise<void> => {
      while (!(await reader.read()).done) {
