...
// directly without a real Electron runtime. isPackaged: true means dataFolderName() === 'PureScience'.
...
describe('storage IPC handlers', () => {
  ...
  it('reveals the main-resolved config root without accepting a renderer path', async () => {
    ...
    expect(openPath).toHaveBeenCalledWith(join('/home/user', '.purescience'))
  })
  ...
  it('get-info reports isDefault true when the data root falls back to the computed default', async () => {
    ...
    // The default root is `<home>/PureScience` (home mocked to /home/user), reproducible from home.
    // Derive with join so the assertion holds on Windows (backslashes), not just POSIX.
    expect(info.defaultDataRoot).toBe(join('/home/user', 'PureScience'))
  ...
  it('get-info flags legacyDataMovePrompt for an unconfigured install with data in the config root', async () => {
    ...
    try {
      // Legacy layout: user data sits directly in the hidden config root, no PureScience folder yet.
      await mkdir(join(home, '.purescience', 'artifacts'), { recursive: true })
      ...
      expect(info.dataRoot).toBe(join(home, '.purescience'))
  ...
  it('get-info flags legacyDataMovePrompt when a legacy config root contains only workspaces', async () => {
    ...
    electronHome.path = home
    try {
      await mkdir(join(home, '.purescience', 'workspaces', 'session-1'), { recursive: true })
      initDataRoot(undefined)
      registerStorageIpcHandlers(fakeDeps())
  ...
  it('get-info reports isDefault false and real usage/availableBytes for a relocated data root', async () => {
    ...
    // one-click return to `<home>/PureScience` and show the destination.
    expect(info.defaultDataRoot).toBe(join('/home/user', 'PureScience'))
    expect(info.defaultParent).toBe('/home/user')
  ...
  it("validate-data-root returns validateNewDataRoot's ok result for a parent with no PureScience subdir", async () => {
    initDataRoot(dataRoot)
    registerStorageIpcHandlers(fakeDeps())
  ...
  it('inspect-data-root returns move and the derived dataRoot for a parent with no PureScience subdir', async () => {
    initDataRoot(dataRoot)
    registerStorageIpcHandlers(fakeDeps())
  ...
  it('set-data-root-and-relaunch creates the derived target directory for a fresh empty folder', async () => {
    ...
    // created `<parent>/PureScience`, so the next launch's startup guard read the configured-but-
