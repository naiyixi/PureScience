...
const sampleMarker = (overrides: Partial<MigrationMarker> = {}): MigrationMarker => ({
  version: 1,
  token: 'tok-123',
  source: '/old/PureScience',
  target: '/new/PureScience',
...
describe('scanInventory', () => {
  ...
  it('includes a top-level SQLite file in the verified inventory', async () => {
    await writeFile(join(root, 'purescience.db'), 'database bytes')

    expect(await scanInventory(root, ['purescience.db'])).toMatchObject({
      dirs: ['purescience.db'],
      fileCount: 1,
