...
const defaultCanWrite = async (dir: string): Promise<boolean> => {
  try {
    const probePath = join(dir, `.purescience-write-test-${randomUUID()}`)
...
export const classifyDataRoot = async (
  ...
  if (!(await canWrite(resolvedParent))) {
    return {
      kind: 'invalid',
      error:
        "PureScience can't write to this folder. Make sure you have permission to it — on macOS, grant access when prompted, or pick a folder inside your home directory."
    }
  }
...
// dirs into `<parent>/PureScience`. NOTHING is committed here — no setDataRoot, no delete. The old
