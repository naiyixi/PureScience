...
describe('copyAndVerify', () => {
  ...
  it('copies and verifies top-level SQLite database files in the same migration set', async () => {
    await writeFile(join(from, 'purescience.db'), 'sqlite bytes')
    ...
    expect(result).toEqual({ ok: true })
    expect(await readFile(join(to, 'purescience.db'), 'utf8')).toBe('sqlite bytes')
    expect(await readFile(join(from, 'purescience.db'), 'utf8')).toBe('sqlite bytes')
  })
