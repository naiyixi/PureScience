...
const createStorageCommandOwner = (deps: StorageCommandOwnerDeps) => {
  ...
  const getInfo = async (): Promise<StorageInfo> => {
    ...
    // lives there. Offer the one-time "move to the visible PureScience folder" prompt until answered.
  ...
  // Discards a completed-but-uncommitted copy at `<parent>/PureScience` when the user picks "Keep
  ...
  // inline error for 'invalid') and display the derived `<parent>/PureScience` path regardless of
  // kind. Never throws.
