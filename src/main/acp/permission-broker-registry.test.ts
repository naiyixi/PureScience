...
describe('ACP permission broker with durable grants', () => {
  ...
  it('commits a Global grant before returning only the provider one-call decision', async () => {
    storageRoot = await mkdtemp(join(tmpdir(), 'purescience-broker-registry-'))
  ...
  it.each([
    ...
    async (shellDialect, command, commandPrefix) => {
      storageRoot = await mkdtemp(join(tmpdir(), 'purescience-broker-secret-'))
      client = createProjectDbClient(storageRoot)
      await ensureProjectSchema(client)
      await client.project.create({ data: { id: 'project-1', name: 'Project one' } })
      const registry = await createPermissionGrantRegistry({ getClient: async () => client! })
  ...
  it('offers only provider Once for a command that executes a mutable script', async () => {
    storageRoot = await mkdtemp(join(tmpdir(), 'purescience-broker-mutable-script-'))
    client = createProjectDbClient(storageRoot)
    await ensureProjectSchema(client)
    await client.project.create({ data: { id: 'project-1', name: 'Project one' } })
    const registry = await createPermissionGrantRegistry({ getClient: async () => client! })
  ...
  it.each([
    ...
    async (...args) => {
      ...
      storageRoot = await mkdtemp(join(tmpdir(), 'purescience-broker-mismatched-command-group-'))
      client = createProjectDbClient(storageRoot)
      await ensureProjectSchema(client)
      await client.project.create({ data: { id: 'project-1', name: 'Project one' } })
      const registry = await createPermissionGrantRegistry({ getClient: async () => client! })
  ...
  it('offers durable scopes for a Codex-proposed command group', async () => {
    storageRoot = await mkdtemp(join(tmpdir(), 'purescience-broker-codex-command-group-'))
    client = createProjectDbClient(storageRoot)
    await ensureProjectSchema(client)
    await client.project.create({ data: { id: 'project-1', name: 'Project one' } })
    const registry = await createPermissionGrantRegistry({ getClient: async () => client! })
  ...
  it.each(['agent_create', 'Skill', 'mcp__purescience_notebook__notebook_execute'])(
    'never creates durable authority from the display-only title %s',
    async (title) => {
      storageRoot = await mkdtemp(join(tmpdir(), 'purescience-broker-title-only-'))
      client = createProjectDbClient(storageRoot)
      await ensureProjectSchema(client)
      await client.project.create({ data: { id: 'project-1', name: 'Project one' } })
      const registry = await createPermissionGrantRegistry({ getClient: async () => client! })
  ...
  it('routes every registered customization and local executor identity into durable scopes', async () => {
    storageRoot = await mkdtemp(join(tmpdir(), 'purescience-broker-registered-'))
    client = createProjectDbClient(storageRoot)
    await ensureProjectSchema(client)
    await client.project.create({ data: { id: 'project-1', name: 'Project one' } })
    const registry = await createPermissionGrantRegistry({ getClient: async () => client! })
  ...
  it('reuses one app MCP grant across Claude Code, Codex, OpenCode, and runtime-trusted sparse requests', async () => {
    storageRoot = await mkdtemp(join(tmpdir(), 'purescience-broker-acp-mcp-aliases-'))
    client = createProjectDbClient(storageRoot)
    await ensureProjectSchema(client)
    await client.project.create({ data: { id: 'project-1', name: 'Project one' } })
    const registry = await createPermissionGrantRegistry({ getClient: async () => client! })
    ...
    const first = broker.requestPermission(
      mcpRequest('session-claude', 'mcp__purescience_notebook__manage_packages'),
      context
    )
    await new Promise<void>((resolve) => setImmediate(resolve))
    broker.respond({
  ...
  it('offers durable Ask scopes for Plan capabilities without sharing their grants', async () => {
    storageRoot = await mkdtemp(join(tmpdir(), 'purescience-broker-plan-grants-'))
    client = createProjectDbClient(storageRoot)
    await ensureProjectSchema(client)
    await client.project.create({ data: { id: 'project-1', name: 'Project one' } })
    const registry = await createPermissionGrantRegistry({ getClient: async () => client! })
    ...
    const context = {
      profile: 'ask' as const,
      projectId: 'project-1',
      mcpServerNames: ['purescience-plan']
    }

    const generate = broker.requestPermission(
      mcpRequest('session-plan', 'mcp__purescience_plan__generate_plan'),
      context
    )
    await new Promise<void>((resolve) => setImmediate(resolve))
    ...
    const update = broker.requestPermission(
      mcpRequest('session-plan', 'mcp__purescience_plan__update_step_status'),
      context
    )
    await new Promise<void>((resolve) => setImmediate(resolve))
    ...
    await expect(registry.list()).resolves.toEqual([
      expect.objectContaining({
        capability: {
          kind: 'mcp_tool',
          key: 'mcp:purescience-plan/generate_plan'
        },
  ...
  it('uses runtime-trusted identity to align sparse dynamic MCP requests', async () => {
    storageRoot = await mkdtemp(join(tmpdir(), 'purescience-broker-dynamic-mcp-aliases-'))
    client = createProjectDbClient(storageRoot)
    await ensureProjectSchema(client)
    await client.project.create({ data: { id: 'project-1', name: 'Project one' } })
    const registry = await createPermissionGrantRegistry({ getClient: async () => client! })
  ...
  it('rejects a trusted MCP identity outside the configured server set', async () => {
    storageRoot = await mkdtemp(join(tmpdir(), 'purescience-broker-mismatched-mcp-'))
    client = createProjectDbClient(storageRoot)
    await ensureProjectSchema(client)
    await client.project.create({ data: { id: 'project-1', name: 'Project one' } })
    const registry = await createPermissionGrantRegistry({ getClient: async () => client! })
  ...
  it('projects and revokes a durable Session grant through the composer seam', async () => {
    storageRoot = await mkdtemp(join(tmpdir(), 'purescience-broker-composer-'))
    client = createProjectDbClient(storageRoot)
    await ensureProjectSchema(client)
    await client.project.create({ data: { id: 'project-1', name: 'Project one' } })
    const registry = await createPermissionGrantRegistry({ getClient: async () => client! })
