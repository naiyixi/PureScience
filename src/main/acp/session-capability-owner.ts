...
export type SessionCapabilityMemoryOptions = {
  mcpEntryPath: string
  mcpCommand?: string
  isEnabled?: () => Promise<boolean>
...
export type SessionCapabilityRoutineOptions = {
  mcpEntryPath: string
  mcpCommand?: string
  isEnabled?: () => Promise<boolean>
...
export type SessionCapabilityEndpointOptions = {
  mcpEntryPath: string
  mcpCommand?: string
  isEnabled?: () => Promise<boolean>
...
export type SessionCapabilityPdfOptions = {
  mcpEntryPath: string
  mcpCommand?: string
  isEnabled?: () => Promise<boolean>
  getRpcConnection: (binding: {
...
export type SessionCapabilityQueryOptions = {
  mcpEntryPath: string
  mcpCommand?: string
  isEnabled?: () => Promise<boolean>
  getRpcConnection: (binding: {
    sessionId: string
    projectId: string
  }) => Promise<HostQueryRpcConnection>
  registerSessionAlias?: (aliasSessionId: string, sessionId: string) => void
}
...
type BuildSessionCapabilitiesRequest = {
  ...
  onQueryConnection?: (connection: HostQueryRpcConnection) => void
}
...
type SessionCapabilityOwnerOptions = {
  ...
  query?: SessionCapabilityQueryOptions
...
export class AcpSessionCapabilityOwner {
  ...
  private querySessionSequence = 0
  ...
  async provision(
    ...
    let queryRelease: (() => void) | undefined
    ...
    try {
      built = await this.build({
        ...
        onRoutineConnection: (connection) => {
          routineRelease = connection.release
        },
        ...
        onQueryConnection: (connection) => {
          queryRelease = connection.release
        }
      })
    } catch (error) {
  ...
  private createRoutingIds(stableAppSessionId?: string): SessionCapabilityRoutingIds {
    if (stableAppSessionId) {
      return Object.freeze({
        ...
        query: this.options.query ? stableAppSessionId : ''
      })
    }
    ...
    if (this.options.query) this.querySessionSequence += 1
    ...
    return Object.freeze({
      ...
      routine: this.options.routine
        ? `routine-session-${timestamp}-${this.routineSessionSequence}`
        : '',
  ...
  private async build(request: BuildSessionCapabilitiesRequest): Promise<BuiltSessionCapabilities> {
    ...
    const queryAllowed = policyAllowsSessionCapability(request.policy, 'query')
    ...
    if (canonicalMcpServerNames.includes(ROUTINE_MCP_SERVER_NAME)) {
      capabilities.push('routine')
    }
    ...
    if (canonicalMcpServerNames.includes(HOST_QUERY_MCP_SERVER_NAME)) {
  ...
  toolingAvailability(input: {
    ...
    return Object.freeze({
      ...
      routine:
        transportAvailable &&
        Boolean(this.options.routine) &&
        policyAllowsSessionCapability(input.policy, 'routine'),
      endpoint:
        transportAvailable &&
      ...
      query:
        transportAvailable &&
        Boolean(this.options.query) &&
        policyAllowsSessionCapability(input.policy, 'query')
    })
  }
  ...
  private async buildStdioServers(
    ...
    if (enabled.memory) {
      const environment = await this.buildMemoryEnvironment(
        request.routingIds.memory,
        request.onMemoryConnection
      )
      if (environment && this.options.memory) {
        servers.push(
          createMemoryMcpServerConfig({
            command: this.options.memory.mcpCommand ?? process.execPath,
            entryPath: this.options.memory.mcpEntryPath,
            ...environment
          })
        )
      }
    ...
    if (enabled.routine) {
      const environment = await this.buildRoutineEnvironment(
        request.routingIds.routine,
        request.onRoutineConnection
      )
      if (environment && this.options.routine) {
        servers.push(
          createRoutineMcpServerConfig({
            command: this.options.routine.mcpCommand ?? process.execPath,
            entryPath: this.options.routine.mcpEntryPath,
            ...environment
          })
        )
      }
    ...
    if (enabled.endpoint) {
      const environment = await this.buildEndpointEnvironment(
        request.routingIds.endpoint,
        request.onEndpointConnection
      )
      if (environment && this.options.endpoints) {
        servers.push(
          createEndpointMcpServerConfig({
            command: this.options.endpoints.mcpCommand ?? process.execPath,
            entryPath: this.options.endpoints.mcpEntryPath,
            ...environment
          })
        )
      }
    ...
    if (enabled.pdf) {
      const environment = await this.buildPdfEnvironment(
        request.routingIds.pdf,
        request.projectName,
        request.onPdfConnection
      )
      if (environment && this.options.pdf) {
        servers.push(
          createPdfMcpServerConfig({
            command: this.options.pdf.mcpCommand ?? process.execPath,
            entryPath: this.options.pdf.mcpEntryPath,
            ...environment
          })
        )
      }
    ...
    if (enabled.query) {
      const environment = await this.buildQueryEnvironment(
        request.routingIds.query,
        request.projectName,
        request.onQueryConnection
      )
      if (environment && this.options.query) {
        servers.push(
          createHostQueryMcpServerConfig({
            command: this.options.query.mcpCommand ?? process.execPath,
            entryPath: this.options.query.mcpEntryPath,
            ...environment
          })
        )
      }
  ...
  private async buildMemoryEnvironment(
    routingId: string,
    onConnection?: (connection: MemoryRpcConnection) => void
  ): Promise<MemoryMcpEnvironment | undefined> {
    if (!this.options.memory || !routingId) return undefined
    const connection = await this.options.memory.getRpcConnection({ sessionId: routingId })
    onConnection?.(connection)
    return { ...connection, sessionId: routingId }
  }
  ...
  private async buildRoutineEnvironment(
    routingId: string,
    onConnection?: (connection: RoutineRpcConnection) => void
  ): Promise<RoutineMcpEnvironment | undefined> {
    if (!this.options.routine || !routingId) return undefined
    const connection = await this.options.routine.getRpcConnection({ sessionId: routingId })
    onConnection?.(connection)
    return { ...connection, sessionId: routingId }
  }

  private async buildEndpointEnvironment(
    routingId: string,
    onConnection?: (connection: EndpointRpcConnection) => void
  ): Promise<EndpointMcpEnvironment | undefined> {
    if (!this.options.endpoints || !routingId) return undefined
    const connection = await this.options.endpoints.getRpcConnection({ sessionId: routingId })
    onConnection?.(connection)
    return { ...connection, sessionId: routingId }
  }
  ...
  private async buildPdfEnvironment(
    routingId: string,
    projectName: string,
    onConnection?: (connection: PdfRpcConnection) => void
  ): Promise<PdfMcpEnvironment | undefined> {
    if (!this.options.pdf || !routingId || !projectName) return undefined
    const connection = await this.options.pdf.getRpcConnection({
      sessionId: routingId,
      projectId: projectName
    })
    onConnection?.(connection)
  ...
  private async buildQueryEnvironment(
    routingId: string,
    projectName: string,
    onConnection?: (connection: HostQueryRpcConnection) => void
  ): Promise<HostQueryMcpEnvironment | undefined> {
    if (!this.options.query || !routingId || !projectName) return undefined
    const connection = await this.options.query.getRpcConnection({
      sessionId: routingId,
      projectId: projectName
    })
    onConnection?.(connection)
