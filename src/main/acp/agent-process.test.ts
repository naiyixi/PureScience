...
const originalDebugAgent = process.env.PURESCIENCE_DEBUG_AGENT

beforeEach(() => {
  vi.clearAllMocks()
  delete process.env.PURESCIENCE_DEBUG_AGENT
})

afterEach(() => {
  if (originalDebugAgent === undefined) delete process.env.PURESCIENCE_DEBUG_AGENT
  else process.env.PURESCIENCE_DEBUG_AGENT = originalDebugAgent
})
...
describe('ACP agent process packaging paths', () => {
  it('uses the real unpacked path for executables resolved inside app.asar', () => {
    expect(
      toUnpackedAsarPath(
        '/Applications/PureScience.app/Contents/Resources/app.asar/node_modules/@anthropic-ai/claude-agent-sdk-darwin-arm64/claude'
      )
    ).toBe(
      '/Applications/PureScience.app/Contents/Resources/app.asar.unpacked/node_modules/@anthropic-ai/claude-agent-sdk-darwin-arm64/claude'
    )
  })
  ...
  it('unpacks asar paths that use Windows separators', () => {
    ...
    expect(
      toUnpackedAsarPath(
        'C:\\Program Files\\PureScience\\resources\\app.asar\\node_modules\\@anthropic-ai\\claude-agent-sdk-win32-x64\\claude.exe'
      )
    ).toBe(
      'C:\\Program Files\\PureScience\\resources\\app.asar.unpacked\\node_modules\\@anthropic-ai\\claude-agent-sdk-win32-x64\\claude.exe'
    )
  })
...
describe('spawnClaudeAgentAcp', () => {
  ...
  it('enables SDK diagnostics only when explicitly requested', () => {
    process.env.PURESCIENCE_DEBUG_AGENT = '1'
