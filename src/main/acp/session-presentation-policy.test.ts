...
const TURN_CONTINUITY_APPEND = [
  '<purescience_turn_continuity_instructions>',
...
const LARGE_DATA_FILE_APPEND = [
  '<purescience_large_file_instructions>',
...
const ARTIFACT_FILE_APPEND = [
  '<purescience_artifact_instructions>',
  'When this turn creates or saves local user-facing files such as images, documents, reports, data exports, XML, SVG, HTML, CSV, PDF, or archives, you MUST save them through the MCP tool `write_artifact_file` from the `purescience-artifacts` server.',
...
describe('ACP Session presentation policy', () => {
  ...
  it('keeps unconditional guidance while omitting unavailable tooling and Skill privacy text', () => {
    ...
    expect(appends.join('\n\n')).not.toContain('<purescience_skill_privacy_instructions>')
  })
  ...
  it('returns exact immutable Session append and turn prefix text for a Specialist identity', () => {
    ...
    const append = [
      '[purescience:specialist-identity]',
