...
const createTemporaryRoot = async (): Promise<string> => {
  temporaryRoot = await mkdtemp(join(tmpdir(), 'purescience-runtime-artifacts-'))
...
describe('ACP runtime session management', () => {
  ...
  it('gives OpenCode stable underscore names for app-owned action MCPs on create and resume', async () => {
    ...
    for (const prompt of fakeAgent.prompts) {
      expect(prompt.text).not.toContain('`purescience_activity_begin_activity_group`')
      expect(prompt.text).toContain('`purescience_artifacts_write_artifact_file`')
      expect(prompt.text).toContain('`purescience_notebook_notebook_execute`')
      expect(prompt.text).toContain('purescience_skills_request_skill_import')
      expect(prompt.text).not.toContain('`purescience-')
    }
  })
  ...
  it('adds the hidden Plan mode context only to the requested turn and preserves user Messages', async () => {
    ...
    expect(planPrompt).toContain(
      'Do NOT run code without an approved plan. Always call `mcp__purescience-plan__generate_plan` first.'
    )
  ...
  it('adopts a fresh agent session under the same app id on a context reset', async () => {
    ...
    const initialEndpoint = getEnvValue(initialNotebookServer, 'PURESCIENCE_NOTEBOOK_RPC_ENDPOINT')
    const initialToken = getEnvValue(initialNotebookServer, 'PURESCIENCE_NOTEBOOK_RPC_TOKEN')
    ...
    const replacementToken = getEnvValue(
      ...
      'PURESCIENCE_NOTEBOOK_RPC_TOKEN'
    )
  ...
  it('gives opencode the stdio artifact MCP server + tool guidance (it accepts stdio like Claude)', async () => {
    ...
    expect(fakeAgent.prompts[0].text).not.toContain('<purescience_skill_privacy_instructions>')
  })
  ...
  it('delivers the large-data-file guidance to Claude session metadata on create and resume', async () => {
    ...
    expect(fakeAgent.newSessions[0]._meta).toMatchObject({
      systemPrompt: {
        preset: 'claude_code',
        append: expect.stringContaining('purescience_large_file_instructions')
      }
    })
  ...
  it('keeps backend-persistent Codex guidance out of the user prompt', async () => {
    ...
    expect(resolvedContext?.systemPromptAppends).toEqual(
      expect.arrayContaining([expect.stringContaining('purescience_large_file_instructions')])
    )
  ...
  it('serves app MCP tools over the http host for an http-only framework', async () => {
    ...
    try {
      ...
      const artifactServer = servers.find((server) => server.name === 'purescience_artifacts')
      ...
      const skillImportServer = servers.find((server) => server.name === 'purescience_skills')
  ...
  it('rejects filesystem callbacks for unknown sessions instead of falling back to global cwd', async () => {
    const workspaceRoot = await mkdtemp(join(tmpdir(), 'purescience-acp-runtime-'))
  ...
  it('audits OpenCode MCP calls with canonical identities without logging raw tool titles', async () => {
      ...
      .onRequest(acp.methods.agent.session.prompt, async (ctx) => {
        await ctx.client.notify(acp.methods.client.session.update, {
          ...
          update: {
            sessionUpdate: 'tool_call',
            toolCallId: 'tool-mcp',
            title: 'purescience_artifacts_write_artifact_file',
            kind: 'other',
            status: 'pending',
        ...
        await ctx.client.notify(acp.methods.client.session.update, {
          ...
          update: {
            ...
            kind: 'other',
            status: 'failed',
            _meta: { toolName: 'purescience_artifacts_write_artifact_file' }
          }
        })
        await ctx.client.request(acp.methods.client.session.requestPermission, {
          sessionId: 'remote-session-1',
          toolCall: {
            ...
            title: 'purescience_artifacts_delete_artifact',
            kind: 'other',
            status: 'pending',
            _meta: { toolName: 'purescience_artifacts_delete_artifact' }
          },
          options: [{ optionId: 'allow-once', name: 'Allow once', kind: 'allow_once' }]
        })
    ...
    // Wire the artifact MCP server so the session records its name (purescience-artifacts); MCP
    ...
    const runtime = new AcpRuntime({
      ...
      callbacks: {
        onPermissionRequest: (request) => {
          if (request.title === 'purescience_artifacts_delete_artifact') {
    ...
    expect(dataFor('tool-mcp').tool).toBe('purescience-artifacts/write_artifact_file')
    ...
    expect(failureAudit?.[1]).toMatchObject({
      tool: 'purescience-artifacts/write_artifact_file',
      toolCallId: 'tool-failed'
    })
    ...
    expect(errorAudit?.[1]).toMatchObject({
      tool: 'purescience-artifacts/delete_artifact',
      toolCallId: 'tool-error'
    })
    ...
    for (const [, data] of [
      ...
      expect(serialized).not.toContain('purescience_artifacts_write_artifact_file')
    }
  })
    ...
    const runtime = new AcpRuntime({
      ...
      callbacks: {
        onPermissionRequest: (request) => {
          ...
          if (!sessionOptionId) throw new Error('Missing PureScience session permission option')
    ...
    const runtime = new AcpRuntime({
      ...
      callbacks: {
        onPermissionRequest: (request) => {
          ...
          if (!sessionOptionId) {
            throw new Error('Expected PureScience to provide a conversation permission option')
          }
          runtime.respondToPermission({
            requestId: request.requestId,
            optionId: sessionOptionId
  ...
  it('maps an OpenCode underscore MCP permission to a canonical notebook grant', async () => {
    ...
    startPermissionProbeAgent(process, {
      ...
      toolTitle: 'purescience_notebook_notebook_execute',
  ...
  it.each([
    { name: 'without the Codex MCP marker', codexMcpMarker: false },
    {
      ...
      codexMcpTitle: 'mcp.purescience-artifacts.unrelated_tool'
    }
  ...
  it('auto-allows a Claude Code Artifact save identified by its qualified MCP title', async () => {
    ...
    startPermissionProbeAgent(process, {
      ...
      toolTitle: 'mcp__purescience-artifacts__write_artifact_file',
      toolKind: 'other',
  ...
  it('strips Codex policy amendments using the Session framework', async () => {
    ...
    expect(permissionRequests[0].options.map((option) => option.optionId)).toEqual([
      'allow-once',
      'decline',
      expect.stringContaining('purescience:allow-session:')
    ])
    ...
    const runtime = new AcpRuntime({
      ...
      callbacks: {
        onPermissionRequest: (request) => {
          ...
          if (!sessionOptionId) throw new Error('Missing PureScience conversation option')
          runtime.respondToPermission({ requestId: request.requestId, optionId: sessionOptionId })
        }
      }
    })
  ...
  it('records MCP server names on resume so a resumed session audits its MCP tool calls as MCP', async () => {
    ...
    startPermissionProbeAgent(process, {
      ...
      toolTitle: 'purescience-artifacts_write_artifact_file',
      resume: 'ok'
    })
    const root = await createTemporaryRoot()
    const runtime = new AcpRuntime({
    ...
    expect(mcpServerNamesFor(runtime, 'resumed-session')).toEqual(['purescience-artifacts'])
  })
  ...
  it('records MCP server names when adopting a fresh session after an unresumable resume', async () => {
    ...
    expect(mcpServerNamesFor(runtime, 'switched-session')).toEqual(['purescience-artifacts'])
  })
  ...
  it('returns an explicit reviewer role while preserving MCP audit routing', async () => {
    ...
    startPermissionProbeAgent(process, {
      ...
      providerToolName: 'purescience-reviewer_submit_findings'
    })
    const runtime = new AcpRuntime({
      appVersion: '0.1.0',
      defaultCwd: '/workspace',
    ...
    expect(reviewerOwnerProbe(runtime).contextFor('reviewer-session-1')).toEqual({
      frameworkId: 'claude-code',
      mcpServerNames: ['purescience-reviewer'],
      role: 'reviewer'
    })
  ...
  it.each([null, 'read'] as const)(
    ...
    async (toolKind) => {
      ...
      startPermissionProbeAgent(process, {
        ...
        toolKind,
        providerToolName: 'mcp__purescience-reviewer__read_turn',
        permissionOptions: [
          { optionId: 'allow-once', name: 'Allow once', kind: 'allow_once' },
          { optionId: 'reject-once', name: 'Reject', kind: 'reject_once' }
        ],
  ...
  it('auto-approves an exact opencode reviewer tool title when provider metadata is absent', async () => {
    ...
    startPermissionProbeAgent(process, {
      ...
      toolTitle: 'purescience-reviewer_read_turn',
      permissionOptions: [
        { optionId: 'allow-once', name: 'Allow once', kind: 'allow_once' },
        { optionId: 'reject-once', name: 'Reject', kind: 'reject_once' }
      ],
  ...
  it('auto-approves a claude-code reviewer MCP tool identified by its sanitized title', async () => {
    ...
    startPermissionProbeAgent(process, {
      newSessionId: 'reviewer-session-1',
      toolCallId: 'mcp__purescience_reviewer__read_turn_0',
      toolTitle: 'mcp__purescience_reviewer__read_turn',
      permissionOptions: [
        { optionId: 'allow-once', name: 'Allow once', kind: 'allow_once' },
        { optionId: 'reject-once', name: 'Reject', kind: 'reject_once' }
      ],
    ...
    const { session } = await runtime.buildReviewerSession({
      cwd: '/workspace',
      mcpServers: [
        { type: 'http', name: 'purescience-reviewer', url: 'http://127.0.0.1:1/mcp', headers: [] }
      ]
    })
    await session.prompt([{ type: 'text', text: 'read the audited turn' }])
  ...
  it('auto-approves a claude-code reviewer MCP tool identified by its hyphenated title', async () => {
    ...
    startPermissionProbeAgent(process, {
      ...
      toolTitle: 'mcp__purescience-reviewer__read_turn',
      permissionOptions: [
        { optionId: 'allow-once', name: 'Allow once', kind: 'allow_once' },
        { optionId: 'reject-once', name: 'Reject', kind: 'reject_once' }
      ],
  ...
  it('refuses a non-loopback reviewer MCP before starting an agent connection', async () => {
    ...
    ).rejects.toThrow(/app-owned purescience-reviewer/)
  ...
  it('accepts the app-owned Reviewer stdio proxy used by Windows named pipes', async () => {
    ...
    startPermissionProbeAgent(process, {
      ...
      toolTitle: 'Submit review checks',
      providerToolName: 'mcp__purescience-reviewer__submit_findings'
    })
    const runtime = new AcpRuntime({
      appVersion: '0.1.0',
      defaultCwd: '/workspace',
      ...
      mcpServers: [
        {
          ...
          args: ['/app/main.js', '--purescience-reviewer-mcp-proxy'],
          env: [
            {
              name: 'PURESCIENCE_REVIEWER_MCP_SOCKET_PATH',
              value: '\\\\.\\pipe\\purescience-reviewer'
            },
            { name: 'PURESCIENCE_REVIEWER_MCP_TOKEN', value: 'reviewer-token' }
          ]
        }
      ]
    })
  ...
  it('reserves a reviewer session id while its permission mode is still starting', async () => {
    ...
    try {
      ...
      expect(duplicateCwd).toMatch(/purescience-reviewer-/)
  ...
  it('rejects a reviewer session id that collides with a primary session before authority setup', async () => {
    ...
    expect(reviewerCwd).toMatch(/purescience-reviewer-/)
    await expect(stat(reviewerCwd!)).rejects.toMatchObject({ code: 'ENOENT' })
    expect(fakeAgent.modeChanges).toEqual([])
  ...
  it('rejects tools from every MCP namespace except the dedicated reviewer server', async () => {
    ...
    // its namespace does not exactly match purescience-reviewer.
  ...
  it('rejects unknown tools inside the reviewer MCP namespace', async () => {
    ...
    startPermissionProbeAgent(process, {
      ...
      toolTitle: 'mcp__purescience-reviewer__run_shell',
      providerToolName: 'mcp__purescience-reviewer__run_shell',
      permissionOptions: [
        { optionId: 'allow-once', name: 'Allow once', kind: 'allow_once' },
        { optionId: 'reject-once', name: 'Reject', kind: 'reject_once' }
      ],
  ...
  it('clears a session MCP server names when the session is deleted', async () => {
    ...
    const session = await runtime.createSession({ cwd: '/workspace' })
    expect(mcpServerNamesFor(runtime, session.sessionId)).toEqual(['purescience-artifacts'])
    ...
    expect(fakeAgent.resumedSessions).toEqual([
      {
        ...
        _meta: {
          ...
          systemPrompt: {
            type: 'preset',
            preset: 'claude_code',
            append: expect.not.stringContaining('purescience_skill_privacy_instructions')
          }
        }
      }
    ])
  ...
  it('prepends a history preamble to the agent content but not the user-facing message', async () => {
    ...
    expect(fakeAgent.prompts[0]?.text).toContain('<purescience_notebook_continuity>')
    ...
    expect(fakeAgent.prompts[1]?.text).not.toContain('<purescience_notebook_continuity>')
  })
  ...
  it('omits the retired activity schema from bridge-backed Codex context estimates', async () => {
    ...
    const counter: TokenCounter = {
      count: (text) => {
        if (text.includes('mcp__purescience_activity__begin_activity_group')) return 101
        if (text.includes('mcp.purescience-activity.begin_activity_group')) return 17
        return 0
      }
    }
    const runtime = new AcpRuntime({
  ...
  it('removes a prompt-scoped Codex Skill estimate when the next turn omits it', async () => {
    temporaryRoot = await mkdtemp(join(tmpdir(), 'purescience-context-skill-'))
  ...
  it.each([
    ...
    async (_name, framework) => {
      ...
      expect(fakeAgent.prompts[0]?.text).toContain('<purescience_protected_plan_context>')
  ...
  it('lets an unrelated ordinary message finish without resuming an approved Plan', async () => {
    ...
    expect(fakeAgent.prompts[0]?.text).not.toContain('<purescience_protected_plan_context>')
  })
  ...
  it('excludes autonomous Claude result lanes from the user prompt model-turn count', async () => {
    ...
    startFakeAgent(process, ['s1'], {
      claudeResultMessagesForPrompt: () => [
        ...
        // silently under-report model turns until PureScience knows its name.
    ...
    startFakeAgent(process, ['s1'], {
      ...
      onPrompt: () => ({
        ...
        _meta: {
          ...
          'purescience/model-turn-count': 2
        }
      })
    })
    const runtime = new AcpRuntime({
  ...
  it('keeps Codex bridge usage visible when the adapter omits whole-turn metadata', async () => {
    ...
    startFakeAgent(process, ['s1'], {
      ...
      onPrompt: () => ({
        ...
        // PureScience's private whole-turn metadata. The footer must not become entirely unavailable.
        usage: {
          totalTokens: 27,
          inputTokens: 19,
          cachedReadTokens: 5,
  ...
  it('passes the artifact MCP server to new and resumed sessions', async () => {
    ...
    const runtime = new AcpRuntime({
      ...
      spawnAgent: () => asAgentProcess(process),
      artifacts: {
        configRoot: '/Users/example/.purescience',
        dataRoot: '/Users/example/.purescience',
        projectName: 'default-project',
        mcpEntryPath: '/app/out/main/index.js',
        mcpCommand: '/Applications/PureScience.app/Contents/MacOS/PureScience'
      }
    })
    ...
    expect(fakeAgent.newSessions[0].mcpServers[0]).toMatchObject({
      name: 'purescience-artifacts',
      command: '/Applications/PureScience.app/Contents/MacOS/PureScience',
      args: ['/app/out/main/index.js', '--purescience-artifact-mcp']
    })
    expect(
      getEnvValue(fakeAgent.newSessions[0].mcpServers[0], 'PURESCIENCE_ARTIFACT_SESSION_ID')
    ).toMatch(/^artifact-session-/)
    expect(fakeAgent.resumedSessions[0].mcpServers).toHaveLength(1)
    expect(
      getEnvValue(fakeAgent.resumedSessions[0].mcpServers[0], 'PURESCIENCE_ARTIFACT_SESSION_ID')
  ...
  it('scopes the artifact MCP project to a caller-supplied projectName on create and resume', async () => {
    ...
    expect(
      getEnvValue(fakeAgent.newSessions[0].mcpServers[0], 'PURESCIENCE_ARTIFACT_PROJECT_NAME')
    ).toBe('project-abc')
    expect(
      getEnvValue(fakeAgent.resumedSessions[0].mcpServers[0], 'PURESCIENCE_ARTIFACT_PROJECT_NAME')
    ).toBe('project-xyz')
  })
  ...
  it('passes notebook MCP server and scoped notebook instructions to new and resumed sessions', async () => {
    ...
    expect(
      getEnvValue(fakeAgent.newSessions[0].mcpServers[0], 'PURESCIENCE_NOTEBOOK_SESSION_ID')
    ).toMatch(/^notebook-session-/)
    expect(
      getEnvValue(fakeAgent.newSessions[0].mcpServers[0], 'PURESCIENCE_NOTEBOOK_WORKSPACE_CWD')
    ).toBe(resolve('/workspace'))
    expect(aliases).toEqual([
      {
        aliasSessionId: getEnvValue(
          fakeAgent.newSessions[0].mcpServers[0],
          'PURESCIENCE_NOTEBOOK_SESSION_ID'
        ),
        sessionId: 'remote-session-1'
      }
    ])
    ...
    expect(fakeAgent.resumedSessions[0].mcpServers).toHaveLength(1)
    expect(
      getEnvValue(fakeAgent.resumedSessions[0].mcpServers[0], 'PURESCIENCE_NOTEBOOK_SESSION_ID')
    ).toBe('remote-session-2')
    expect(fakeAgent.newSessions[0]._meta).toMatchObject({
      systemPrompt: {
        type: 'preset',
  ...
  it('passes the conversation Skill import tool a session-scoped route', async () => {
    ...
    expect(fakeAgent.newSessions[0].mcpServers).toHaveLength(1)
    expect(fakeAgent.newSessions[0].mcpServers[0]).toMatchObject({
      name: 'purescience-skills',
      command: '/Applications/PureScience.app/Contents/MacOS/PureScience',
      args: ['/app/out/main/index.js', '--purescience-skill-import-mcp']
    })
    const aliasSessionId = getEnvValue(
      fakeAgent.newSessions[0].mcpServers[0],
      'PURESCIENCE_SKILL_IMPORT_SESSION_ID'
    )
    ...
    expect(JSON.stringify(fakeAgent.newSessions[0]._meta)).toContain(
      'mcp__purescience-skills__request_skill_import'
    )
  })
  ...
  it('passes only the workspace as a static allowed import root, not the pre-start notebook alias', async () => {
    ...
    const notebookSessionId = getEnvValue(notebookServer, 'PURESCIENCE_NOTEBOOK_SESSION_ID')
    ...
    const staticRoots = JSON.parse(
      getEnvValue(artifactServer, 'PURESCIENCE_ARTIFACT_ALLOWED_IMPORT_ROOTS')
    )
    expect(staticRoots).toEqual([resolve('/workspace')])
    expect(staticRoots).not.toContain(
      join('/Users/example/.purescience', 'notebooks', 'default-project', notebookSessionId)
    )
  })
  ...
  it.each([
    ...
    async ({ promptError }) => {
      ...
      currentRunFile = getEnvValue(
        fakeAgent.newSessions[0].mcpServers[0],
        'PURESCIENCE_ARTIFACT_CURRENT_RUN_FILE'
      )
    ...
    const fakeAgent = startFakeAgent(process, ['remote-session-1'], {
      onPrompt: async ({ sessionId }) => {
        ...
        await repository.writePendingFile({
          ...
          sessionId: getEnvValue(
            fakeAgent.newSessions[0].mcpServers[0],
            'PURESCIENCE_ARTIFACT_SESSION_ID'
          ),
    ...
    expect(receivedPrompt).toEqual([
      {
        ...
        text: 'summarize the paper',
        _meta: {
          'purescience/skill-inputs': [{ name: 'research', path: skillPath }]
        }
      }
    ])
    ...
    expect(receivedPrompt).toEqual([
      {
        ...
        _meta: {
          'purescience/skill-inputs': [{ name: 'mcp-pubmed', path: skillPath }]
        }
      }
    ])
  ...
  it('hot-switches OpenCode across pre-registered provider transports', async () => {
    ...
    const configOptions = [
      {
        ...
        name: 'Model',
        category: 'model',
        currentValue: 'purescience-a/model-a',
        options: ['purescience-a/model-a', 'purescience-b/model-b'].map((value) => ({
          value,
          name: value
        }))
      } as SessionConfigOption
    ...
    const runtime = new AcpRuntime({
      ...
      resolveBackend: () => ({
        ...
        executablePath: '/bin/opencode',
        env: {},
        sessionModel: 'purescience-a/model-a',
        supportsImageInput: false,
        contextUsageModel: 'model-a',
    ...
    await runtime.applyModelChange({
      ...
      model: 'model-b',
      sessionModel: 'purescience-b/model-b',
      sessionModelRequired: false,
      supportsImageInput: false,
      reasoningEffort: 'default',
    ...
    expect(receivedPrompt).toEqual([
      {
        ...
        _meta: {
          'purescience/skill-inputs': [newConnector]
        }
      }
    ])
  })
