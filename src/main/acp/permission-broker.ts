...
const SESSION_ALLOW_OPTION_ID_PREFIX = 'purescience:allow-session:'
const PROJECT_ALLOW_OPTION_ID_PREFIX = 'purescience:allow-project:'
...
// PureScience owns per-session grants, so Codex approvals omit options that grant persistent
...
class AcpPermissionBroker {
  ...
  async respond(response: AcpPermissionResponse): Promise<boolean> {
    ...
    // Legacy Session grants are owned by PureScience. The Agent receives only its one-shot option.
  ...
  // Records the category when the user picks PureScience's synthetic session scope.
