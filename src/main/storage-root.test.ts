...
describe('dataFolderName', () => {
  ...
  it('is PureScience when packaged', () => {
    appMock.isPackaged = true
    expect(dataFolderName()).toBe('PureScience')
  })

  it('is PureScience-DEV in dev (not packaged)', () => {
    appMock.isPackaged = false
    expect(dataFolderName()).toBe('PureScience-DEV')
  })
})
...
describe('dataRootForParent', () => {
  it('joins the parent with the data folder name', () => {
    appMock.isPackaged = true
    expect(dataRootForParent('/mnt/data')).toBe(join('/mnt/data', 'PureScience'))
  })
})
...
describe('dataRootForPicked', () => {
  ...
  it('is case-sensitive off Windows (a differently-cased folder is not the data folder)', () => {
    ...
    appMock.isPackaged = true
    const lower = '/mnt/data/purescience'
    expect(dataRootForPicked(lower)).toBe(join(resolve(lower), 'PureScience'))
  })
})
...
describe('samePath / isPathInsideOrEqual (platform-aware)', () => {
  ...
  it('compares case-insensitively on win32 (NTFS is case-insensitive)', () => {
    setPlatform('win32')
    expect(samePath(p('Data', 'PureScience'), p('data', 'purescience'))).toBe(true)
    expect(isPathInsideOrEqual(p('Data'), p('data', 'PureScience'))).toBe(true)
  })
...
describe('computeDefaultDataRoot', () => {
  ...
  it('defaults to <home>/PureScience for a fresh config root', () => {
    // resolveConfigRoot() resolves under homeDir but nothing has been created there.
    expect(computeDefaultDataRoot()).toBe(join(homeDir, 'PureScience'))
  })

  it('keeps packaged E2E config and data under the disposable certification root', () => {
    const e2eRoot = join(homeDir, 'certification')
    vi.stubEnv('PURESCIENCE_E2E_STORAGE_ROOT', e2eRoot)

    expect(resolveConfigRoot()).toBe(e2eRoot)
    expect(computeDefaultDataRoot()).toBe(join(e2eRoot, 'PureScience'))
  })

  it('stays at the config root when it already has legacy data and no PureScience subdir', async () => {
  ...
  it('does not treat a config root with an PureScience subdir as legacy', async () => {
    const configRoot = resolveConfigRoot()
    await mkdir(join(configRoot, 'artifacts'), { recursive: true })
    await mkdir(join(configRoot, 'PureScience'), { recursive: true })
  ...
  it('stays at the legacy config root when <home>/PureScience exists but carries a migration marker', async () => {
    ...
    const configRoot = resolveConfigRoot()
    await mkdir(join(configRoot, 'artifacts'), { recursive: true })
    const homeDefault = join(homeDir, 'PureScience')
  ...
  it('does not treat a markerless partial <home>/PureScience copy as committed', async () => {
    const configRoot = resolveConfigRoot()
    await mkdir(join(configRoot, 'artifacts'), { recursive: true })
    await mkdir(join(homeDir, 'PureScience', 'artifacts'), { recursive: true })
  ...
  it('treats an explicitly configured <home>/PureScience as the committed default', async () => {
    const configRoot = resolveConfigRoot()
    const homeDefault = join(homeDir, 'PureScience')
    await mkdir(join(configRoot, 'artifacts'), { recursive: true })
...
describe('computeDefaultDataRoot (dev mode)', () => {
  ...
  it('stays at the (dev) config root when it already has legacy data and no PureScience-DEV subdir', async () => {
    const configRoot = resolveConfigRoot()
    await mkdir(join(configRoot, 'artifacts'), { recursive: true })
  ...
  it('defaults to <home>/PureScience-DEV for a fresh (dev) config root', () => {
    // resolveConfigRoot() resolves under homeDir but nothing has been created there.
    expect(computeDefaultDataRoot()).toBe(join(homeDir, 'PureScience-DEV'))
  })

  it('prefers an explicitly configured <home>/PureScience-DEV over legacy data', async () => {
    ...
    const configRoot = resolveConfigRoot()
    await mkdir(join(configRoot, 'artifacts'), { recursive: true })
    const homeDefault = join(homeDir, 'PureScience-DEV')
    await mkdir(join(homeDefault, 'artifacts'), { recursive: true })
    initDataRoot(homeDefault)
...
describe('resolveDataRoot / initDataRoot', () => {
  ...
  it('prefers an explicit settings.dataRoot over the computed default', () => {
    initDataRoot('/mnt/data/purescience')
    expect(resolveDataRoot()).toBe('/mnt/data/purescience')
  })
...
describe('resolveStorageRoot', () => {
  ...
  it('uses the normal development directory by default', () => {
    expect(resolveStorageRoot()).toBe(join('/Users/tester', '.purescience-project'))
  })

  it('uses an absolute development preview override without changing HOME', () => {
    vi.stubEnv('PURESCIENCE_STORAGE_ROOT', '/tmp/purescience-preview/storage')

    expect(resolveStorageRoot()).toBe(normalize('/tmp/purescience-preview/storage'))
    expect(appMock.getPath).not.toHaveBeenCalled()
  })

  it('uses an absolute disposable E2E root in packaged builds', () => {
    appMock.isPackaged = true
    vi.stubEnv('PURESCIENCE_E2E_STORAGE_ROOT', '/tmp/purescience-certification/storage')

    expect(resolveStorageRoot()).toBe(normalize('/tmp/purescience-certification/storage'))
    expect(appMock.getPath).not.toHaveBeenCalled()
  })

  it('rejects an ambiguous relative E2E root', () => {
    vi.stubEnv('PURESCIENCE_E2E_STORAGE_ROOT', 'certification/storage')

    expect(() => resolveStorageRoot()).toThrow(
      'PURESCIENCE_E2E_STORAGE_ROOT must be an absolute path'
    )
  })

  it('rejects an ambiguous relative preview override', () => {
    vi.stubEnv('PURESCIENCE_STORAGE_ROOT', 'preview/storage')
  ...
  it('ignores the preview override in packaged builds', () => {
    appMock.isPackaged = true
    vi.stubEnv('PURESCIENCE_STORAGE_ROOT', '/tmp/ignored')

    expect(resolveStorageRoot()).toBe(join('/Users/tester', '.purescience'))
  })
})
