...
describe('ReviewerMcpServer named-pipe proxy', () => {
  it('lists and calls the existing scoped tools through stdio without loopback TCP', async () => {
    ...
    expect(config.args).toEqual(['C:\\app\\main.js', '--purescience-reviewer-mcp-proxy'])

    const proxy = await createReviewerMcpStdioProxy({
      socketPath: environment.PURESCIENCE_REVIEWER_MCP_SOCKET_PATH!,
      token: environment.PURESCIENCE_REVIEWER_MCP_TOKEN!
    })
