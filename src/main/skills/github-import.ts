...
const GITHUB_HEADERS = { 'User-Agent': 'purescience', Accept: 'application/vnd.github+json' }
...
const fetchSkillPreview = async (
  ...
  const walk = async (path: string, depth: number): Promise<void> => {
    ...
    for (const entry of entries) {
      ...
      const raw = await request(entry.download_url, { headers: { 'User-Agent': 'purescience' } })
