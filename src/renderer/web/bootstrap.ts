...
import openScienceLogoSvg from '../../main/remote-access/purescience-logo.svg?raw'
...
const REMOTE_ACCESS_OFF_MESSAGE =
  'Remote access is off on the home computer. Re-enable a remote access mode in PureScience, then try again.'
...
const clientId = sessionStorage.getItem('purescience-web-client') ?? crypto.randomUUID()
sessionStorage.setItem('purescience-web-client', clientId)

const listeners = new Map<string, Set<Listener>>()

const connectionMessage = (): HTMLElement | null =>
  document.getElementById('purescience-connection-message')
...
const connectionLogo = document.getElementById('purescience-connection-logo')
if (connectionLogo) {
...
const fetchBootstrap = async (): Promise<unknown> => {
    ...
    try {
      ...
      if (!response.ok) {
        throw responseError(
          response,
          await response.text(),
          `PureScience returned HTTP ${response.status}.`
        )
      }
  ...
  throw lastError instanceof Error
    ? lastError
    : new Error('Unable to initialize PureScience Remote.')
}

const showConnectionFailure = (error: unknown): void => {
  const state = document.getElementById('purescience-connection-state')
  ...
  state.querySelector('.purescience-connection-panel')?.append(retry)
}
...
const invoke = async (channel: string, args: unknown[]): Promise<unknown> => {
  ...
  try {
    ...
    throw new Error(
      'PureScience returned an invalid response. Try reconnecting to the remote computer.'
    )
  }
...
const installWebApi = async (): Promise<void> => {
  ...
  if (!parsedBootstrap.success) {
    throw new Error(
      `Incompatible PureScience Web RPC protocol. Expected version ${WEB_RPC_PROTOCOL_VERSION}.`
    )
  }
