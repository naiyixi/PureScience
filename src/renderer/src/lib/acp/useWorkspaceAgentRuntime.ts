...
type SendWorkspaceMessageInput = {
  ...
  // Branch from a specific completed Agent message instead of the session head; the new session
  // carries the transcript up to and including that message and stays idle (no prompt is sent).
...
const startPendingSessionPrompt = (
  ...
  void (async () => {
    ...
    if (!bound || !bound.messageId) return
    const boundMessageId = bound.messageId
    if (!sessionOwnsActivePrompt(runtimeSessionId, boundMessageId)) return
...
const sendWorkspaceMessage = async (
  ...
  if (branchSourceSessionId) {
    ...
    const pendingResult: SendWorkspaceMessageResult = {
    ...
    const pendingMessageId = pending.messageId
    ...
    try {
      ...
      if (!sessionOwnsActivePrompt(pending.sessionId, pendingMessageId)) return pendingResult
...
const recoverContextOverflowWorkspaceSession = async (
  ...
  const retried = await sendWorkspaceMessage(retryRuntime, {
    ...
    // PureScience to replay the prior transcript into its first prompt.
