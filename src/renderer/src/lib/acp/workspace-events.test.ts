...
describe('workspace runtime events', () => {
  ...
  it('restores image data from the existing runtime raw projection and removes its sentinel', async () => {
    await applyWorkspaceRuntimeEvent(
      createEvent({
        ...
        role: 'assistant',
        messageId: 'assistant-message-1',
        text: '[purescience:acp-message-image]',
        raw: {
          update: {
            content: {
              type: 'image',
  ...
  it('auto-opens a generated molecule artifact in the preview panel', async () => {
    const finalizedArtifact = createArtifactFile({
      ...
      runId: undefined,
      name: 'aspirin.mol',
      path: '/Users/example/.purescience/artifacts/default-project/transport-session-1/message-1/aspirin.mol',
      fileUrl:
        'file:///Users/example/.purescience/artifacts/default-project/transport-session-1/message-1/aspirin.mol'
    })
    const finalizeRunArtifacts = vi.fn().mockResolvedValue([finalizedArtifact])
    const saveSession = vi.fn().mockResolvedValue(undefined)
