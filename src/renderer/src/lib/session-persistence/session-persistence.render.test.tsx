...
describe('session persistence startup', () => {
  ...
  beforeEach(() => {
    ...
    loadAll = vi
      ...
      .mockRejectedValueOnce(
        new Error('EACCES: /Users/private/.purescience/sessions could not be read')
      )
  ...
  it('surfaces a save failure and retries the latest in-memory session', async () => {
    ...
    saveSession.mockImplementation(async (session) => {
      if (writesFail) {
        throw new Error(
          'ENOENT: could not write /Users/private/.purescience/sessions/project-a/session-1.json'
        )
      }
      return session
    })
