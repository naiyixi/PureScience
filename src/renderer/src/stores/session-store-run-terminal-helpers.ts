...
export const projectAttachContextWindowSample = (
  session: ChatSession,
  promptMessageId: string,
  ...
  const messages = session.messages.map((message) => {
    ...
    const existing = message.contextWindowSamples ?? []
    if (existing.some((entry) => entry.id === sample.id)) return message
    return { ...message, contextWindowSamples: [...existing, sample] }
  })
