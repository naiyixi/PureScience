...
import {
  ...
  projectFailedRun,
  projectFinishedRun,
  projectAttachContextWindowSample
...
>(
  ...
  return {
    ...
    attachContextWindowSample: (sessionId, promptMessageId, sample) => {
      setSessionState((state) => ({
        sessions: projectSession(state.sessions, sessionId, (session) =>
          projectAttachContextWindowSample(session, promptMessageId, sample)
        )
      }))
    },
