...
const ProviderStep = ({
  ...
  const handleSaveProvider = async (): Promise<void> => {
    ...
    try {
      ...
      if (validation.applied === false) {
        setValidationOk(false)
        setValidationMessage(t('onboarding.providerChangedDuringTesting'))
        return
      }
  ...
  return (
    <>
      <CardHeader className="gap-1 rounded-t-lg px-6 py-5">
        <CardTitle className="text-[15px] font-semibold">{t('onboarding.connectModel')}</CardTitle>
        ...
        <section aria-label={t('ui.configuremodel')}>
          ...
          {formValue.type === 'claude-isolated' ? (
            <p className="mt-4 text-sm text-muted-foreground">
              {t('onboarding.signInBrowserClaude')}
