// Single source of truth for identifying the PureScience notebook MCP server's kernel-run tools.
...
// purescience_notebook; we normalize `_`→`-` before the exact comparison so both forms match.
const NOTEBOOK_SERVER_SEGMENT = 'purescience-notebook'
...
// name (purescience_notebook) and must not split. The segment immediately before the suffix must
// equal the notebook server exactly, so a lookalike (purescience-notebook-staging) or an unrelated
...
const matchNotebookTool = (
  ...
  for (const suffix of suffixes) {
    if (
      ...
      name === `purescience_notebook_${suffix}`
    ) {
      return suffix
    }
  }
