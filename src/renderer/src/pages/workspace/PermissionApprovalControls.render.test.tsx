...
const replRequest: AcpPermissionRequest = {
  ...
  title: 'mcp__purescience-notebook__repl_execute',
  providerToolName: 'mcp__purescience-notebook__repl_execute',
  isMcp: true,
  mcpIdentity: 'purescience-notebook/repl_execute',
...
const bashExecuteRequest: AcpPermissionRequest = {
  ...
  title: 'mcp__purescience-notebook__bash_execute',
  providerToolName: 'mcp__purescience-notebook__bash_execute',
  isMcp: true,
  mcpIdentity: 'purescience-notebook/bash_execute',
  rawInput: { command: 'ls' },
  options: [{ optionId: 'allow-once', name: 'Allow once', kind: 'allow_once' }]
}
...
describe('PermissionApprovalControls', () => {
    ...
    const html = renderToStaticMarkup(
      <PermissionApprovalControls
        requests={[
          {
            ...permissionRequest,
            title: 'mcp.purescience-artifacts.write_artifact_file',
  ...
  it('keeps an otherwise-opaque MCP request distinguishable without its protocol identity', () => {
    ...
    expect(html).not.toContain('mcp.purescience-artifacts')
  })

  it('does not treat an unclassified raw MCP title as an artifact save', () => {
    const rawTitle = 'mcp__purescience-artifacts__write_artifact_file'
    const html = renderToStaticMarkup(
      <PermissionApprovalControls
        requests={[
          {
  ...
  it('keeps an MCP request external while showing its humanized action and paths', () => {
    ...
    expect(html).toContain('Artifact save</span>')
    expect(html).not.toContain('PureScience Artifacts / Write Artifact File')
  ...
  it('does not classify an untrusted notebook-looking name as notebook control', () => {
    const identifier = 'purescience_notebook_notebook_restart'
  ...
  it('keeps trusted notebook control details in an impact tooltip without exposing its identifier', () => {
    const identifier = 'mcp__purescience-notebook__notebook_restart'
    const restart: AcpPermissionRequest = {
      requestId: 'restart-1',
      sessionId: 'session-1',
      toolCallId: 'tool-restart',
