...
const WorkspacePage = ({
    ...
    (direction: 'previous' | 'next'): boolean => {
      ...
      if (!navigation) {
        ...
        if (direction === 'next' && cursor === 0) {
          ...
          setHistoryBrowsingKey(undefined)
          setHistoryStatus(t('ws.draftRestored'))
          return true
        }
  ...
  const compactContextDisabledReason = !activeSessionSupportsNativeCompaction
    ? t('ws.reconnectBeforeCompacting')
    : activeSession?.status === 'error'
      ? t('ws.resolveErrorBeforeCompacting')
      : t('ws.waitForActivityBeforeCompacting')
  ...
  useEffect(() => {
    ...
    void window.api.notebook
      ...
      .catch((error) => {
        console.warn(t('ws.notebookHydrationFailed'), error)
      })
  ...
  const stageAttachmentFiles = (files: File[]): void => {
    ...
    if (files.some((file) => file.type.startsWith('image/')) && supportsImageInput !== true) {
      setAttachmentError(t('ws.modelNotConfiguredForImage'))
      return
    }
      ...
      for (const { file, transfer } of pending) {
        ...
        try {
          ...
          await window.api.uploads
            ...
            .catch((error) => console.warn(t('ws.failedToClaimStagedUpload'), error))
        } catch (uploadError) {
          if (controller.signal.aborted) {
            updateDraftTransfers(draftKey, (transfers) =>
  ...
  const respondToRestoredPlan = useCallback(
    async (
      ...
      if (!session || session.activeRun || plan?.approval !== 'pending') {
        throw new Error(t('ws.planNoLongerAvailable'))
      }
      const pendingAction =
        ...
        .then((result) => {
          ...
          if (wasNewConversation && draftEnabledComputeHosts.length > 0) {
            ...
            void window.api.compute
              ...
              .catch((err: unknown) => {
                console.warn(t('ws.syncDraftComputeHostsFailed'), err)
              })
          }
          setNewConversationAutoReviewEnabled(false)
          setNewConversationEnabledComputeHosts([])
  ...
  // Branches from a completed Agent message: the new session copies the transcript through that
  // message and stays idle (no prompt is sent) — the user's next instruction starts the run.
  const branchFromAgentMessage = (messageId: string): void => {
      ...
      <div className="relative flex h-full">
        {isMobileSidebarOpen ? (
          <button
            ...
            aria-label={t('ws.closeNavigation')}
        ...
        >
          {!isMobile ? (
            <>
              ...
              <ResizableHandle
                ...
                aria-label={t('ws.resizeLeftPanel')}
          ...
          <ConversationPanel
            ...
            canBranchInNewSession={canSendMessage && activeSession?.status === 'idle'}
            onBranchFromAgentMessage={branchFromAgentMessage}
          ...
          {!isMobile ? (
            <>
              <ResizableHandle
                ...
                aria-label={t('ws.resizeRightPanel')}
