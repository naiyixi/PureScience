...
describe('PreviewFileContent', () => {
    ...
    window.api = {
      previewResources: {
        acquire: vi.fn().mockResolvedValue({
          id: 'resource-1',
          url: 'purescience-preview://resource-1/report.html',
          size: 88,
  ...
  it('renders HTML from a managed stream inside a script sandbox', async () => {
    ...
    expect(iframe?.getAttribute('src')).toBe('purescience-preview://resource-1/report.html')
    ...
    await act(async () => {
      window.dispatchEvent(
        new MessageEvent('message', {
          data: 'purescience-preview-load-error',
  ...
  it('reads upload-sourced text previews through the uploads API', async () => {
    ...
    await renderFile(
      createFileItem({
        ...
        format: 'text',
        name: 'notes.txt',
        path: '/Users/example/.purescience/uploads/default-project/session-1/notes.txt'
      })
    )
