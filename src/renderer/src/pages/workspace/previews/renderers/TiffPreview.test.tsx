...
describe('TiffPreviewContent', () => {
    ...
    window.api = {
      previewResources: {
        acquire: vi.fn().mockResolvedValue({
          id: 'resource-1',
          url: 'purescience-preview://resource-1/chart.tiff',
  ...
  it('renders an LZW TIFF page with the same zoom surface as other images', async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      'purescience-preview://resource-1/chart.tiff',
  ...
  it('navigates between pages in a multi-page TIFF', async () => {
    ...
    vi.mocked(window.api.previewResources.acquire).mockResolvedValue({
      id: 'resource-2',
      url: 'purescience-preview://resource-2/stack.tiff',
      size: bytes.byteLength,
      mimeType: 'image/tiff',
      version: 1
    })
  ...
  it('keeps page controls available after one page fails and returns to a valid page', async () => {
    ...
    vi.mocked(window.api.previewResources.acquire).mockResolvedValue({
      ...
      url: 'purescience-preview://resource-page-error/stack.tiff',
      size: bytes.byteLength,
      mimeType: 'image/tiff',
      version: 1
    })
  ...
  it('shows stable preview copy instead of decoder internals for an invalid TIFF', async () => {
    ...
    vi.mocked(window.api.previewResources.acquire).mockResolvedValue({
      id: 'resource-invalid',
      url: 'purescience-preview://resource-invalid/broken.tiff',
      size: bytes.byteLength,
      mimeType: 'image/tiff',
      version: 1
    })
