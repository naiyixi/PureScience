...
describe('renderOfficeFile', () => {
  ...
  it('renders DOCX with active-content features disabled and cleans up Blob URLs', async () => {
    ...
    expect(wrapper?.style.getPropertyValue('--purescience-docx-scale')).toBe('0.5')
    expect(container.querySelector('style[data-purescience-docx-fit]')?.textContent).toContain(
      'zoom: var(--purescience-docx-scale, 1)'
    )
  ...
  it('removes the DOCX wrapper frame while preserving paper styling', async () => {
    ...
    expect(wrapper?.style.getPropertyValue('--purescience-docx-scale')).toBe('1')
  ...
  it('fits mixed DOCX page sizes using the widest rendered page', async () => {
    ...
    expect(
      container
        ...
        ?.style.getPropertyValue('--purescience-docx-scale')
    ).toBe('0.4')
  ...
  it.each([
    ...
    async ({ containerWidth, expectedScale, expectedAlignment }) => {
      ...
      const wrapper = container.querySelector<HTMLElement>('.docx-wrapper')
      expect(wrapper?.style.getPropertyValue('--purescience-docx-scale')).toBe(expectedScale)
  ...
  it('hides the vendor blocking loader before parsing while preserving background progress', async () => {
    ...
    mocks.renderSpreadsheet.mockImplementation(async (_buffer, target, _type, context) => {
      const statusStyle = document.head.querySelector<HTMLStyleElement>(
        'style[data-purescience-spreadsheet-status]'
      )
      expect(target.hasAttribute('data-purescience-spreadsheet-preview')).toBe(true)
      expect(statusStyle?.textContent).toContain(
        '[data-purescience-spreadsheet-preview] .excel-wrapper .loading'
      )
    ...
    await cleanup()
    expect(document.head.querySelector('style[data-purescience-spreadsheet-status]')).toBeNull()
    expect(container.hasAttribute('data-purescience-spreadsheet-preview')).toBe(false)
  })
