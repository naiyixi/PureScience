...
type ScopeOption = { scope: PermissionScope; label: TranslationKey; subtitle: TranslationKey }

const SCOPE_OPTIONS: ScopeOption[] = [
  { scope: 'once', label: 'ws.once', subtitle: 'ws.thisCallOnly' },
  { scope: 'session', label: 'ws.thisSession', subtitle: 'ws.acrossRestartsForSession' },
  { scope: 'project', label: 'ws.thisProject', subtitle: 'ws.acrossSessionsInProject' },
  { scope: 'global', label: 'ws.global', subtitle: 'ws.acrossAllProjects' }
]
...
const CANONICAL_ACTION_LABEL: Record<string, TranslationKey> = {
  allow_once: 'ws.allowOnce',
  allow_always: 'ws.allowAlways',
  reject_once: 'ws.rejectOnce',
  reject_always: 'ws.rejectAlways'
}
...
const getScopeConfirmationSubject = (
  ...
  switch (presentation.categoryLabel) {
    case 'Command execution':
      return {
        subject: request.commandPrefix?.length ? t('ws.thisCommandGroup') : t('ws.thisCommand'),
        codeExecution: true
      }
    case 'File access':
      return { subject: t('ws.thisFileRead'), codeExecution: false }
    case 'File change':
      return { subject: t('ws.thisFileChange'), codeExecution: false }
    case 'Network access':
      return { subject: t('ws.thisNetworkRequest'), codeExecution: false }
    case 'Artifact save':
      return { subject: t('ws.thisArtifactSave'), codeExecution: false }
    case 'External service':
      return {
        subject: request.mcpIdentity
          ? (presentation.actionDetail ?? t('ws.thisExternalService'))
          : t('ws.thisExternalService'),
        codeExecution: false
      }
    case 'Notebook control':
      return { subject: t('ws.thisNotebookAction'), codeExecution: false }
    default:
      return { subject: t('ws.thisTool'), codeExecution: false }
  }
}
      ...
      <TooltipProvider delayDuration={200}>
        <Tooltip>
          <TooltipTrigger asChild>
            <button
              type="button"
              aria-label={t('ws.permissionInfo')}
...
const ScopeDropdown = ({
  ...
  return (
    <div
      ref={ref}
      role="menu"
      aria-label={t('ws.authorizationScope')}
...
const PermissionApprovalControls = ({
  ...
  const requestBroadScopeConfirmation = (
    ...
    const confirmationCopy = getScopeConfirmationSubject(presentation, request, t)
    setScopeOpen(false)
            ...
            {hasScopePicker ? (
              <>
                ...
                <button
                  ...
                  aria-label={t('ui.chooseauthorizationscope')}
