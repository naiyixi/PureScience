...
const BASE_URL_HELP_CONTENT = (t: (key: TranslationKey) => string): React.JSX.Element => (
  <>
    {t('settings.gatewayRootHint')}
    <code>/v1</code>
    {t('settings.gatewayRootSuffix')}
  </>
)
...
const API_FORMAT_HELP_CONTENT = (t: (key: TranslationKey) => string): React.JSX.Element => (
  <>
    {t('settings.apiFormatHint')}
    <code>/v1/messages</code>
    {t('settings.apiFormatMid')}
    <code>/v1/responses</code>
    {t('settings.apiFormatSuffix')}
  </>
)
    ...
    <div className="space-y-1.5">
      ...
      {needsKey ? (
        <p className={fieldErrorClassName} role="alert">
          {t('settings.storedKeyUndecryptable')}
        </p>
      ) : errors.key ? (
        <p className={fieldErrorClassName} role="alert">
          {errors.key}
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-1.5">
        ...
        >
          <SelectTrigger aria-label={t('settings.providerType')}>
      ...
      {!isCodexSubscription && !isClaudeSubscription ? (
        <div className="space-y-1.5">
          ...
          <Input
            ...
            value={value.name}
            disabled={disabled}
            placeholder={vendor ? vendor.label : t('settings.eGMyGateway')}
      ...
      {isCodexSubscription ? (
        <div className="space-y-3 rounded-lg border border-border bg-muted/30 p-3">
          <div className="space-y-1.5">
            ...
            >
              <SelectTrigger aria-label={t('settings.codexAuthentication')} disabled={disabled}>
                <span>
                  {value.type === 'codex-shared'
                    ? t('settings.importExistingCodexSignIn')
                    : t('settings.signInWithPureScience')}
                </span>
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="codex-shared">{t('settings.importExistingCodex')}</SelectItem>
                <SelectItem value="codex-isolated">{t('settings.signInWithApp')}</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <p className="text-xs text-muted-foreground">
            {value.type === 'codex-shared'
              ? t('settings.codexAuthImportHint')
              : t('settings.codexSeparateLoginHint')}
          </p>
        </div>
        ...
        <>
          <div className="space-y-3 rounded-lg border border-border bg-muted/30 p-3">
            <div className="space-y-1.5">
              ...
              >
                <SelectTrigger aria-label={t('settings.claudeAuthentication')} disabled={disabled}>
                  <span>
                    {value.type === 'claude-shared'
                      ? t('settings.useExistingClaudeProfile')
            ...
            <p className="text-xs text-muted-foreground">
              {value.type === 'claude-shared'
                ...
                : 'Advanced. Signs in through the browser and stores a separate Claude login in PureScience, completely isolated from your personal Claude profile.'}
            </p>
            <div className="space-y-1.5 border-t border-border-200 pt-3">
              <p className="text-xs text-muted-foreground">
                {value.type === 'claude-shared' ? (
                  <>
                    {t('settings.claudeSharedHint')}
                    <code className="font-mono">~/.claude</code>
                    {t('settings.claudeSharedSuffix')}
                  </>
                ) : (
                  <>
                    {t('settings.claudeIsolatedHint')}
                    <code className="font-mono">claude setup-token</code>
                    {t('settings.claudeIsolatedMid')}
                    <code className="font-mono">~/.claude</code>
                    {t('settings.claudeIsolatedSuffix')}
                  </>
                )}
              </p>
            </div>
        ...
        <>
          <div className="space-y-1.5">
            <div className="flex items-center gap-1">
              ...
              <FieldHelp content={BASE_URL_HELP_CONTENT(t)} />
            </div>
            <Input
          ...
          <div className="space-y-1.5">
            ...
            >
              <SelectTrigger aria-label={t('settings.apiFormat')} disabled={disabled}>
          ...
          <div className="flex items-center justify-between gap-4 border-t border-border-200 pt-3">
            ...
            <Switch
              ...
              aria-label={t('settings.supportsImageInput')}
          ...
          <div className="space-y-3 border-t border-border-200 pt-3">
            <div className="flex items-center justify-between gap-4">
              ...
              <Switch
                ...
                aria-label={t('settings.supportsReasoningEffort')}
            ...
            {value.reasoningEffortPreset !== 'unsupported' ? (
              <div className="space-y-3">
                ...
                >
                  <SelectTrigger
                    aria-label={t('settings.reasoningEffortLevels')}
                    disabled={disabled}
                  >
                    <span>
                      {
                ...
                <div className="space-y-1.5">
                  ...
                  >
                    <SelectTrigger
                      aria-label={t('settings.reasoningEffortRequestFormat')}
                      disabled={disabled}
                    >
                      <span>
                        {
