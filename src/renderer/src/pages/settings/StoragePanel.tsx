...
const CATEGORY_LABELS: Record<UsageCategoryKey, TranslationKey> = {
  artifacts: 'settings.artifacts',
  uploads: 'settings.uploads',
  runtime: 'settings.runtime',
  notebooks: 'settings.notebooks',
  workspaces: 'settings.sessionWorkspaces'
}
...
const StoragePanel = ({ onContinueToAgent }: StoragePanelProps): React.JSX.Element => {
  ...
  const handleRevealAppStorage = async (): Promise<void> => {
    ...
    try {
      ...
      if (!result.revealed) setRevealError(result.error ?? t('settings.couldNotRevealAppStorage'))
    } catch (error) {
      setRevealError(
        error instanceof Error ? error.message : t('settings.couldNotRevealAppStorage')
      )
    }
  }
  ...
  const handleAdopt = async (): Promise<void> => {
    ...
    if (!result.ok) {
      setIsAdopting(false)
      setAdoptError(result.error ?? t('settings.couldNotSwitchToFolder'))
    }
    // On success the app relaunches; nothing left to update here.
  }

  // "Use default location": relocate back to the default `<home>/PureScience` (the reverse of any
  ...
  const handleUseDefault = async (): Promise<void> => {
    ...
    setDefaultError(result.error ?? t('settings.defaultLocationNotUsable'))
  }
  ...
  return (
    <div className="space-y-5 p-5">
      {storageRepairActive && storageCheck ? (
        <SettingsSection
          title={t('settings.applicationStorage')}
          description={t('settings.appStorageWriteAccessHint')}
          aria-label={t('settings.applicationStorage')}
        >
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {agentRepairRequired ? (
                <Button type="button" onClick={onContinueToAgent}>
                  {t('settings.continueRepairAgent')}
                </Button>
              ) : null}
            </div>
          </div>
      ...
      <SettingsSection
        ...
        description={t('settings.whereStorageLives')}
                ...
                <div className="mt-3 flex gap-2">
                  {kind === 'adopt' ? (
                    ...
                    >
                      ...
                      {isAdopting ? t('settings.switching') : t('settings.useThisFolder')}
                    </Button>
                  ) : (
                    <Button
                      type="button"
        ...
        >
              ...
              {categories
                ...
                .map((category) => (
                  <div
                    ...
                    title={`${t(CATEGORY_LABELS[category.key])}: ${formatBytes(category.bytes)}`}
                  />
                ))}
            </div>
          ) : (
          ...
          <div className="mt-2 space-y-1.5">
            ...
            <div className="flex items-center justify-between text-xs text-muted-foreground">
              <span>{t('settings.availableOnDisk')}</span>
