...
const describeValidationFailure = (
  ...
  switch (failure.category) {
    case 'auth':
      return failure.message
        ? t('settings.testFailed').replace('{message}', failure.message)
        : t('settings.testFailedAuth')
    case 'network':
      return t('settings.testFailedNetwork')
    case 'bad-url':
      return t('settings.testFailedBadUrl')
    case 'model-not-found':
      return t('settings.testFailedModelNotFound')
    case 'timeout':
      return t('settings.testFailedTimeout')
    case 'incompatible':
    ...
    case 'server-error':
      ...
      return failure.message
        ? t('settings.testFailedHttpStatus')
            ...
            .replace('{status}', String(failure.status ?? ''))
            .replace(' (HTTP )', '')
        : t('settings.testFailedServerError')
    default:
      return failure.message
        ? t('settings.testFailed').replace('{message}', failure.message)
...
const describeType = (provider: ProviderView, t: (key: TranslationKey) => string): string => {
  if (provider.type === 'custom') return t('settings.custom')
  if (provider.type === 'claude-isolated' || provider.type === 'claude-shared')
    return t('settings.claudeSubscription')
  if (isCodexSubscriptionProvider(provider.type)) return codexSubscriptionProviderIdentity().name

  return provider.vendorId
    ? (getOfficialVendor(provider.vendorId)?.label ?? t('settings.official'))
    : t('settings.official')
}
              ...
              <div className="flex items-start justify-between gap-3">
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <span
                            ...
                            aria-label={t('settings.connectionVerified')}
                          >
                            <CircleCheck className="size-3.5" strokeWidth={2} aria-hidden="true" />
                          </span>
                        </TooltipTrigger>
                        <TooltipContent>{t('settings.connectionVerified')}</TooltipContent>
                      </Tooltip>
                    ) : null}
                  </div>
                  <div className="mt-1 space-y-0.5 text-xs text-muted-foreground">
                    {codexSubscriptionType === 'codex-shared' ? (
                      <div>{t('settings.authImportedIntoApp')}</div>
                    ) : codexSubscriptionType === 'codex-isolated' ? (
                      <div>{t('settings.codexLoginStoredSeparately')}</div>
                      ...
                      <div>
                        {t('settings.browserOauthDidntOpen')}
                        {t('settings.setupTokenSuffix')}
                      </div>
                      ...
                      provider.maskedKey ? (
                        ...
                        <div>{t('settings.notSignedIn')}</div>
                      )
                    ) : (
                ...
                <div className="flex shrink-0 items-center gap-1.5">
                  ...
                  !isClaudeIsolatedLoginPending ? (
                    <>
                      <SettingsIconAction
                        label={t('settings.signInWithBrowser')}
                        icon={LogIn}
                      ...
                      />
                      <SettingsIconAction
                        label={t('settings.useSetupToken')}
                        icon={KeyRound}
