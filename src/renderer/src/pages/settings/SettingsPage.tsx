...
const buildSettingsGroups = (t: (key: TranslationKey) => string): ReadonlyArray<SettingsGroup> => [
  {
...
const flattenPanels = (groups: ReadonlyArray<SettingsGroup>): ReadonlyArray<SettingsPanel> =>
  groups.flatMap((group) => group.panels)
...
const SettingsPage = forwardRef<SettingsPageHandle, SettingsPageProps>(function SettingsPage(
  ...
  const breadcrumb = ((): {
      ...
      const leaf =
        skillsView.kind === 'create'
          ...
          : skillsView.kind === 'upload'
            ? t('settings.uploadSkills')
            : skillsView.kind === 'import'
              ? t('settings.importFromGitHub')
              : skillsView.kind === 'import-agent-home'
                ? t('settings.importInstalledSkills')
                : (() => {
                    const name = skills.find((skill) => skill.id === skillsView.id)?.name ?? ''
                    return skillsView.kind === 'edit'
                      ? t('settings.editName').replace('{name}', name)
                      : name
                  })()
      return {
        rootLabel: t('settings.skills'),
    ...
    if (activePanel === 'model' && modelView.kind !== 'list') {
      const name =
        modelView.kind === 'edit'
          ...
          : ''
      return {
        rootLabel: t('settings.model'),
        rootTo: { panel: 'model', skills: currentLocation.skills, model: { kind: 'list' } },
        leaf: modelView.kind === 'create' ? t('settings.addProvider') : `Edit ${name}`.trim()
      }
    }
    if (activePanel === 'network' && networkView.kind !== 'list') {
      return {
        rootLabel: t('settings.network'),
        rootTo: {
          panel: 'network',
    ...
    if (activePanel === 'connectors' && connectorsView.kind !== 'list') {
      ...
      return {
        rootLabel: t('settings.connectors'),
        rootTo: {
          panel: 'connectors',
          skills: currentLocation.skills,
          model: currentLocation.model,
    ...
    if (activePanel === 'specialists' && specialistsView.kind !== 'list') {
      ...
      const leaf =
        specialistsView.kind === 'create'
          ? t('settings.newSpecialist')
          : (editingSpecialist?.name ?? t('settings.editSpecialist'))
      return {
        rootLabel: t('settings.specialists'),
        rootTo: {
          panel: 'specialists',
          skills: currentLocation.skills,
          model: currentLocation.model,
        ...
        >
          ...
          {isMobileNavOpen ? (
            <button
              ...
              aria-label={t('settings.closeSettingsNav')}
              ...
              <div className="flex h-12 shrink-0 items-center justify-between gap-2 border-b border-border bg-card px-2 md:px-3">
                <div className="flex min-w-0 items-center gap-1">
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        ...
                        aria-label={t('settings.openSettingsNav')}
                        className="shrink-0 rounded-lg text-muted-foreground md:hidden"
                      >
                        <Menu className="size-4" aria-hidden="true" />
                      </Button>
                    </TooltipTrigger>
                    <TooltipContent>{t('settings.navigation')}</TooltipContent>
                  </Tooltip>
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent>{t('settings.back')}</TooltipContent>
                  </Tooltip>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        ...
                        aria-label={t('settings.forward')}
                        className="shrink-0 rounded-lg text-muted-foreground disabled:opacity-40"
                      >
                        <ArrowRight className="size-4" aria-hidden="true" />
                      </Button>
                    </TooltipTrigger>
                    <TooltipContent>{t('settings.forward')}</TooltipContent>
                  </Tooltip>
                  ...
                  {breadcrumb !== null ? (
                    ...
                    <h2 className="truncate text-sm font-semibold text-foreground">
                      {settingsPanels.find((panel) => panel.id === activePanel)?.label}
                    </h2>
                  )}
                </div>
                <div className="flex shrink-0 items-center gap-1">
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent>{t('settings.closeSettings')}</TooltipContent>
                  </Tooltip>
                </div>
              </div>
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        ...
                        variant="ghost"
                        size="icon-sm"
                        aria-label={t('settings.dismissSettingsError')}
                    ...
                    <TooltipContent>{t('settings.dismiss')}</TooltipContent>
                  </Tooltip>
                </div>
              ) : null}
            </TooltipProvider>
            ...
            <div data-slot="settings-content-scroll" className="min-h-0 flex-1 overflow-y-auto">
              <div className="mx-auto min-h-full w-full max-w-[880px]">
                {activePanel === 'skills' ? (
                  ...
                  <AgentPanel
                    title={t('settings.agentFrameworkTitle')}
                    description={t('settings.agentFrameworkDesc')}
                  />
                ) : isProviderFormOpen ? (
