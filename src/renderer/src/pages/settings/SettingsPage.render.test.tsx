  ...
  ;(window as unknown as { api: unknown }).api = {
    ...
    cli: {
      getStatus: vi.fn().mockResolvedValue({
        installed: false,
        target: '/Users/x/.local/bin/purescience',
        onPath: false
      }),
      install: vi.fn(),
      uninstall: vi.fn()
...
describe('SettingsPage layout', () => {
  ...
  it('mounts the sidebar + content with grouped nav items and a close control', () => {
    ...
    expect(navItems[5]?.textContent).toContain('Tags')
    expect(navItems[6]?.textContent).toContain('Compute')
    expect(navItems[7]?.textContent).toContain('Scheduled tasks')
    expect(navItems[8]?.textContent).toContain('Local models')
    expect(navItems[9]?.textContent).toContain('Network')
    expect(navItems[10]?.textContent).toContain('Model')
    expect(navItems[11]?.textContent).toContain('Agent')
    expect(navItems[12]?.textContent).toContain('Permissions')
    expect(navItems[13]?.textContent).toContain('Runtimes')
    expect(navItems[14]?.textContent).toContain('Storage')
    expect(navItems[15]?.textContent).toContain('Token usage')
    expect(navItems[16]?.textContent).toContain('General')
    expect(navItems[17]?.textContent).toContain('Remote control')
    expect(navItems[18]?.textContent).toContain('Archived')
  ...
  it('switches to the General panel and shows the diagnostic log file', async () => {
    ...
    // Appearance, Licensing, AppVersion, Notifications, App icon, Diagnostics, Command line tool, Community.
    expect(document.body.querySelectorAll('[data-slot="settings-section"]')).toHaveLength(8)
      ...
      checks: [
        {
          ...
          status: 'failed' as const,
          summary: 'PureScience cannot write to its private data folder.'
        }
      ],
      ready: false,
      canAutoInstall: false,
      ...
      checks: [
        {
          ...
          summary: 'PureScience can write to its private data folder.'
        },
        {
          id: 'agent' as const,
          label: 'Claude runtime',
