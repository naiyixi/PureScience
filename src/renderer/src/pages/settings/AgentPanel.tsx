...
const AgentPanel = ({
    ...
    if (failure) {
      setFrameworkDetectionError(
        failure.reason instanceof Error
          ...
          : t('settings.agentRuntimesNotDetected')
      )
    }
  }
  ...
  const frameworkCards: FrameworkCardModel[] = [
    ...
    },
    {
      ...
      name: t('settings.agentOpencodeName'),
      icon: <AgentFrameworkIcon frameworkId="opencode" size={24} />,
      description: t('settings.agentOpencodeDesc'),
    ...
    },
    {
      key: 'codex',
      frameworkId: 'codex',
      name: t('settings.agentCodexName'),
      ...
      notReadyHint: codex.resolvedPath
        ? t('settings.adapterDetectionFailed')
        : t('settings.agentCodexNotReady'),
  ...
  if (failedCheckIds.has('system')) {
    blockedInstallSources.managed = t('settings.systemRequirementsNotMet')
  }
  if (failedCheckIds.has('install-network')) {
    blockedInstallSources.managed ??= t('settings.installationNetworkUnavailable')
    blockedInstallSources.npm = t('settings.installationNetworkUnavailable')
  }
  ...
  const installFramework = async (
    ...
    try {
      ...
      setInstallActionError(
        error instanceof Error ? error.message : t('settings.installerCouldNotStart')
      )
      return
    }
    if (!result?.ok) return
        ...
        action={
          ...
          >
            <RefreshCw
              ...
              aria-hidden="true"
            />
            {isDetectingAnyFramework ? t('settings.detecting') : t('settings.redetect')}
          </Button>
        }
      >
        <div className="space-y-5">
          ...
          {!isOnboarding && agentCheckFailures.length > 0 ? (
            <div
              aria-label={t('settings.agentRuntimeRepairIssues')}
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-medium text-foreground">
                    {t('settings.agentCannotBeAccessed').replace(
                      '{name}',
                      activeFramework?.displayName ?? t('settings.theSelectedAgent')
                    )}
                  </p>
                  <p className="text-xs leading-5 text-muted-foreground">
                    {t('settings.repairAgentFirst')}
                  </p>
                </div>
              </div>
          ...
          {installBlockers.length > 0 ? (
            <div
              aria-label={t('settings.agentInstallationBlockers')}
              className="space-y-2 rounded-lg border border-amber-500/30 bg-amber-500/5 p-3"
            >
