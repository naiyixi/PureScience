    ...
    try {
      ...
      if (result.repositories !== undefined) {
        ...
        if (result.repositories.length === 0) {
          setMessage({
            kind: 'status',
            text: t('settings.noMatchingSkillRepos')
          })
        }
        return
      }
      ...
      if (result.skills.length === 0) {
        setMessage({ kind: 'status', text: t('settings.noSkillsFoundInRepo') })
      }
    } catch (error) {
      if (inputRef.current !== visibleInputAtStart) return
      setMessage({
        kind: 'error',
        text: error instanceof Error ? error.message : t('settings.githubRequestFailed')
      })
    } finally {
      setOperation(null)
    }
  ...
  return (
    <div className="p-5">
      ...
      <div className="mt-4">
        ...
        >
          {t('settings.githubKeywordOrRepo')}
        </label>
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
          <Input
            id="github-skill-source"
            aria-label={t('settings.githubKeywordOrRepo')}
            aria-invalid={
              (message?.kind === 'error' &&
      ...
      <div aria-busy={busy}>
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              {repositories.length > 0 ? (
                <Button
                  type="button"
                  variant="outline"
                  aria-label={`${repositoriesExpanded ? t('settings.hide') : t('settings.show')} repositories`}
                ...
                >
                  ...
                  {repositoriesExpanded
                    ? t('settings.hideRepositories')
                    : t('settings.showRepositories')}
                </Button>
              ) : null}
            </div>
            {repositories.length > 0 ? (
              ...
              <div className="mt-2 flex items-center gap-2 py-3 text-xs text-muted-foreground">
                ...
                <p>
                  {message?.kind === 'status' ? message.text : t('settings.noRepositoriesFound')}
                </p>
              </div>
            ) : null}
          </section>
        ...
        {scanned && scanned.length > 0 ? (
          <section aria-label={`Skills found in ${scannedRepo}`} className="mt-5">
            ...
            <div
              role="group"
              aria-label={t('settings.skillSelectionControls')}
