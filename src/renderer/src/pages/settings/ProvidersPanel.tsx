...
const ProvidersPanel = ({
  ...
  const handleClaudeSharedDisconnect = async (): Promise<void> => {
    ...
    try {
      ...
      if (!result.ok) {
        setProviderTestError(result.message ?? 'Could not disconnect Claude from PureScience.')
      }
    } catch (error) {
      setProviderTestError(
        error instanceof Error ? error.message : 'Could not disconnect Claude from PureScience.'
      )
    }
  }
