...
const ProvidersPanel = ({
  ...
  const handleTest = async (provider: ProviderView): Promise<void> => {
    ...
    try {
      ...
      setProviderTestError(
        error instanceof Error ? error.message : t('settings.couldNotTestProviderConnection')
      )
    } finally {
  ...
  const handleCodexLogin = async (): Promise<void> => {
    ...
    try {
      ...
      setProviderTestError(
        error instanceof Error ? error.message : t('settings.couldNotSignInToCodex')
      )
    } finally {
  ...
  const handleCodexLogout = async (): Promise<void> => {
    ...
    try {
      ...
      if (!result.ok) {
        setProviderTestError(result.message ?? t('settings.codexSignOutIncomplete'))
      }
    } catch (error) {
      setProviderTestError(
  ...
  const handleClaudeSignIn = async (token: string): Promise<ValidateProviderResult | undefined> => {
    ...
    try {
      ...
      if (!result.ok) {
        setProviderTestError(result.message ?? t('settings.claudeTokenSaveFailedRetry'))
      }

      return result
    } catch (error) {
      setProviderTestError(
        error instanceof Error ? error.message : t('settings.couldNotSaveClaudeToken')
      )
  ...
  const handleClaudeIsolatedBrowserLogin = async (): Promise<void> => {
    ...
    try {
      ...
      if (result.ok) {
        ...
        setProviderTestError(result.message ?? t('settings.couldNotSignInToClaudeRetry'))
      }
    } catch (error) {
      if (manualClaudePasteWonRef.current) return
      setProviderTestError(
        error instanceof Error ? error.message : t('settings.couldNotSignInToClaude')
      )
    } finally {
  ...
  const handleClaudeLogout = async (): Promise<void> => {
    ...
    try {
      ...
      if (!result.ok) {
        setProviderTestError(result.message ?? t('settings.claudeSignOutIncomplete'))
      }
    } catch (error) {
      setProviderTestError(
        error instanceof Error ? error.message : t('settings.couldNotSignOutOfClaude')
      )
    }
  }
  ...
  const handleClaudeSharedDisconnect = async (): Promise<void> => {
    ...
    try {
      ...
      if (!result.ok) {
        setProviderTestError(result.message ?? t('settings.couldNotDisconnectClaude'))
      }
    } catch (error) {
      setProviderTestError(
        error instanceof Error ? error.message : t('settings.couldNotDisconnectClaude')
      )
    }
  }
  ...
  return (
    <div className="space-y-5 p-5">
      ...
      {visibleProviders.length > 0 ? (
        <SettingsSection
          title={t('settings.activeModel')}
          aria-label={t('settings.activeModel')}
          description={t('settings.activeModelHint')}
        >
      ...
      <SettingsSection
        ...
        description={t('settings.visionModelHint')}
      ...
      <SettingsSection
        ...
        description={t('settings.reasoningEffortHint')}
        separated={visibleProviders.length > 0}
      >
        <div className="max-w-md">
