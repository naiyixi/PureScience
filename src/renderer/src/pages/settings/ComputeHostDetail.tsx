...
const probedLabel = (host: ComputeHost, t: (key: TranslationKey) => string): string | null => {
  ...
  if (seconds < 60) return t('settings.probedJustNow')
  const minutes = Math.round(seconds / 60)
  if (minutes < 60) return t('settings.probedMinAgo').replace('{n}', String(minutes))
  const hours = Math.round(minutes / 60)
  if (hours < 24) return t('settings.probedHAgo').replace('{n}', String(hours))
  return t('settings.probedDAgo').replace('{n}', String(Math.round(hours / 24)))
}
...
export function ComputeHostDetail({
    ...
    return (
      <div className="p-5">
        <p className="py-8 text-center text-sm text-muted-foreground">
          {isLoaded ? t('settings.hostNoLongerExists') : t('settings.loadingHost')}
        </p>
      </div>
    )
  }
  ...
  const probedAgo = probedLabel(host, t)
  ...
  const handleProbe = async (): Promise<void> => {
    ...
    try {
      ...
      setProbeError(err instanceof Error ? err.message : t('settings.probeFailedUnexpectedly'))
    }
  }
  ...
  const handleDetailsSave = async (): Promise<void> => {
    ...
    try {
      ...
      setDetailsError(err instanceof Error ? err.message : t('settings.failedToSaveDetails'))
    } finally {
  ...
  const handleScratchSave = async (): Promise<void> => {
    ...
    try {
      ...
      setScratchError(err instanceof Error ? err.message : t('settings.failedToSetScratchRoot'))
    } finally {
  ...
  const handleConcurrencySave = async (): Promise<void> => {
    ...
    if (!Number.isInteger(n) || n < 1 || n > 500) {
      setConcurrencyError(t('settings.concurrencyMustBeInteger'))
      return
    }
    ...
    try {
      ...
      setConcurrencyError(
        err instanceof Error ? err.message : t('settings.failedToSetConcurrencyLimit')
      )
    } finally {
    ...
    <div className="p-5">
          ...
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {status === 'connected' ? (
                ...
                <Badge variant="outline">{t('settings.notProbed')}</Badge>
              )}
            </div>
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              onClick={() => void handleProbe()}
              disabled={isProbing}
              aria-label={t('settings.retryProbe')}
      ...
      {status === 'connected' && probed ? (
        <div className="mt-6 flex flex-col gap-2">
          <h4 className="text-sm font-medium text-foreground">{t('settings.resources')}</h4>
            ...
            {probed.memMib != null ? (
              <div className="flex items-center gap-1.5 rounded-lg border border-border bg-muted/30 px-2.5 py-1.5 text-sm">
                ...
                <span>
                  ...
                  <span className="text-muted-foreground">{t('settings.gbRam')}</span>
                </span>
              </div>
            ) : null}
        ...
        {isEditingDetails ? (
          <div className="mt-3 flex flex-col gap-2">
            <textarea
              ...
              aria-label={t('settings.detailsDocument')}
          ...
          <div className="mt-3">
              ...
              >
                ...
                {isSkeleton ? (
                  <span className="ml-2 text-muted-foreground">
                    {t('settings.autoGeneratedFromProbe')}
                  </span>
                ) : null}
              </pre>
            ...
            {needsExpand ? (
              ...
              >
                {isDetailsExpanded ? (
                  <>
                    {t('settings.showLess')} <ChevronUp className="ml-1 size-3" />
                  </>
                ) : (
                  <>
                    {t('settings.showMore')} <ChevronDown className="ml-1 size-3" />
                  </>
                )}
              </Button>
            ) : null}
        ...
        {isEditingScratch ? (
          <div className="mt-3 flex flex-col gap-2">
            <input
              ...
              aria-label={t('settings.scratchRootPath')}
        ...
        {isEditingConcurrency ? (
          <div className="mt-3 flex flex-col gap-2">
            <input
              ...
              placeholder="10"
              aria-label={t('settings.concurrentJobLimit')}
      ...
      <div className="mt-7">
        ...
        <div className="mt-3 flex flex-col gap-2">
          ...
          >
            <option value="direct_ssh">{t('settings.directSsh')}</option>
