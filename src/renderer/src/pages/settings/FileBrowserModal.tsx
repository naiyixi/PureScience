...
const inferType = (name: string): string => {
  ...
  if (dot === -1 || dot === name.length - 1) return 'settings.fileTypeGeneric'
  const ext = name.slice(dot + 1).toLowerCase()
  const map: Record<string, string> = {
    py: 'settings.fileTypePython',
    ipynb: 'settings.fileTypeNotebook',
    sh: 'settings.fileTypeShell',
    txt: 'settings.fileTypeText',
    csv: 'CSV',
    tsv: 'TSV',
    json: 'JSON',
    yaml: 'YAML',
    yml: 'YAML',
    md: 'settings.fileTypeMarkdown',
    pdf: 'PDF',
    png: 'settings.fileTypeImage',
    jpg: 'settings.fileTypeImage',
    jpeg: 'settings.fileTypeImage',
    gif: 'settings.fileTypeImage',
    svg: 'SVG',
    zip: 'settings.fileTypeArchive',
    tar: 'settings.fileTypeArchive',
    gz: 'settings.fileTypeArchive',
    bz2: 'settings.fileTypeArchive',
    h5: 'settings.fileTypeHdf5',
    hdf5: 'settings.fileTypeHdf5',
    nc: 'settings.fileTypeNetCDF',
    r: 'settings.fileTypeR',
    rds: 'settings.fileTypeRData',
    exe: 'settings.fileTypeBinary',
    so: 'settings.fileTypeLibrary',
    dylib: 'settings.fileTypeLibrary',
    log: 'settings.fileTypeLog'
  }
...
function DetailPanel({
  ...
  const handleDownload = async (): Promise<void> => {
    ...
    try {
      ...
      const detail = fsErr?.detail ?? e.message ?? t('settings.downloadFailed')
    ...
    <div className="flex w-52 shrink-0 flex-col border-l border-border">
      <div className="flex items-center justify-between border-b border-border px-3 py-2">
        ...
        <button
          type="button"
          onClick={onClose}
          aria-label={t('settings.closeDetailPanel')}
      ...
      <div className="flex-1 overflow-auto p-3 space-y-3">
        ...
        <Button
          ...
          aria-label={t('settings.downloadToOsDownloads')}
        >
        ...
        {activeProjectId && (
          <Button
            ...
            className="w-full gap-1.5 text-xs"
            disabled
            title={t('settings.comingSoonArtifactImport')}
            aria-label={t('settings.addToProjectArtifactLabel')}
          >
            <FolderOpen className="size-3.5" />
            {t('settings.addToProject')}
          </Button>
        )}
        ...
        <Button
          ...
          aria-label={t('settings.copyRemoteAbsolutePath')}
        >
...
export function FileBrowserModal({
  ...
  const navigate = useCallback(
    async (path: string, pushHistory = true) => {
      ...
      try {
        ...
        const e = err as Error & { remoteFsError?: { detail: string; remoteKind: string } }
        const fsErr = e.remoteFsError ?? decodeRemoteFsError(e.message ?? '')
        const detail = fsErr?.detail ?? e.message ?? t('settings.unknownError')
  ...
  const handleAddressSubmit = (e: React.FormEvent): void => {
    ...
    if (validateRemotePath(resolved) === 'outside_roots') {
      setBrowserState({
        kind: 'error',
        detail: t('settings.pathMustBeAbsolute'),
    ...
    >
      <Dialog.Portal>
        ...
        <Dialog.Content
          ...
          aria-label={t('settings.remoteFileBrowser')}
        >
          ...
          <div className="flex shrink-0 items-center gap-1.5 border-b border-border px-3 py-1.5">
            ...
            <Button
              ...
              disabled={isAtRoot()}
              onClick={handleUp}
              aria-label={t('settings.goUpOneLevel')}
            >
              ...
              {gotoOpen && (
                ...
                >
                  ...
                  >
                    ...
                    <span>{t('settings.pinCurrentFolder')}</span>
                  </button>
                  {/* Bookmarks */}
                  {bookmarks.length > 0 && (
                    <>
            ...
            <form onSubmit={handleAddressSubmit} className="flex flex-1 items-center">
              <input
                ...
                aria-label={t('settings.currentDirectoryPath')}
                spellCheck={false}
              />
            </form>
            ...
            <Button
              ...
              aria-label={t('settings.refreshDirectoryListing')}
            >
