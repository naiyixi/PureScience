...
const SkillEditor = ({ initial, onCancel, onSave }: SkillEditorProps): React.JSX.Element => {
  ...
  const slugError = useMemo((): string | null => {
    ...
    if (!currentSlug) return t('settings.skillIdRequired')
    if (!/^[a-z0-9-]+$/.test(currentSlug)) {
      return t('settings.onlyLowercaseId')
    }
    ...
    <div className="flex min-h-0 flex-1 flex-col">
        ...
        <div className="p-5">
              ...
              <label className="flex flex-col gap-1.5">
                ...
                {slugError ? (
                  ...
                  <span className="text-xs text-muted-foreground">
                    {isCreate
                      ? t('settings.skillIdFolderHint')
                      : t('settings.skillIdFixedAfterCreation')}
                  </span>
                )}
              </label>
              <label className="flex flex-col gap-1.5">
          ...
          <section>
            <div className="flex items-start justify-between gap-3">
              ...
              <div
                role="radiogroup"
                aria-label={t('settings.contentMode')}
            ...
            {contentMode === 'write' ? (
              <>
                <Textarea
                  aria-label={t('settings.skillBody')}
                  value={body}
                ...
                {!frontmatterImportMode && metadataEntries.length > 0 ? (
                  <div
                    aria-label={t('settings.skillMetadata')}
                  ...
                  >
                    ...
                    <Button
                      ...
                      variant="ghost"
                      size="sm"
                      aria-label={t('settings.clearSkillMetadata')}
          ...
          <section>
            ...
            >
              ...
              <input
                type="file"
                multiple
                aria-label={t('settings.addReferenceFiles')}
                className="hidden"
      ...
      <div className="flex shrink-0 items-center justify-end gap-2 border-t border-border bg-card px-5 py-3">
        ...
        <Button type="button" onClick={() => void handleSave()} disabled={!canSave}>
          {saving ? t('settings.saving') : initial.id ? t('settings.save') : t('settings.publish')}
        </Button>
      </div>
    </div>
  )
