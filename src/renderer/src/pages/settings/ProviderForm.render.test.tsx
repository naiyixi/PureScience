...
describe('ProviderForm field switching', () => {
  ...
  it('describes existing Codex authentication as a one-time import', () => {
    ...
    expect(container.textContent).toContain(
      "Copies Codex authentication and, when compatible, the active provider's non-secret loopback route into PureScience"
    )
  ...
  it('describes encrypted and unavailable storage accurately', () => {
    expect(getApiKeySecurityCopy(true)).toEqual({
      ...
      description:
        'It is stored only on this device and never uploaded to PureScience. Your OS secure storage protects it, and it is sent only to the selected provider when you make a request.'
    })
    expect(getApiKeySecurityCopy(false)).toEqual({
      title: 'Secure storage is unavailable.',
      description:
        'PureScience will not save API keys until the operating-system credential vault is available. Unlock or authorize the system keychain, then retry.'
    })
  })
