...
beforeEach(() => {
  ...
  ;(window as unknown as { api: unknown }).api = {
    storage: {
      ...
      inspectDataRoot: vi
        .fn()
        .mockResolvedValue({ kind: 'move', dataRoot: '/mnt/data/PureScience' }),
...
describe('StoragePanel', () => {
  it('uses shared settings dialog chrome for data-location confirmations', async () => {
    ...
    ;(
      ...
      kind: 'adopt',
      dataRoot: '/mnt/existing/PureScience'
    })
  ...
  it('shows the exact application-storage failure and reveals the trusted config root', async () => {
    const detail = '/home/u/.purescience — EACCES: permission denied'
      ...
      environmentCheck: environment([
        {
          ...
          status: 'failed',
          summary: 'PureScience cannot write to its private data folder.',
          detail: '/home/u/.purescience — EACCES: permission denied'
        }
      ]),
      checkEnvironment
    } as never)
  ...
  it('shows the hand-editing warning only after Change location is clicked', async () => {
    ...
    expect(container.textContent).not.toContain('PureScience manages this folder')
    ...
    expect(document.body.textContent).toContain('PureScience manages this folder')
  ...
  it('collects the target path via Browse and opens the migration flow on Change', async () => {
    ...
    // The final `<parent>/PureScience` path is shown so the user sees the appended folder before
    // acting, not just the raw parent they picked.
    expect(container.textContent).toContain('Data will be stored in')
    expect(container.textContent).toContain('/mnt/data/PureScience')
  ...
  it('a path classified as move shows the migrate action, not the adopt one', async () => {
    ...
    ;(
      ...
      kind: 'move',
      dataRoot: '/mnt/empty/PureScience'
    })
  ...
  it('a path classified as adopt shows "Use this folder" and switches without moving', async () => {
    ...
    expect(container.textContent).toContain('already contains PureScience data')
    expect(container.textContent).toContain('Data will be stored in')
    expect(container.textContent).toContain('/mnt/existing/PureScience')
    expect(
      Array.from(container.querySelectorAll('button')).some(
        (button) => button.textContent?.trim() === 'Change location'
      )
    ...
    // The confirm dialog shows the derived `<parent>/PureScience` target, not the raw parent.
    expect(dialog?.textContent).toContain('/mnt/existing/PureScience')
  ...
  it('offers return-to-default inside the editor only when the current root is custom', async () => {
    ;(
      ...
      dataRoot: '/mnt/data/PureScience',
      isDefault: false,
      defaultDataRoot: '/home/u/PureScience',
