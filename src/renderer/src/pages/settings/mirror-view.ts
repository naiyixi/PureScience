...
export const mirrorStatusText = (
  mirror: PackageMirror | undefined,
  t: (key: TranslationKey) => string
): string => {
  if (!isMirrorConfigured(mirror)) {
    return t('settings.mirrorNotConfigured')
  }
  const parts = [mirror!.condaChannel, mirror!.pypiIndex].filter(Boolean)
  return t('settings.fetchingPackages').replace('{hosts}', parts.join(' , '))
}
