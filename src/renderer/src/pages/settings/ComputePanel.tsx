...
const probedLabel = (host: ComputeHost, t: (key: TranslationKey) => string): string | null => {
  ...
  if (hours < 24) return t('settings.probedHoursAgo').replace('{n}', String(hours))
  return t('settings.probedDaysAgo').replace('{n}', String(Math.round(hours / 24)))
}
...
const HostCard = ({
  ...
  const status: 'connected' | 'failed' | 'none' = probed
    ? probed.ok
      ...
      : 'failed'
    : 'none'
  const probed_ago = probedLabel(host, t)
    ...
    >
      ...
      <TooltipProvider delayDuration={200}>
        ...
        <Tooltip>
          ...
          <TooltipContent>
            {status === 'connected' ? t('settings.browseFiles') : t('settings.probeHostFirst')}
          </TooltipContent>
        </Tooltip>
        <Tooltip>
          <TooltipTrigger asChild>
          ...
          <TooltipContent>{t('settings.removeHost')}</TooltipContent>
        </Tooltip>
      </TooltipProvider>
  ...
  return (
    <div className="p-5">
      <div className="mb-5 flex items-start gap-2 rounded-xl bg-muted/50 px-3 py-2.5 text-sm text-muted-foreground">
        ...
        <span>{t('settings.computeConnectHint')}</span>
      </div>

      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <h3 className="text-base font-medium text-foreground">{t('settings.sshHosts')}</h3>
          <p className="mt-0.5 max-w-2xl text-sm leading-5 text-muted-foreground">
            {t('settings.sshHostsDesc')}
          </p>
        </div>
        <Button
          type="button"
