...
const SKILL_SOURCE_LABELS: Record<SkillSource, TranslationKey> = {
  featured: 'settings.featuredFilter',
  imported: 'settings.importedFilter',
  personal: 'settings.personalFilter'
}
...
const SEVERITY_GROUPS = [
  { severity: 'error', label: 'settings.blockingErrors' },
  { severity: 'warning', label: 'settings.warnings' },
  { severity: 'info', label: 'settings.information' }
...
const SpecialistsPanel = ({ view, onNavigate }: SpecialistsPanelProps): React.JSX.Element => {
  ...
  useEffect(() => {
    ...
    void previewExport(view.id)
      ...
      .catch(() => {
        if (active) setExportError(t('settings.couldNotPreviewSpecialistExport'))
      })
    return () => {
      active = false
    }
  ...
  const runDirectExport = async (id: string): Promise<void> => {
    ...
    try {
      ...
      setExportError(t('settings.couldNotSaveSpecialistExport'))
      onNavigate({ kind: 'export', id })
    } finally {
      setExportingId(null)
    }
    ...
    void (async () => {
      ...
      try {
        if (typeof window.api?.specialist?.exportContributionTemplate !== 'function') {
          setTemplateSaveError(t('settings.contributionTemplatesDesktopOnly'))
          return
        }
        const result = await window.api.specialist.exportContributionTemplate()
        if (result?.saved) setTemplateSaved(true)
      } catch {
        setTemplateSaveError(t('settings.couldNotSaveContributionTemplate'))
      } finally {
      ...
      return (
        <div className="p-5">
          <div className="rounded-xl border border-border px-6 py-10 text-center" role="status">
            ...
            <h2 className="mt-4 text-lg font-semibold">{t('settings.exportComplete')}</h2>
            <p className="mt-2 text-sm text-muted-foreground">
        ...
        <div className="mb-5 flex items-start justify-between gap-4 border-b border-border pb-4">
          <div>
            ...
            <h2 className="mt-1 text-xl font-semibold">
              {exportPreview ? t('settings.chooseSkillsToInclude') : t('settings.preparingExport')}
            </h2>
            <p className="mt-1 text-sm text-muted-foreground">
              {t('settings.builtinSkillsSelectedDefault')}
        ...
        {exportPreview ? (
          <div className="flex flex-col gap-4">
            ...
            <div className="rounded-lg border border-border p-3 text-sm" role="status">
              <strong className="block">{t('settings.whatPackageCarries')}</strong>
              <p className="text-muted-foreground">
                {t('settings.onlyCheckedSkillsBundled')}
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  void exportSpecialist(exportPreview, includedExportSkillIds)
                    ...
                    .catch(() => setExportError(t('settings.couldNotSaveSpecialistExport')))
                    .finally(() => setExportBusy(false))
                }}
              >
                {exportBusy ? t('settings.saving') : t('settings.exportZip')}
              </Button>
            </div>
          </div>
        ) : null}
    ...
    return (
      <div className="p-5">
        <div className="mb-5 flex items-start justify-between gap-4 border-b border-border pb-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-primary">
              {packagePreview ? t('settings.importZipPreview') : t('settings.importZip')}
            </p>
            <h2 className="mt-1 text-xl font-semibold">
              {packagePreview
                ? packagePreview.installable
                  ? t('settings.readyToContinue')
                  : t('settings.cannotContinue')
                : t('settings.importSpecialistPackage')}
              {packagePreview ? (
                <span
                  role="status"
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              {packagePreview
                ? t('settings.reviewPackageSummary')
                : t('settings.chooseOneZipSpecialist')}
            </p>
          </div>
          <Button type="button" variant="outline" onClick={() => onNavigate({ kind: 'list' })}>
            Back
        ...
        {!packagePreview ? (
          <div className="rounded-xl border border-border p-6 text-center">
            ...
            <h3 className="mt-3 text-sm font-semibold">{t('settings.selectSpecialistZip')}</h3>
            ...
            <div className="mt-5 flex justify-center gap-2">
              ...
              >
                ...
                {templateSaving ? t('settings.savingTemplate') : t('settings.downloadTemplate')}
              </Button>
              <Button
                type="button"
          ...
          >
            <div className="grid grid-cols-2 gap-3 rounded-xl border border-border p-4 text-sm">
              <div>
                ...
                {summary?.name ?? t('settings.unknown')}
              </div>
              <div>
                <span className="block text-xs text-muted-foreground">
                  {t('settings.immutableId')}
                </span>
                {summary?.id ?? t('settings.unknown')}
              </div>
              <div>
                <span className="block text-xs text-muted-foreground">
                  {t('settings.packageVersion')}
                </span>
                {summary?.version ?? t('settings.unknown')}
              </div>
            </div>

            <section className="rounded-xl border border-border p-4">
              <h3 className="text-sm font-semibold">{t('settings.skills')}</h3>
              <p className="mt-1 text-xs text-muted-foreground">
                {summary?.bundledSkillIds.length
                  ? `Bundled: ${summary.bundledSkillIds.join(', ')}`
                  : t('settings.noBundledSkills')}
              </p>
                  ...
                  <Button
                    ...
                    onClick={() => {
                      void window.api.specialist
                        ...
                        .then((result) =>
                          setReportStatus(result.saved ? t('settings.reportSaved') : undefined)
                        )
                        .catch(() => setReportStatus(t('settings.couldNotSaveReport')))
                    }}
                  >
                    <Download data-icon="inline-start" aria-hidden="true" />
                    Download JSON
            ...
            <div className="flex justify-between gap-2">
              ...
              >
                {packagePreview.overwrite ? t('settings.reviewOverwrite') : t('settings.next')}
              </Button>
            </div>
  ...
  if (
    ...
    view.kind === 'marketplace-sources' ||
    view.kind === 'marketplace-release'
  ) {
    return <SpecialistMarketplace view={view} onNavigate={onNavigate} />
  }
    ...
    <div className="p-5">
      ...
      <div className="mb-4 flex items-center gap-2">
        ...
        <div className="relative flex-1">
          ...
          <Input
            type="search"
            aria-label={t('settings.searchSpecialists')}
            placeholder={t('settings.searchSpecialists')}
            value={query}
                                ...
                                <DropdownMenuTrigger asChild>
                                  ...
                                  >
                                    {exportingId === item.id ? (
                                      <span
                                        role="status"
                                        aria-label={t('settings.preparingExportLabel')}
                                      >
                                        <Loader2
                            ...
                            <DropdownMenuItem
                              ...
                              onSelect={() => {
                                ...
                                void previewSpecialistDelete(item.id)
                                  ...
                                  .catch(() =>
                                    setDeleteError(t('settings.couldNotLoadSkillRelationships'))
                                  )
                              }}
                            >
                    ...
                    skill.reasons
                      .map((reason) =>
                        reason.code === 'standalone'
                          ? t('settings.alreadyExistsIndependently')
                          : reason.code === 'shared-owner'
                            ? t('settings.alsoOwnedByAnotherSpecialist')
                            : reason.code === 'referenced'
                              ? t('settings.usedByAnotherSpecialist')
                              : t('settings.managedByAppAndKept')
                      )
                      .join(' ') || t('settings.usedOnlyByThisSpecialist')
                  return (
                    <label
                      key={skill.id}
