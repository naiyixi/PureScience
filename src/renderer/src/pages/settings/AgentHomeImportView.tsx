...
const SOURCE_INFO: Record<AgentHomeSkillSource, { label: TranslationKey; path: string }> = {
  agents: { label: 'settings.shared', path: '~/.agents/skills' },
  claude: { label: 'settings.claudeCode', path: '~/.claude/skills' },
  codex: { label: 'settings.codex', path: '~/.codex/skills' }
}

const skillKey = (skill: AgentHomeSkillRef): string => `${skill.source}:${skill.slug}`

// Lists skills installed outside PureScience's isolated agent profile, then copies the checked
  ...
  const runScan = useCallback(
    async (
      ...
      try {
        ...
        setSkillsFrameworkId(frameworkId)
        setSelected(new Set())
        setMessage(error instanceof Error ? error.message : t('settings.scanFailed'))
      } finally {
  ...
  const importSelected = async (): Promise<void> => {
    ...
    try {
      ...
      if (useSettingsStore.getState().agentFrameworkId === frameworkId) {
        setMessage(
          error instanceof Error ? error.message : t('settings.couldNotImportSelectedSkills')
        )
      }
    } finally {
      setImporting(false)
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <label className="flex items-center gap-1.5 text-xs text-muted-foreground">
                <input
                  type="checkbox"
                  aria-label={t('settings.selectAllInstalledSkills')}
                  checked={allSelected}
                  onChange={toggleAll}
                ...
                <li key={skillKey(skill)} className="flex items-center gap-3 py-2.5">
                  ...
                  >
                    ...
                    <span className="shrink-0 rounded-full bg-muted px-2 py-0.5 text-xs text-muted-foreground">
                      {skill.alreadyImported ? t('settings.imported') : t(source.label)}
                    </span>
                  </button>
                </li>
              )
