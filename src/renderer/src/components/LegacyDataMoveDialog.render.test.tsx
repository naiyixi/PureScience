...
const installApi = (overrides: Partial<MockStorageApi> = {}): MockStorageApi => {
  const api: MockStorageApi = {
    ...
    // Default: resolving the move destination (from defaultParent) yields the visible PureScience path.
    inspectDataRoot: vi.fn().mockResolvedValue({ kind: 'move', dataRoot: '/home/u/PureScience' }),
  ...
  await act(async () => {
    root.render(
      <LegacyDataMoveDialog
        currentDataRoot="/home/u/.purescience"
...
describe('LegacyDataMoveDialog', () => {
  it('uses shared settings dialog chrome without changing the move choices', async () => {
    ...
    expect(document.body.textContent).toContain('Move to PureScience')
  ...
  it('shows both paths and the three choices', async () => {
    ...
    expect(document.body.textContent).toContain('/home/u/.purescience')
    expect(document.body.textContent).toContain('/home/u/PureScience')
    for (const label of [
      /Move to PureScience/,
  ...
  it('"Move to PureScience" hands off to the migration flow (detects sessions first)', async () => {
    const api = installApi()
    await renderDialog()

    await act(async () => {
      clickButton(/Move to PureScience/)
      await Promise.resolve()
    })
  ...
  it('a chosen empty folder starts the move; an unusable pick shows an inline error', async () => {
    const api = installApi({
      ...
      inspectDataRoot: vi
        .fn()
        .mockResolvedValue({ kind: 'invalid', dataRoot: '/mnt/bad/PureScience', error: 'Nope.' })
    })
    await renderDialog()
