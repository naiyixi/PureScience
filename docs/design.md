# PureScience shadcn/ui Design Specification

This specification defines the PureScience workspace design system. The design system is based on shadcn/ui, Radix primitives, Tailwind CSS variables, and semantic tokens. Use shadcn semantic tokens (`bg-background`, `text-foreground`, `bg-card`, and so on) by default. Use workspace tokens (`bg-bg-10`, `text-text-000`, and so on) only for the named surfaces listed in **Workspace Tokens** and the component guidelines below. The canonical token values live in `src/renderer/src/assets/main.css`.
...
- Use `neutral` as the `baseColor`; the PureScience deep green should only appear through semantic tokens such as `--primary` and `--ring`.
...
- Brand title: display `PureScience`, `text-[26px] leading-none font-medium`.
...
- Export uses a row-level `Download` action and immediately opens the native Save As dialog. The portable ZIP contains the Skill's `SKILL.md` and ordinary supporting files, excludes PureScience provenance/ownership metadata, and can be uploaded again through the standard Skill import flow. Cancellation is silent, a successful save shows a short status, and failures use the standard Settings danger banner.
...
- Product naming is consistently `PureScience` in visible app surfaces such as window titles, sidebars, app menus, about information, and help entry points.
...
- Support and diagnostics copy should use generic wording, such as "Contact PureScience support", "Download diagnostic logs", and "Share diagnostic ID".
