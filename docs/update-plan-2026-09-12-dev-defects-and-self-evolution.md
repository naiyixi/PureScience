...
- `permission-broker.ts` 的 `ConversationPermissionGrantStore`：`has / remember / revoke / list`（会话级类别记忆）；
- 请求带 `categoryKey` 时，应用**自动追加** "This session" 选项（`scope:'session'`，见 broker 约 874-905 行）；
...
**实测（59 个会话的 `contextUsage.breakdown.categories`，中位 `used` = 186,518 / 1,000,000）**：

| 分项 | 中位 tokens | 最大 | 占 used |
|---|---|---|---|
| **tools** | **115,006** | **818,272** | **≈62%** |
| other（估算差） | 43,721 | 366,880 | ≈23% |
| mcp | 32,826 | 153,409 | ≈18% |
| system | 4,042 | 4,583 | ≈2% |
| messages | 2,962 | 13,691 | ≈2% |
| skills | 53 | 115 | ≈0% |
...
- 一个 **turn** = 一条携带 `turnUsage.turnCount` 的 agent 消息；
- token 取 agent 消息上的 `turnUsage`（`inputTokens/outputTokens/cachedRead|WriteTokens`），**不是** `contextUsage`（那是上下文占用，非消耗）；
- **明确不使用"技能数增长"作为成功指标**（排期原文禁令）；
- "成功/百万 token" **没有一手信号**，故只作为**代理**呈现并标注，绝不与质量等同。

**实测（本机 59 个有内容的会话，56 个报告了用量）**：

| 指标 | 数值 |
|---|---|
| turns（Σ turnCount） | **4003** |
| input / output tokens | **5,727,844 / 5,351,121** |
| cache read / write tokens | **376,010,240 / 0** |
| 每 turn | in ≈ **1,431** / out ≈ **1,337** |
| 工具调用 | **3740**，失败 **360（9.6%）**；**45/59** 会话至少一次工具失败 |
| reviewer 结论（真实质量信号） | 评审 **7** 次：`flagged` 4 / `pass` 2 / 无结论 1；findings：pass 21 / warn 8 / fail 3 |
| 每百万计费 token 对应会话数（**代理**） | ≈ **0.0903** |

**计算不出的部分（如实标注）**：

- **逐版本对照表（v_n vs v_{n-1}）**：需要第二个版本的同一口径重算，当前只有基线，**不虚构趋势**；
- **SciBench 口径**：仓库内以确定性规则 + golden 基线形式存在（不是发版期数字），发版说明应引用其**通过率**而非把规则数当成绩；
- **成本/货币口径**：无定价接入，故不报金额。

**注**：cache read 占绝大多数（376M）说明长会话里前缀复用很高——但这属于工程事实，**不当作"效率提升"的宣传点**，除非有对照版本。
