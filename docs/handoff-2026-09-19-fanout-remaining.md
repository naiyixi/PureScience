...
## 一、`dbCanaryMs >100 ms` 高位：**已收口 —— 本机不可复现**（真实最重负载也压不出来）

- **结论**：在真实规模语料（123 会话 / 2607 产物 / 完整配置根）+ **真实最重负载**（并发读 12 个最大产物、合计 19.9 MB、每个解析含查询+整文件读+checksum）下，引擎金丝雀**最深 7 ms**、main 自身的慢 resolve 仪器（50 ms 阈值）**一条未触发**；12 个重 resolve 在 ~102 ms 内**并发**完成（未见串行排队）。U11 的 135/124 ms 判定为**环境特异**。
- **保留**：方向与量级 —— Home 扇出 **52 → 1**、启动期金丝雀中位 **42 ms → 3 ms**、无闸 vs 有闸排队深度 **~6×**。完整数字见 `docs/evidence/2026-09-19-u14-fanout-batched-verified.md` §11.9。
- **不再列为待办**；若要重开，需要一个能证明"引擎确实在排队"的新场景（而不是更大的语料）。
