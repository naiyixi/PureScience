...
| 判据 | 实测 |
|---|---|
| ① 启动期列表读仍 < 150 ms | **冷启动 68.2 ms / 稳态 p95 23.2 ms**（且当时那趟正在跑，见 `docs/evidence/2026-09-18-incremental-catalog-index.md`） |
...
**什么情况会重开**：① 出现"派生状态在启动后长时间不一致"的真机证据（不是猜）；② 该趟被放到用户可见路径上（例如将来让"文件"面板等它）；③ 语料规模上一个量级后，那趟的时间开始与用户操作重叠。届时按本档下面那张表选 **C（只改调度）** 或 **B（第二把尺）**。
