...
## 1. 启动协调仍是全量 —— **已按方案 D 收口（2026-09-18 拍板，零代码）**

**决定**：不再对它做增量；**判据改写**为"用户可见路径不受这趟影响"，本项以**已量清 + 结论落档**收口。

**三条判据的实测（都在本批内，非新测）**：

| 判据 | 实测 |
|---|---|
| ① 启动期列表读仍 < 150 ms | **稳态：冷启动 68.2 ms / p95 23.2 ms**（`docs/evidence/2026-09-18-incremental-catalog-index.md`）。**⚠️ 更正（U1 实测）：这一条不覆盖"启动后第一次读"** —— 那次读排在协调持有的数据根租约后面，实测 **489 ms**（`docs/evidence/2026-09-18-u1-ipc-channel-timing.md`）。 |
| ② 恢复/索引最终一致 | `status: 'ready'`、`sessionCount: 59`、`degradedReconciliationCount: 0`、`unpublishedArtifactVersionCount: 0`（dev 与打包版两侧各一次） |
| ③ 后台耗时继续可观测 | 该趟每次完成都记 `scanDurationMs` + 相位 `durationMs`（`688a716` 起） |

**放弃原案的理由**（已量）：启动那趟 ≈扫 512 ms + 协调 1336 ms；文档水位只能动扫描那半，而协调那半吃的是整份文档 **+ 产物存储/文件索引 DB 的状态**，文档没变也必须跑（恢复用例保护的正是这种情形）。

**D 的已知代价（接受）**：极端情况下用户**极早**打开"文件"面板，可能先看到尚未回填/恢复完成的列表，直到该趟跑完（数秒）。

**什么情况会重开**：① 出现"派生状态在启动后长时间不一致"的真机证据（不是猜）；② 该趟被放到用户可见路径上；③ 语料规模上一个量级后，那趟的时间开始与用户操作重叠。**④（U1 实测后新增，已触发）**：**启动后第一次列表读排在协调持有的数据根租约后面 —— 实测 489 ms**，而稳态只有 23–68 ms。这落在 ② 类，因此本条**重新立案**：要做的是**看清数据根租约的粒度与顺序**（协调是否必须全程持有它；列表读能否在其之前被服务），而不是回到文档水位那条路。届时按本档下面那张表选 **C（只改调度）** 或 **B（第二把尺）**。

**这趟的组成（本批实测；保留以便重开时对照）**：
...
- **结论**：**文档水位增量最多省掉 512 ms 里的一部分**，而 `reconcile-derived-state` 那 1336 ms 吃的是**整份文档 + 产物存储/文件索引 DB 的状态**，文档没变也必须跑（`artifact-finalization-recovery.integration.test.ts` 保护的正是这种"文档未变但需要恢复"的场景）。
