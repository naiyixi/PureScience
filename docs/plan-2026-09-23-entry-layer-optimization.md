...
| U7              | 未开始        | 10 处硬编码英文入 9 语字典（侧栏的 `Pinned/Active` 已顺带清掉）。**U6 侦察中新增定位（并入 U7）**：`pages/workspace/session-plan/SessionPlanSurfaces.tsx:141` 的 `Approve`/`Reject` 按钮与 `:150` 的 `● {n} confidence`；`components/omics/OmicsPreviewPanel.tsx` 整体硬编码中文 |
