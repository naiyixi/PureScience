...
| U6              | 1/27          | ✅ 文件视图搜索未命中：`No files match “x”` 由硬编码插值改为 `ws.noFilesMatch` + 提示「清空搜索框」（`ProjectFilesView.tsx:1908`，旧文案连句末标点都没有），同文件 `Loading...` 硬编码改用 `common.loading`。**剩余 26 处目标已定位**（`rg -c` 出清单）：`stores/settings-preferences-slice.ts`(7) 与 `stores/settings-provider-auth-slice.ts`(4) 的 toast 失败文案、`ReportErrorDialog.tsx`(3) 剪贴板/日志揭示失败、`MoleculePreview.tsx:66` 渲染失败兜底文案、`LocalFileBrowser`/`SessionNotebookDialog`/`DownloadSessionArtifactsDialog`/`HandoffLifecycleStatus`/`FoldTimelinePanel`/`ConversationPanel`/`ArtifactMentionPopup`/`SkillEditor`/`SpecialistsPanel`/`StoragePanel`/`validation-message` 各 1 处；纯 `console.error` 的（`PdfPreview.tsx:177,557`、`MoleculePreview.tsx:80`）**归档**（非用户可见）                                                                                                                                                                                                                                                                     |
