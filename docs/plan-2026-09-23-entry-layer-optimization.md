...
| U6              | 3/27 + 归档   | ✅ 文件视图搜索未命中：`No files match “x”` 由硬编码插值改为 `ws.noFilesMatch` + 提示「清空搜索框」（`ProjectFilesView.tsx:1908`），同文件 `Loading...` 改用 `common.loading`。✅✅ **store 层 11 条 toast 失败文案改为字典 key**：新增 `stores/settings-write-error-keys.ts`（11 个 key + `isSettingsWriteErrorKey` 守卫），`settings-preferences-slice.ts`(7) 与 `settings-provider-auth-slice.ts`(4) 不再拼英文句子，`SettingsPage.tsx:1086` 单点翻译（非 key 的原始 IPC 报文原样透传，已有测试守住）。**剩余 13 处**：`ReportErrorDialog.tsx:129,143,145`（剪贴板/日志揭示失败）、`MoleculePreview.tsx:66` 渲染兜底，以及 `LocalFileBrowser`/`SessionNotebookDialog`/`DownloadSessionArtifactsDialog`/`HandoffLifecycleStatus`/`FoldTimelinePanel`/`ConversationPanel`/`ArtifactMentionPopup`/`SkillEditor`/`SpecialistsPanel`/`validation-message` 各 1 处；纯 `console.error` 的（`PdfPreview.tsx:177,557`、`MoleculePreview.tsx:80`）**归档**（非用户可见）                                                                                                                      |
| U7              | 未开始        | 10 处硬编码英文入 9 语字典（侧栏的 `Pinned/Active` 已顺带清掉）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
