...
### U7d 立案：omics 面板不是「换几个标签」，是语义层要改形状

**为什么单独立案**：`components/omics/OmicsPreviewPanel.tsx` 与 `OmicsPreviewLauncher.tsx` 的可见文案是硬编码中文（面板 10 处、launcher 10 处），但它们展示的**句子不产生在组件里**——`describeOmicsScope` / `summarizeOmicsPreview` 位于 `src/shared/omics-preview.ts:60`、`:78`、`:100`，直接返回成品句子：

- `src/shared/omics-preview.ts:60` `describeOmicsScope` → `基于 2000/20000 降采样（头部截取）` / `全量（20000）`
- `src/shared/omics-preview.ts:100` → `预览即全量（…），可直接用于结论；仍需标注数据来源与版本。`
- 同文件还产出 `需全量计算` / `未选择计算主机` 等提案用词

`src/shared/` 是**主进程与渲染进程共用的模块**，所以这里不能像 store 那样「发 key、渲染层翻译」——必须先判定这些字符串有没有非 UI 消费者（agent 工具回包、`purescience` CLI、`.preview.json` 清单文本）。两条路：

1. **结构化返回**（推荐，若无非 UI 消费者或消费者可接受改形状）：helper 返回 `{ kind: 'downsampled', shown: 2000, total: 20000 }` 这类判定结果，由渲染层 `t()` 成句（语序不同的语言也能正确成句）。
2. **注入 `t`**（若调用面太散）：沿用 `describeValidation(result, t)` 的形状。

**验收要求**：分子层测试 `components/omics/OmicsPreviewPanel.render.test.tsx:58-113`、`OmicsPreviewLauncher.render.test.tsx`、`shared/omics-preview` 的用例目前**断言中文字符串**（如 `toBe('基于 2000/20000 降采样（头部截取）')`），改形状时这些断言要一并改成读 key / 读结构化字段——不能为了让测试过而保留中文兜底。

**不得偷懒**：面板里 `预览为子集：不得作为最终结论交付…` 是**安全性声明**（防止把降采样预览当结论交付），翻译必须保持这层禁止语气，不能弱化成「预览结果」这类中性描述。
