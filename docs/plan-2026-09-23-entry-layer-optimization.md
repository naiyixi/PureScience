...
| 单元            | 状态   | 已落地 / 剩余                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| --------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| U5（11 处空态） | 2/11   | ✅ `e8c3376` 侧栏会话空态（说明 + 下一步按钮，分组标题与 nav/settings 无障碍名入 i18n）；✅ `e041249` 权限面板空态（原来写成 `sr-only`，现区分「完全没记住」与「当前范围为空」并给清除筛选按钮）。剩余 9 处：`GlobalSearchDialog.tsx`（浏览态 `length>0` 门控 + pin 空态 + 无结果文案）、`AgentPanel.tsx:655,664`、`ArtifactProvenancePanel.tsx`、`ContextWindowDialog.tsx`、`OmicsPreviewPanel.tsx`、`StoragePanel.tsx:495`、`MemoryPanel.tsx`、`SkillUploadView.tsx`、`ConnectorImportView.tsx` |
| U6              | 未开始 | 15 处加载态 + 12 处错误态（含重试）                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
