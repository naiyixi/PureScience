...
**下一步（未开工）**：写一个一次性探针 spec，复现后把数据根整份拷到固定路径，再直接查清单文件的真实哈希与 DB 里那条运行记录引用的 checksum，把分歧点钉到具体字段/具体写入路径。定位到之前不动代码。
