...
**判读**：墙钟上仍**分辨不出**（−3% 不构成声明）。图标帧**平掉**有明确原因：`streaming-profile.spec.ts` 只给**前 3 个流式回合**挂 profiler，而本改动的收益**随时间线变长而放大**（harness 里是 12 行规模，profile 采样时时间线还很短）⇒ 要真机量出这一项，需要「长转录后对下一回合取 profile」的 spec（已立案，未做，不声称）。因此本单元的证据是确定性读数：**一次活动更新重画的行数 12 → 1**（图标 13 → 2），并保留活性断言防冻结。
