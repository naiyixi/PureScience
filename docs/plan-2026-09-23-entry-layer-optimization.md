...
### 下一层立项（有数，未动）：每块一次的 `acp:state` 整份快照

主进程发报探针实测（40 块/回合，可见正文 0.7KB）：`acp:state` **40/26/32 次、单条 7.5–30KB、回合合计 294KB→941KB**，随转录线性增长；`acp:event` 是薄的（≈400B/条）。这是「会话越长越卡」的直接来源，也是渲染进程 `wrappedListener`（Electron 桥）在流式 profile 里占 23% 的主要来源。方向：(a) 每块只发增量；(b) 发送侧加合并窗。两者都改主进程发送语义，须单独立项 + 用同一台探针做前后对照。明细见 `docs/evidence/2026-09-25-interaction-smoothness.md`「下一层」一节。
