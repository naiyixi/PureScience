...
- 仍待处理：其余「无 Trigger」的 Radix 对话框（`grep -rl "Dialog.Content" src/renderer/src --include=*.tsx` 减去 `Dialog.Trigger`，共 47 个文件，其中仅卸载式关闭的会真丢焦点）。**不并入本批次**：无逐条验证的批量改动属于半截工程，需作为独立单元逐个接线并补测试。
  ...
  - **✅ 该观察已落地为修复（嵌套模态的 Escape 只关最内层）**：机制是 Radix 把 Escape 监听挂在 `document` 上 ⇒
    一层里的按键被**每一层**都收到 ⇒ 设置面与上层确认框同时关闭（真机失败截图：Escape 后回到工作区，开它的
    `Change location` 随之消失）。修法为**单点**（`SettingsPage` 的 `Dialog.Content`）：有更上层时不关自己，
    把同一个 Escape 转发给最上层（`closeActivePane` 已在用的手法）。两个坑写进了代码注释：
    ① Radix 回调里 `event.currentTarget` 是 **document**，不能用它做"是否在本层内"的判定（改用"目标是否落在最上层内"）；
    ② 转发事件会回到同一监听器 ⇒ 用组件内 ref 做转发标志，否则死循环（实测 521 次）。
    真机断言加在确认框用例上：Escape ⇒ 确认框关、**设置面仍在**、焦点回开者；整份 spec 6 passed（设置面自身 Escape 关闭未回归）。
...
**结论（不夸大）**：可归因于应用渲染路径的只有 **~1.0–1.2ms/键（约 12–15%）**，其余是输入事件/contenteditable 编辑器/GC 的固有开销；8ms/键仍在 60fps 帧预算（16.7ms）内。⇒ **不为 15% 去重构页面状态归属**（把 `draftDoc` 下沉到 composer 区域或专用 store 的改法已写明，**立案，低值 ⇒ 本轮按审计归档零代码收口**：
    实测可优化仅 ~1.0–1.2ms/键、8ms/键仍在 60fps 帧预算内，且收益集中在输入事件/contenteditable 的固有开销之外，
    "不为 15% 去重构页面状态归属"这条判断不变）。若"卡"的体感仍在，更可能来自**流式期间的突发 work**（每轮 ~150ms 主线程），而不是打字或滚动本身。
