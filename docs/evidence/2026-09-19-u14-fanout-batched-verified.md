...
## 十、源头修法：把"每卡片一次可用性探测"改成按 tick 批量（已落地并有真机收益）

### 10.1 先量：那 40 次探测为什么贵

给探针加**数值指纹**（只记 `read-preview` 的 `maxBytes`，不记路径/内容）后，84 次 `read-preview` 拆开：

| 指纹 | 调用数 | 峰值 | **单次耗时中位/最大** | 调用者 |
|---|---|---|---|---|
| `mb=1` | 40 | **15** | **22 / 43 ms** | 会话转写每条产物卡片的可用性探测 |
| `mb=32768` | 44 | 8 | 11 / 42 ms | 产物卡片预览 |
| `mb=262144` | **0** | — | — | 缩略图（上一轮那把闸从未被走到，已撤） |

根因：转写卡片的 `path` 是**版本定位符**，因此每次"1 字节探测"在 main 里都走 `resolveVersionContent` —— **一次 DB 查询 + 整文件读取 + sha256 校验**，只为了回答一个是/否。

### 10.2 改动

- **主进程**：`provenance.resolveVersionPaths({projectId, versionIds})` —— 整批一次 `findMany({id:{in}})` 返回路径，**不读文件、不校验**；新增 `artifacts:probe-availability`（`probeAvailability` handler）：定位符按项目批量解析、其余走既有 fs 路径解析、最后并行 `stat`，结果**按提问顺序**返回，输入先过滤再按路径去重、按项目去重 version id，上限 200。两端注册（Electron IPC + 本地 Web）、catalog/preload/生成映射/契约计数全部同步。
- **渲染层**：`previews/preview-availability.ts` —— 同一 tick 内所有卡片的请求**合并成一次**调用，带结果缓存；**卡片之间不互相阻塞**（这正是上一版共享闸被撤回的原因），失败不缓存可重试，无桥时不悬挂。`useUnavailablePreviewProbe` 改走批量；`local` 等非托管来源保留原单路径读（行为不变）。
- **用例**：主进程 4 条（一次查询覆盖整批 / `resolveVersionContent` 零调用 / 归类 / 去重与超限），渲染层 4 条（一次调用覆盖同 tick 全部请求 / 缓存 / 失败可重试 / 无桥不悬挂），并更新 3 条既有用例到新契约。

### 10.3 真机收益（同一驱动、同一仪器，`cmtuc9z5…` 项目 + 文件面板）

| | 改造前 | 改造后 |
|---|---|---|
| `read-preview mb=1` | **40 次 / 峰值 15 / 中位 22 ms** | **0 次** |
| `artifacts:probe-availability` | — | **2 次**（n=15、n=5）/ **峰值 1 / 中位 2 ms** |
| 卡片预览 `read-preview mb=32768` | 44 次 / 峰值 8 / 中位 **11 ms** | 44 次 / 峰值 8 / 中位 **4 ms** |
