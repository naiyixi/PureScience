...
## 流畅度实测基线（用户长期反馈「卡、不如对标流畅」，此前无任何测量设施）

新增 `e2e/perf/smoothness.spec.ts` + `npm run test:e2e:perf`（**不进 CI 认证矩阵**：性能断言在共享 runner 上必然抖，而抖的断言会被关掉，比没有更糟）。测量三类与手感直接相关的量：渲染主线程**长任务**（PerformanceObserver `longtask`）、**帧间隔** p95/最大值（最大间隔才是掉帧堆积的信号）、**IPC 往返**百分位，外加动作→可见耗时。

探针自身的坑（已修）：`observe({buffered: true})` 会把应用启动以来的历史长任务**重放进每个阶段**，四个不同交互因此报同一个 `longest=136ms` 的假值。

### 10 轮会话（小会话）

| 场景        | 最长阻塞任务       | 帧间隔 p95 / 最大 | 其他                            |
| ----------- | ------------------ | ----------------- | ------------------------------- |
| IPC         | —                  | —                 | `projects.list` p50=1ms p95=6ms |
| 流式 10 轮  | 0ms（>50ms：0 条） | 18ms / 19ms       | —                               |
| 输入框打字  | 0ms                | 18ms / 19ms       | —                               |
| 滚动记录    | 0ms                | 18ms / 19ms       | —                               |
| ⌘K 命令面板 | 0ms                | 18ms / 18ms       | **44ms 出可见**                 |
| 切回重会话  | 0ms                | 17ms / 17ms       | **57ms 出可见**                 |

### 45 轮会话（`PERF_TURNS=45`）——拐点

| 场景        | 最长阻塞任务                  | 帧间隔 p95 / 最大 | 其他             |
| ----------- | ----------------------------- | ----------------- | ---------------- |
| 流式 45 轮  | **78ms（>50ms：23 / 24 条）** | **35ms / 99ms**   | —                |
| 输入框打字  | 0ms                           | 19ms / **50ms**   | —                |
| 滚动记录    | 0ms                           | 18ms / 19ms       | —                |
| ⌘K 命令面板 | 50ms                          | 34ms / 34ms       | **71ms 出可见**  |
| 切回重会话  | 65ms                          | 49ms / 49ms       | **129ms 出可见** |

结论：**小会话流畅（无一条超 50ms、最长帧间隔 19ms）**；**长会话里流式回复期间几乎每次都超 50ms 预算**（23/24），帧间隔峰值 99ms ⇒ 这就是用户说的「卡」，拐点在 10～45 轮之间。下一轮据此定位渲染热点并给出前后对比数字。
