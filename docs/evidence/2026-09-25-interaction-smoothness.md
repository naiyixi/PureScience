...
**为什么图标帧是平的（诚实解释）**：`streaming-profile.spec.ts` 只给**前 3 个流式回合**挂 profiler，采样时时间线还很短，而本改动收益**随行数放大**（harness 用的是 12 行规模）⇒ 真机量这一项需要「长转录后对下一回合取 profile」的 spec（已立案，未做，不声称）。本单元证据 = 确定性读数 12 → 1 行。
