...
目标：`acp:state` 的每块整份快照（**0.3–0.9MB/回合**，随转录长度线性增长，是「会话越长越卡」的直接来源）。方向二选一：(a) 每块只发增量（像 `acp:event` 那样薄）；(b) 至少加合并窗（同本轮保存回声的手法，把 40 次/回合压到个位数）。两者都需要主进程侧的发送语义改动，须单独立项 + 复用本文件里这台探针做前后对照。
