...
## 四、判据改写（2026-09-18 拍板：方案 D 收口）

这条立项的**起因**是"读列表要等 1.2 s"。那条路径现在 **23 ms**，而这一趟（扫 512 ms + 协调 1336 ms）是 `startupCleanupEligible` 的 **fire-and-forget**，跑在启动边界上、不挡任何用户操作。因此判据从**"这趟 < 1 s"**改写为：

> **用户可见路径不受这趟影响**：① 启动期列表读仍 < 150 ms；② 恢复/索引最终一致；③ 这趟的耗时继续可从日志读出。

三条都已在本档实测（① 68.2 ms / p95 23.2 ms；② `status: ready`、0 降级；③ `scanDurationMs` + 相位 `durationMs`）。**接受的新增代价**：用户极早打开"文件"面板时，可能先看到尚未回填完成的结果，直到该趟跑完（数秒）。**重开条件**：见 `docs/handoff-2026-09-18-session-catalog-remaining.md` 第 1 条。
