...
- **A ✅ spike 通过（2026-09-13，真实会话实测）——结论：走 A**
  ...
  **消费端进度（2026-09-13）**：
  ...
  - ✅ **已完成并实机验证（2026-09-13，提交 `cfe4cec`）**：真实 UI 面板显示
    `复用 1 次 · 失败 0`（技能 `mcp-genes`），与采集文件内容一致 → 链路 **agent 加载 → 钩子采集 → 主进程读取 → IPC → store → 面板** 全部跑通。
  - ❗ **验证中发现并修掉的真缺口（非管道 bug）**：`mcp-genes` 是**连接器技能**（物化给 agent 用、**不进技能面板**），
    面板 65 行里没有它 → 这条真实复用**会被记录却无处显示**，而说明文字还写着"有记录"，看起来像坏了。
    现已新增**未挂靠区块**（`data-testid="skill-reuse-unattached"`，标注"连接器提供或已移除"），9 语种译文 + render 测试钉住。
  - ⚠️ **两个配套坑（已写入技能，勿再犯）**：① 新增应用命令必须同时落在**命令映射 + 命令组 + handler/IPC** 三处，
    否则运行时清单认证 `certifyInventory()` 在 **应用启动阶段**抛 `Application command inventory mismatch`
    → **应用完全打不开**（单测全绿也照样发生，因为证书只在启动时评估）；② 渲染端改动后必须 `npm run build:web`，
    否则浏览器 UI 报 `Incompatible PureScience Web RPC protocol`（协议版本随目录变化，旧包不匹配）。
  - **历史 119 个会话仍无名**（采集只从装了钩子的版本开始）——面板已用说明文字显式标注，未冒充回溯覆盖。
