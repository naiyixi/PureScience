...
1. **方法学正确性**：识别出 reduced 数据集前置处理已 normalize+log1p+scale、原始 count 未存——**主动放弃重跑 normalize_total**（对已归一化数据再归一化属方法学错误），并在脚本/报告中声明取舍与 `target_sum=1e4` 惯例。独立核查 X/raw 结构确认判断成立。
