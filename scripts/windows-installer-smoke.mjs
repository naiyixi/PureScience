...
const APP_EXECUTABLE = 'purescience.exe'
const ARTIFACT_MCP_SERVER_ARG = '--purescience-artifact-mcp'
const NOTEBOOK_MCP_SERVER_ARG = '--purescience-notebook-mcp'
const CONFIG_DIRECTORY = '.purescience'
...
const SMOKE_ROOT_PREFIX = 'purescience-installer-smoke-'
const RPC_SMOKE_ROOT_PREFIX = 'purescience-rpc-smoke-'
...
const installerVersion = (installer) => {
  const match = basename(installer).match(/^zerolink-purescience-(.+)-win-x64-setup\.exe$/i)
...
const runPackagedLocalRpcSmoke = async ({ installDirectory, env }) => {
  ...
  const socketPath = `\\\\.\\pipe\\purescience-installer-smoke-${process.pid}-${randomUUID()}`
  ...
  try {
    ...
    notebookMcp = await connectPackagedMcp({
      ...
      env: {
        ...sharedEnv,
        PURESCIENCE_NOTEBOOK_RPC_ENDPOINT: 'http://localhost',
        PURESCIENCE_NOTEBOOK_RPC_SOCKET_PATH: socketPath,
        PURESCIENCE_NOTEBOOK_RPC_TOKEN: token,
        PURESCIENCE_NOTEBOOK_PROJECT_NAME: 'installer-smoke-project',
        PURESCIENCE_NOTEBOOK_SESSION_ID: 'installer-smoke-session',
        PURESCIENCE_NOTEBOOK_WORKSPACE_CWD: workspace
      }
    })
    ...
    artifactMcp = await connectPackagedMcp({
      ...
      env: {
        ...sharedEnv,
        PURESCIENCE_ARTIFACT_STORAGE_ROOT: artifactStorage,
        PURESCIENCE_ARTIFACT_PROJECT_NAME: 'installer-smoke-project',
        PURESCIENCE_ARTIFACT_SESSION_ID: 'installer-smoke-session',
        PURESCIENCE_ARTIFACT_CURRENT_RUN_FILE: currentRunFile,
        PURESCIENCE_ARTIFACT_ALLOWED_IMPORT_ROOTS: JSON.stringify([workspace]),
        PURESCIENCE_ARTIFACT_RPC_ENDPOINT: 'http://localhost',
        PURESCIENCE_ARTIFACT_RPC_SOCKET_PATH: socketPath
      }
    })
...
const launchAndProbe = async ({ installDirectory, expectedVersion, env, legacyConfigRoots }) => {
  ...
  const child = spawn(executable, ['--purescience-headless', '--serve=0'], {
    env,
    windowsHide: true
  })
  let stdout = ''
