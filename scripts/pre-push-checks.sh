...
# Scan only tracked files (staged + committed), skipping .web-rooter and dist.
HITS="$(git grep -niE "$SENSITIVE_PATTERNS" -- . ':(exclude).web-rooter' ':(exclude)dist' 2>/dev/null | grep -v 'Binary file' || true)"
if [[ -n "$HITS" ]]; then
  bad "发现敏感词（${VERSION}）:"
