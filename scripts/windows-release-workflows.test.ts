...
describe('post-merge Windows validation', () => {
  ...
  it('runs one canonical packaged P0 and visual gate plus native package smoke on every target', () => {
    ...
    expect(packaged.run).toContain('PureScience.app/Contents/MacOS/PureScience')
    expect(packaged.run).toContain('win-unpacked/purescience.exe')
    expect(packaged.run).toContain('linux-unpacked/purescience')
    expect(p0.env?.PURESCIENCE_E2E_EXECUTABLE).toBe('${{ steps.packaged_app.outputs.executable }}')
    expect(visual.env?.PURESCIENCE_E2E_EXECUTABLE).toBe(
  ...
  it('uploads built packages before enforcing collected certification outcomes', () => {
    ...
    // Visual regression is informational (env-sensitive on CI runners): the evidence step must not
    // gate on its outcome — it records passed/not-applicable inside the step instead.
    expect(evidence.if).not.toContain('steps.visual.outcome')
