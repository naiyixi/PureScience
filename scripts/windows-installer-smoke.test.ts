...
describe('Windows installer smoke plan', () => {
  it('selects one setup executable and rejects ambiguous artifacts', async () => {
    const root = await mkdtemp(join(tmpdir(), 'purescience-installer-artifacts-'))
    await writeFile(join(root, 'portable.zip'), '')
    await writeFile(join(root, 'zerolink-purescience-0.8.0-win-x64-setup.exe'), '')

    await expect(findSetupInstaller(root)).resolves.toBe(
      join(root, 'zerolink-purescience-0.8.0-win-x64-setup.exe')
    )

    await mkdir(join(root, 'nested'))
    await writeFile(join(root, 'zerolink-purescience-0.9.0-win-x64-setup.exe'), '')
  ...
  it('derives the packaged version from stable and nightly installer names', () => {
    expect(installerVersion('zerolink-purescience-0.8.0-win-x64-setup.exe')).toBe('0.8.0')
    expect(installerVersion('zerolink-purescience-0.8.0-nightly.abc1234-win-x64-setup.exe')).toBe(
  ...
  it('discovers the authenticated service endpoint from packaged app output', () => {
    expect(
      parsePackagedAppEndpoint(`
[main] app starting
PureScience Web: http://127.0.0.1:52378/?token=iUFHGSACwBz2k1kSJfPixHbclDywVg0CrcdTs42uvLE
`)
    ).toEqual({
  ...
  it('accepts only a packaged bootstrap that reports an absolute Windows config root', async () => {
    const configRoot = 'C:\\Users\\runneradmin\\.purescience'
    await expect(
      readPackagedAppConfigRoot(
        {
          appName: 'PureScience',
    ...
    await expect(
      readPackagedAppConfigRoot(
        { appName: 'PureScience', appVersion: '0.8.0', platform: 'win32' },
        '0.8.0'
      )
  ...
  it('authenticates whichever legacy config-root candidate the previous app actually used', async () => {
    const runnerConfigRoot = 'C:\\Users\\runneradmin\\.purescience'
    const isolatedConfigRoot = 'D:\\smoke\\profile\\.purescience'
    ...
    await expect(
      readPackagedAppConfigRoot(
        { appName: 'PureScience', appVersion: '0.7.0', platform: 'win32' },
        '0.7.0',
        {
  ...
  it('writes and verifies the upgrade sentinel in the app-reported config root', async () => {
    const profile = await mkdtemp(join(tmpdir(), 'purescience-upgrade-profile-'))
    const configRoot = join(profile, '.purescience')
  ...
  it('rejects an upgrade that starts the current app with a different config root', async () => {
    const previousConfigRoot = await mkdtemp(join(tmpdir(), 'purescience-previous-config-'))
    const currentConfigRoot = await mkdtemp(join(tmpdir(), 'purescience-current-config-'))
  ...
  it('removes the upgrade sentinel during cleanup after the current app preserves it', async () => {
    const configRoot = await mkdtemp(join(tmpdir(), 'purescience-upgrade-config-'))
  ...
  it('never overwrites or removes a pre-existing upgrade sentinel collision', async () => {
    const configRoot = await mkdtemp(join(tmpdir(), 'purescience-upgrade-collision-'))
  ...
  it('tracks multiple packaged resources for uninstall verification', () => {
    ...
    expect(packagedResourcePaths(installDirectory)).toEqual([
      join(installDirectory, 'purescience.exe'),
