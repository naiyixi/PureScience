...
describe('buildDevWebCommand', () => {
  it('injects the default web port when unset', () => {
    ...
    expect(env.PURESCIENCE_WEB_PORT).toBe(DEFAULT_WEB_PORT)
  })

  it('respects an existing PURESCIENCE_WEB_PORT', () => {
    const { env } = buildDevWebCommand(['node', 'dev-web.cjs'], { PURESCIENCE_WEB_PORT: '44200' })
    expect(env.PURESCIENCE_WEB_PORT).toBe('44200')
  })

  it('forwards --headless to Electron as the namespaced --purescience-headless flag', () => {
    const { args } = buildDevWebCommand(['node', 'dev-web.cjs', '--headless'], {})
    expect(args).toEqual(['electron-vite', 'dev', '--', '--purescience-headless', '--disable-gpu'])
  })
